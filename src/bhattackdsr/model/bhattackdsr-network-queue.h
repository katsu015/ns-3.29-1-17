 ...
 */
class BhattackdsrNetworkQueueEntry
{
public:
  /**
   * Construct a BhattackdsrNetworkQueueEntry with the given parameters
   *
   * \param pa packet
   ...
   * \param r Route
   */
  BhattackdsrNetworkQueueEntry (Ptr<const Packet> pa = 0, Ipv4Address s = Ipv4Address (), Ipv4Address n = Ipv4Address (),
   ...
   */
  bool operator== (BhattackdsrNetworkQueueEntry const & o) const
  {
...
class BhattackdsrNetworkQueue : public Object
{
public:
  /**
  ...
  BhattackdsrNetworkQueue ();
  /**
   * Construct a BhattackdsrNetworkQueue with the given
   * maximum length and maximum delay.
   *
   * \param maxLen Maximum queue size
   * \param maxDelay Maximum entry lifetime in the queue
   */
  BhattackdsrNetworkQueue (uint32_t maxLen, Time maxDelay);
  ~BhattackdsrNetworkQueue ();
   ...
   */
  bool FindPacketWithNexthop (Ipv4Address nextHop, BhattackdsrNetworkQueueEntry & entry);
  /**
   ...
   *         false otherwise
   */
  bool Enqueue (BhattackdsrNetworkQueueEntry & entry);
  /**
   ...
   *         false otherwise
   */
  bool Dequeue (BhattackdsrNetworkQueueEntry & entry);
  /**
   * Number of entries
   *
   ...
   * \return the current queue entry
   */
  std::vector<BhattackdsrNetworkQueueEntry> & GetQueue ()
  {
    return m_bhattackdsrNetworkQueue;
  }
...
private:
   ...
   */
  void Cleanup (void);
  std::vector<BhattackdsrNetworkQueueEntry> m_bhattackdsrNetworkQueue; //!< Queue (vector) of entries

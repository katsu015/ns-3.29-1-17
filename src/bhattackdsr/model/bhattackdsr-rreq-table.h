 ...
 */
class BhattackdsrReceivedRreqEntry
{
public:
  /**
   * Construct a BhattackdsrReceivedRreqEntry with the given parameters
   *
   * \param d IPv4 address of the destination
   * \param i identification
   */
  BhattackdsrReceivedRreqEntry (Ipv4Address d = Ipv4Address (), uint16_t i = 0)
    : m_destination (d),
      m_identification (i)
  {
  }
   ...
   * \param o another BhattackdsrReceivedRreqEntry
   * \return true if equal
   */
  bool operator== (BhattackdsrReceivedRreqEntry const & o) const
  {
...
/**
 * \ingroup bhattackdsr
 * \brief maintain list of BhattackdsrRreqTable entry
 */
class BhattackdsrRreqTable  : public Object
{
public:
  /**
  ...
  BhattackdsrRreqTable ();
  virtual ~BhattackdsrRreqTable ();
...
private:
  ...
  /// Map of entries
  std::list<BhattackdsrReceivedRreqEntry> m_sourceRequests;
  ...
  std::map<Ipv4Address, std::list<BhattackdsrReceivedRreqEntry> > m_sourceRreqMap;

 ...
 */
class BhattackdsrSendBuffEntry
{
public:
  /**
   * Construct BhattackdsrSendBuffEntry with the given parameters.
   *
   * \param pa packet
   ...
   */
  BhattackdsrSendBuffEntry (Ptr<const Packet> pa = 0, Ipv4Address d = Ipv4Address (),
   ...
   * \param o another BhattackdsrSendBuffEntry
   * \return true if equal
   */
  bool operator== (BhattackdsrSendBuffEntry const & o) const
  {
 ...
 */
/************************************************************************************************************************/
class BhattackdsrSendBuffer
{
public:
  /**
   * Default constructor
   */
  BhattackdsrSendBuffer ()
  {
  }
  /**
   ...
   *
   * \param entry BhattackdsrSendBuffEntry to put in the queue
   * \return true if successfully enqueued,
   *         false otherwise
   */
  bool Enqueue (BhattackdsrSendBuffEntry & entry);
  /**
   ...
   *         false otherwise
   */
  bool Dequeue (Ipv4Address dst, BhattackdsrSendBuffEntry & entry);
  /**
   ...
   * \return a pointer to the internal queue
   */
  std::vector<BhattackdsrSendBuffEntry> & GetBuffer ()
  {
    return m_sendBuffer;
  }

private:
  std::vector<BhattackdsrSendBuffEntry> m_sendBuffer;                   ///< The send buffer to cache unsent packet
  void Purge ();                                                ///< Remove all expired entries

  /// Notify that packet is dropped from queue by timeout
  /// \param en BuffEntry Buffer entry
  /// \param reason Drop reason
  void Drop (BhattackdsrSendBuffEntry en, std::string reason);
   ...
   *         false otherwise
   */
  static bool IsEqual (BhattackdsrSendBuffEntry en, const Ipv4Address dst)
  {

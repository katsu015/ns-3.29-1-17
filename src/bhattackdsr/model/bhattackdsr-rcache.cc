...
NS_LOG_COMPONENT_DEFINE ("BhattackdsrRouteCache");

namespace bhattackdsr {

bool CompareRoutesBoth (const BhattackdsrRouteCacheEntry &a, const BhattackdsrRouteCacheEntry &b)
{
...
bool CompareRoutesHops (const BhattackdsrRouteCacheEntry &a, const BhattackdsrRouteCacheEntry &b)
{
  // compare based on hops
  return a.GetVector ().size () < b.GetVector ().size ();
}

bool CompareRoutesExpire (const BhattackdsrRouteCacheEntry &a, const BhattackdsrRouteCacheEntry &b)
{
...
BhattackdsrNodeStab::BhattackdsrNodeStab (Time nodeStab)
  : m_nodeStability (nodeStab + Simulator::Now ())
{
}

BhattackdsrNodeStab::~BhattackdsrNodeStab ()
{
}

BhattackdsrLinkStab::BhattackdsrLinkStab (Time linkStab)
  : m_linkStability (linkStab + Simulator::Now ())
{
}

BhattackdsrLinkStab::~BhattackdsrLinkStab ()
{
}

void BhattackdsrLinkStab::Print ( ) const
{
  NS_LOG_LOGIC ("LifeTime: " << GetLinkStability ().GetSeconds ());
}

typedef std::list<BhattackdsrRouteCacheEntry>::value_type route_pair;

BhattackdsrRouteCacheEntry::BhattackdsrRouteCacheEntry (IP_VECTOR const  & ip, Ipv4Address dst, Time exp)
...
BhattackdsrRouteCacheEntry::~BhattackdsrRouteCacheEntry ()
{
}

void
BhattackdsrRouteCacheEntry::Invalidate (Time badLinkLifetime)
{
  m_reqCount = 0;
...
void
BhattackdsrRouteCacheEntry::Print (std::ostream & os) const
{
  os << m_dst << "\t" << (m_expire - Simulator::Now ()).GetSeconds ()
     << "\t";
}

NS_OBJECT_ENSURE_REGISTERED (BhattackdsrRouteCache);

TypeId BhattackdsrRouteCache::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::bhattackdsr::BhattackdsrRouteCache")
    .SetParent<Object> ()
    .SetGroupName ("Bhattackdsr")
    .AddConstructor<BhattackdsrRouteCache> ()
  ;
  return tid;
}

BhattackdsrRouteCache::BhattackdsrRouteCache ()
  : m_vector (0),
    ...
    m_delay (MilliSeconds (100))
{
  ...
  m_ntimer.SetFunction (&BhattackdsrRouteCache::PurgeMac, this);
  m_txErrorCallback = MakeCallback (&BhattackdsrRouteCache::ProcessTxError, this);
}
...
void
BhattackdsrRouteCache::RemoveLastEntry (std::list<BhattackdsrRouteCacheEntry> & rtVector)
{
...
bool
BhattackdsrRouteCache::UpdateRouteEntry (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
  std::map<Ipv4Address, std::list<BhattackdsrRouteCacheEntry> >::const_iterator i =
  ...
  if (i == m_sortedRoutes.end ())
    ...
    {
      std::list<BhattackdsrRouteCacheEntry> rtVector = i->second;
      BhattackdsrRouteCacheEntry successEntry = rtVector.front ();
       ...
       */
      std::pair<std::map<Ipv4Address, std::list<BhattackdsrRouteCacheEntry> >::iterator, bool> result =
...
bool
BhattackdsrRouteCache::LookupRoute (Ipv4Address id, BhattackdsrRouteCacheEntry & rt)
{
  ...
  if (IsLinkCache ())
  ...
  else
    {
      ...
      std::map<Ipv4Address, std::list<BhattackdsrRouteCacheEntry> >::const_iterator i = m_sortedRoutes.find (id);
      if (i == m_sortedRoutes.end ())
        {
          NS_LOG_LOGIC ("No Direct Route to " << id << " found");
          for (std::map<Ipv4Address, std::list<BhattackdsrRouteCacheEntry> >::const_iterator j =
                 m_sortedRoutes.begin (); j != m_sortedRoutes.end (); ++j)
            {
              std::list<BhattackdsrRouteCacheEntry> rtVector = j->second; // The route cache vector linked with destination address
              /*
               * Loop through the possibly multiple routes within the route vector
               */
              for (std::list<BhattackdsrRouteCacheEntry>::const_iterator k = rtVector.begin (); k != rtVector.end (); ++k)
                {
                  // return the first route in the route vector
                  BhattackdsrRouteCacheEntry::IP_VECTOR routeVector = k->GetVector ();
                  BhattackdsrRouteCacheEntry::IP_VECTOR changeVector;

                  for (BhattackdsrRouteCacheEntry::IP_VECTOR::iterator l = routeVector.begin (); l != routeVector.end (); ++l)
                    {
                      if (*l != id)
                        {
                  ...
                  if ((changeVector.size () < routeVector.size ())  && (changeVector.size () > 1))
                    {
                      BhattackdsrRouteCacheEntry changeEntry; // Create the route entry
                      ...
                      std::list<BhattackdsrRouteCacheEntry> newVector;
      ...
      std::map<Ipv4Address, std::list<BhattackdsrRouteCacheEntry> >::const_iterator m = m_sortedRoutes.find (id);
       ...
       */
      std::list<BhattackdsrRouteCacheEntry> rtVector = m->second;
...
void
BhattackdsrRouteCache::SetCacheType (std::string type)
{
...
bool
BhattackdsrRouteCache::IsLinkCache ()
{
  NS_LOG_FUNCTION (this);
...
void
BhattackdsrRouteCache::RebuildBestRouteTable (Ipv4Address source)
{
          ...
          for (std::map<Ipv4Address, uint32_t>::const_iterator k = m_netGraph[tempip].begin (); k != m_netGraph[tempip].end (); ++k)
            {
               ...
               */
                ...
                {
                  std::map<Link, BhattackdsrLinkStab>::iterator oldlink = m_linkCache.find (Link (k->first, pre[k->first]));
                  std::map<Link, BhattackdsrLinkStab>::iterator newlink = m_linkCache.find (Link (k->first, tempip));
  ...
  for (std::map<Ipv4Address, Ipv4Address>::iterator i = pre.begin (); i != pre.end (); ++i)
    {
      ...
      BhattackdsrRouteCacheEntry::IP_VECTOR route;
      ...
      if (!i->second.IsBroadcast () && iptemp != source)
        {
          ...
          // Reverse the route
          BhattackdsrRouteCacheEntry::IP_VECTOR reverseroute;
          for (BhattackdsrRouteCacheEntry::IP_VECTOR::reverse_iterator j = route.rbegin (); j != route.rend (); ++j)
            {
...
bool
BhattackdsrRouteCache::LookupRoute_Link (Ipv4Address id, BhattackdsrRouteCacheEntry & rt)
{
  NS_LOG_FUNCTION (this << id);
  /// We need to purge the link node cache
  PurgeLinkNode ();
  std::map<Ipv4Address, BhattackdsrRouteCacheEntry::IP_VECTOR>::const_iterator i = m_bestRoutesTable_link.find (id);
  if (i == m_bestRoutesTable_link.end ())
  ...
  else
    {
      ...
      BhattackdsrRouteCacheEntry newEntry; // Create the route entry
...
void
BhattackdsrRouteCache::PurgeLinkNode ()
{
  NS_LOG_FUNCTION (this);
  for (std::map<Link, BhattackdsrLinkStab>::iterator i = m_linkCache.begin (); i != m_linkCache.end (); )
    {
      NS_LOG_DEBUG ("The link stability " << i->second.GetLinkStability ().GetSeconds ());
      std::map<Link, BhattackdsrLinkStab>::iterator itmp = i;
  ...
  for (std::map<Ipv4Address, BhattackdsrNodeStab>::iterator i = m_nodeCache.begin (); i != m_nodeCache.end (); )
    {
      NS_LOG_DEBUG ("The node stability " << i->second.GetNodeStability ().GetSeconds ());
      std::map<Ipv4Address, BhattackdsrNodeStab>::iterator itmp = i;
...
void
BhattackdsrRouteCache::UpdateNetGraph ()
{
  NS_LOG_FUNCTION (this);
  m_netGraph.clear ();
  for (std::map<Link, BhattackdsrLinkStab>::iterator i = m_linkCache.begin (); i != m_linkCache.end (); ++i)
    {
...
bool
BhattackdsrRouteCache::IncStability (Ipv4Address node)
{
  NS_LOG_FUNCTION (this << node);
  std::map<Ipv4Address, BhattackdsrNodeStab>::const_iterator i = m_nodeCache.find (node);
  if (i == m_nodeCache.end ())
    {
      NS_LOG_INFO ("The initial stability " << m_initStability.GetSeconds ());
      BhattackdsrNodeStab ns (m_initStability);
      m_nodeCache[node] = ns;
      return false;
    }
  else
    {
      ...
      BhattackdsrNodeStab ns (Time (i->second.GetNodeStability () * m_stabilityIncrFactor));
      m_nodeCache[node] = ns;
      return true;
    }
  return false;
...
bool
BhattackdsrRouteCache::DecStability (Ipv4Address node)
{
  NS_LOG_FUNCTION (this << node);
  std::map<Ipv4Address, BhattackdsrNodeStab>::const_iterator i = m_nodeCache.find (node);
  if (i == m_nodeCache.end ())
  ...
  else
    {
      ...
      BhattackdsrNodeStab ns (Time (i->second.GetNodeStability () / m_stabilityDecrFactor));
      m_nodeCache[node] = ns;
      return true;
    }
  return false;
...
bool
BhattackdsrRouteCache::AddRoute_Link (BhattackdsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address source)
{
  NS_LOG_FUNCTION (this << source);
  ...
  for (uint32_t i = 0; i < nodelist.size () - 1; i++)
    {
      BhattackdsrNodeStab ns;                /// This is the node stability
      ...
      BhattackdsrLinkStab stab;                /// Link stability
...
void
BhattackdsrRouteCache::UseExtends (BhattackdsrRouteCacheEntry::IP_VECTOR rt)
{
  NS_LOG_FUNCTION (this);
  /// Purge the link node cache first
  PurgeLinkNode ();
  ...
  for (BhattackdsrRouteCacheEntry::IP_VECTOR::iterator i = rt.begin (); i != rt.end () - 1; ++i)
    {
      Link link (*i, *(i + 1));
  ...
  for (BhattackdsrRouteCacheEntry::IP_VECTOR::iterator i = rt.begin (); i != rt.end (); ++i)
    {
...
bool
BhattackdsrRouteCache::AddRoute (BhattackdsrRouteCacheEntry & rt)
{
  NS_LOG_FUNCTION (this);
  Purge ();
  std::list<BhattackdsrRouteCacheEntry> rtVector;   // Declare the route cache entry vector
...
bool BhattackdsrRouteCache::FindSameRoute (BhattackdsrRouteCacheEntry & rt, std::list<BhattackdsrRouteCacheEntry> & rtVector)
{
  NS_LOG_FUNCTION (this);
  for (std::list<BhattackdsrRouteCacheEntry>::iterator i = rtVector.begin (); i != rtVector.end (); ++i)
    {
      // return the first route in the route vector
      BhattackdsrRouteCacheEntry::IP_VECTOR routeVector = i->GetVector ();
      BhattackdsrRouteCacheEntry::IP_VECTOR newVector = rt.GetVector ();
...
bool
BhattackdsrRouteCache::DeleteRoute (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
...
void
BhattackdsrRouteCache::DeleteAllRoutesIncludeLink (Ipv4Address errorSrc, Ipv4Address unreachNode, Ipv4Address node)
{
  ...
  if (IsLinkCache ())
    {
      ...
      std::map<Ipv4Address, BhattackdsrNodeStab>::iterator i = m_nodeCache.find (errorSrc);
      if (i == m_nodeCache.end ())
        {
  ...
  else
    {
       ...
       */
      for (std::map<Ipv4Address, std::list<BhattackdsrRouteCacheEntry> >::iterator j =
             m_sortedRoutes.begin (); j != m_sortedRoutes.end (); )
        {
          std::map<Ipv4Address, std::list<BhattackdsrRouteCacheEntry> >::iterator jtmp = j;
          Ipv4Address address = j->first;
          std::list<BhattackdsrRouteCacheEntry> rtVector = j->second;
          /*
           * Loop all the routes for a single destination
           */
          for (std::list<BhattackdsrRouteCacheEntry>::iterator k = rtVector.begin (); k != rtVector.end (); )
            {
              // return the first route in the route vector
              BhattackdsrRouteCacheEntry::IP_VECTOR routeVector = k->GetVector ();
              BhattackdsrRouteCacheEntry::IP_VECTOR changeVector;
               ...
               */
              for (BhattackdsrRouteCacheEntry::IP_VECTOR::iterator i = routeVector.begin (); i != routeVector.end (); ++i)
                {
                  if (*i != errorSrc)
                    {
                  ...
                  if (m_subRoute)
                    {
                       ...
                       */
                      k = rtVector.erase (k);
                      BhattackdsrRouteCacheEntry changeEntry;
                      changeEntry.SetVector (changeVector);
...
void
BhattackdsrRouteCache::PrintVector (std::vector<Ipv4Address>& vec)
{
  NS_LOG_FUNCTION (this);
  /*
...
void
BhattackdsrRouteCache::PrintRouteVector (std::list<BhattackdsrRouteCacheEntry> route)
{
  NS_LOG_FUNCTION (this);
  for (std::list<BhattackdsrRouteCacheEntry>::iterator i = route.begin (); i != route.end (); i++)
    {
...
BhattackdsrRouteCache::Purge ()
{
  ...
  for (std::map<Ipv4Address, std::list<BhattackdsrRouteCacheEntry> >::iterator i =
         m_sortedRoutes.begin (); i != m_sortedRoutes.end (); )
    {
      // Loop of route cache entry with the route size
      std::map<Ipv4Address, std::list<BhattackdsrRouteCacheEntry> >::iterator itmp = i;
      /*
      ...
      if (rtVector.size ())
        {
          for (std::list<BhattackdsrRouteCacheEntry>::iterator j = rtVector.begin (); j != rtVector.end (); )
            {
...
void
BhattackdsrRouteCache::Print (std::ostream &os)
{
  NS_LOG_FUNCTION (this);
  Purge ();
  os << "\nDSR Route Cache\n"
     << "Destination\tGateway\t\tInterface\tFlag\tExpire\tHops\n";
  for (std::list<BhattackdsrRouteCacheEntry>::const_iterator i =
 ...
 */
uint16_t
BhattackdsrRouteCache::CheckUniqueAckId (Ipv4Address nextHop)
{
  NS_LOG_FUNCTION (this);
...
uint16_t
BhattackdsrRouteCache::GetAckSize ()
{
 ...
 */
bool
BhattackdsrRouteCache::IsNeighbor (Ipv4Address addr)
{
  NS_LOG_FUNCTION (this);
...
Time
BhattackdsrRouteCache::GetExpireTime (Ipv4Address addr)
{
  NS_LOG_FUNCTION (this);
  PurgeMac ();
...
void
BhattackdsrRouteCache::UpdateNeighbor (std::vector<Ipv4Address> nodeList, Time expire)
{
  NS_LOG_FUNCTION (this);
...
void
BhattackdsrRouteCache::AddNeighbor (std::vector<Ipv4Address> nodeList, Ipv4Address ownAddress, Time expire)
{
...
struct CloseNeighbor
{
   ...
   *
   * \param nb BhattackdsrRouteCache::Neighbor entry
   * \return true if expired or closed, false otherwise
   */
  bool operator() (const BhattackdsrRouteCache::Neighbor & nb) const
  {
...
void
BhattackdsrRouteCache::PurgeMac ()
{
  if (m_nb.empty ())
    {
      return;
...
void
BhattackdsrRouteCache::ScheduleTimer ()
{
  m_ntimer.Cancel ();
  m_ntimer.Schedule ();
}
...
void
BhattackdsrRouteCache::AddArpCache (Ptr<ArpCache> a)
{
  m_arp.push_back (a);
}

void
BhattackdsrRouteCache::DelArpCache (Ptr<ArpCache> a)
{
  m_arp.erase (std::remove (m_arp.begin (), m_arp.end (), a), m_arp.end ());
}

Mac48Address
BhattackdsrRouteCache::LookupMacAddress (Ipv4Address addr)
{
  Mac48Address hwaddr;
...
void
BhattackdsrRouteCache::ProcessTxError (WifiMacHeader const & hdr)
{

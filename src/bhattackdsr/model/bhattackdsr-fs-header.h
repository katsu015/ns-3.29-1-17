...
/**
 * \class BhattackdsrHeader
 * \brief Header for Bhattackdsr Routing.
 */

/**
* \ingroup bhattackdsr
* \brief Bhattackdsr fixed size header Format
  \verbatim
   |      0        |      1        |      2        |      3        |
...
/**
* \ingroup bhattackdsr
* \brief The modified version of Bhattackdsr fixed size header Format
  \verbatim
   |      0        |      1        |      2        |      3        |
   0 1 2 3 4 5 6 7 0 1 2 3 4 5 6 7 0 1 2 3 4 5 6 7 0 1 2 3 4 5 6 7
   +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  ...
  \endverbatim
*/
class BhattackdsrFsHeader : public Header
{
public:
  /**
...
/**
 * \class BhattackdsrOptionField
 * \brief Option field for an BhattackdsrFsHeader
 * Enables adding options to an BhattackdsrFsHeader
 *
 * Implementor's note: Make sure to add the result of
 * OptionField::GetSerializedSize () to your BhattackdsrFsHeader::GetSerializedSize ()
 * return value. Call OptionField::Serialize and OptionField::Deserialize at the
 * end of your corresponding BhattackdsrFsHeader methods.
 */
class BhattackdsrOptionField
{
public:
   ...
   */
  BhattackdsrOptionField (uint32_t optionsOffset);
  /**
   * \brief Destructor.
   */
  ~BhattackdsrOptionField ();
  /**
   * \brief Get the serialized size of the packet.
   * \return size
   */
   ...
   */
  void AddBhattackdsrOption (BhattackdsrOptionHeader const& option);
  /**
   ...
   */
  uint32_t GetBhattackdsrOptionsOffset ();
  /**
   * \brief Get the buffer.
   * \return buffer
   */
  Buffer GetBhattackdsrOptionBuffer ();
...
private:
   ...
   */
  uint32_t CalculatePad (BhattackdsrOptionHeader::Alignment alignment) const;
  /**
   * \brief Data payload.
   */
  Buffer m_optionData;
...
/**
 * \class BhattackdsrRoutingHeader
 * \brief Header of Bhattackdsr Routing
 */
class BhattackdsrRoutingHeader : public BhattackdsrFsHeader,
                         public BhattackdsrOptionField
{
public:
  /**
   * \brief Get the type identificator.
   ...
   * \brief Constructor.
   */
  BhattackdsrRoutingHeader ();
  /**
   * \brief Destructor.
   */
  virtual ~BhattackdsrRoutingHeader ();
  /**
   * \brief Print some information about the packet.
   * \param os output stream
   * \return info about this packet
...
static inline std::ostream & operator<< (std::ostream& os, const BhattackdsrRoutingHeader & bhattackdsr)
{
  bhattackdsr.Print (os);
  return os;
}

...
NS_LOG_COMPONENT_DEFINE ("BhattackdsrErrorBuffer");

namespace bhattackdsr {

uint32_t
BhattackdsrErrorBuffer::GetSize ()
{
  Purge ();
...
bool
BhattackdsrErrorBuffer::Enqueue (BhattackdsrErrorBuffEntry & entry)
{
  Purge ();
  for (std::vector<BhattackdsrErrorBuffEntry>::const_iterator i = m_errorBuffer.begin (); i
       != m_errorBuffer.end (); ++i)
    {
...
void
BhattackdsrErrorBuffer::DropPacketForErrLink (Ipv4Address source, Ipv4Address nextHop)
{
   ...
   */
  for (std::vector<BhattackdsrErrorBuffEntry>::iterator i = m_errorBuffer.begin (); i
       != m_errorBuffer.end (); ++i)
    {
      if (LinkEqual (*i, link))
        {
  ...
  m_errorBuffer.erase (std::remove_if (m_errorBuffer.begin (), m_errorBuffer.end (),
                                       std::bind2nd (std::ptr_fun (BhattackdsrErrorBuffer::LinkEqual), link)), m_errorBuffer.end ());
}

bool
BhattackdsrErrorBuffer::Dequeue (Ipv4Address dst, BhattackdsrErrorBuffEntry & entry)
{
  Purge ();
  /*
   * Dequeue the entry with destination address dst
   */
  for (std::vector<BhattackdsrErrorBuffEntry>::iterator i = m_errorBuffer.begin (); i != m_errorBuffer.end (); ++i)
    {
...
bool
BhattackdsrErrorBuffer::Find (Ipv4Address dst)
{
  /*
...
struct IsExpired
{
   ...
   */
  bool
  operator() (BhattackdsrErrorBuffEntry const & e) const
  {
...
void
BhattackdsrErrorBuffer::Drop (BhattackdsrErrorBuffEntry en, std::string reason)
{
...
void
BhattackdsrErrorBuffer::DropLink (BhattackdsrErrorBuffEntry en, std::string reason)
{

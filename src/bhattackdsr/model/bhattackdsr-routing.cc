...
#include "bhattackdsr-rreq-table.h"
#include "bhattackdsr-rcache.h"
#include "bhattackdsr-routing.h"
#include "bhattackdsr-fs-header.h"
...
NS_LOG_COMPONENT_DEFINE ("BhattackdsrRouting");

namespace bhattackdsr {

NS_OBJECT_ENSURE_REGISTERED (BhattackdsrRouting);

/* see http://www.iana.org/assignments/protocol-numbers */
const uint8_t BhattackdsrRouting::PROT_NUMBER = 48;
/*
 * The extension header is the fixed size bhattackdsr header, it is response for recognizing DSR option types
...
BhattackdsrRouting::BhattackdsrRouting ()
{
   ...
   */
  Ptr<bhattackdsr::BhattackdsrOptionPad1> pad1Option = CreateObject<bhattackdsr::BhattackdsrOptionPad1> ();
  Ptr<bhattackdsr::BhattackdsrOptionPadn> padnOption = CreateObject<bhattackdsr::BhattackdsrOptionPadn> ();
  Ptr<bhattackdsr::BhattackdsrOptionRreq> rreqOption = CreateObject<bhattackdsr::BhattackdsrOptionRreq> ();
  Ptr<bhattackdsr::BhattackdsrOptionRrep> rrepOption = CreateObject<bhattackdsr::BhattackdsrOptionRrep> ();
  Ptr<bhattackdsr::BhattackdsrOptionSR>   srOption = CreateObject<bhattackdsr::BhattackdsrOptionSR> ();
  Ptr<bhattackdsr::BhattackdsrOptionRerr>   rerrOption = CreateObject<bhattackdsr::BhattackdsrOptionRerr> ();
  Ptr<bhattackdsr::BhattackdsrOptionAckReq> ackReq = CreateObject<bhattackdsr::BhattackdsrOptionAckReq> ();
  Ptr<bhattackdsr::BhattackdsrOptionAck> ack = CreateObject<bhattackdsr::BhattackdsrOptionAck> ();
  ...
  m_sendBuffTimer.SetFunction (&BhattackdsrRouting::SendBuffTimerExpire, this);
...
void
BhattackdsrRouting::NotifyNewAggregate ()
{
  ...
  Simulator::ScheduleNow (&BhattackdsrRouting::Start, this);
}

void BhattackdsrRouting::Start ()
{
  ...
  for (uint32_t i = 0; i < m_numPriorityQueues; i++)
    {
      ...
      Ptr<bhattackdsr::BhattackdsrNetworkQueue> queue_i = CreateObject<bhattackdsr::BhattackdsrNetworkQueue> (m_maxNetworkSize,m_maxNetworkDelay);
      std::pair<std::map<uint32_t, Ptr<bhattackdsr::BhattackdsrNetworkQueue> >::iterator, bool> result_i = m_priorityQueue.insert (std::make_pair (i, queue_i));
      NS_ASSERT_MSG (result_i.second, "Error in creating queues");
    }
  Ptr<bhattackdsr::BhattackdsrRreqTable> rreqTable = CreateObject<bhattackdsr::BhattackdsrRreqTable> ();
  ...
  Ptr<bhattackdsr::BhattackdsrPassiveBuffer> passiveBuffer = CreateObject<bhattackdsr::BhattackdsrPassiveBuffer> ();
      ...
      for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
        {
          ...
          if (addr != loopback)
            {
              /*
               * bhattackdsrルートキャッシュを設定する
               */
              Ptr<bhattackdsr::BhattackdsrRouteCache> routeCache = CreateObject<bhattackdsr::BhattackdsrRouteCache> ();
              ...
              // routeCache->SetCallback (MakeCallback (&BhattackdsrRouting::SendRerrWhenBreaksLinkToNextHop, this));
              SetRouteCache (routeCache);
              // Set the main address as the current ip address
              m_mainAddress = addr;

              m_ipv4->GetNetDevice (1)->SetPromiscReceiveCallback (MakeCallback (&BhattackdsrRouting::PromiscReceive, this));
...
Ptr<NetDevice>
BhattackdsrRouting::GetNetDeviceFromContext (std::string context)
{
...
BhattackdsrRouting::GetElementsFromContext (std::string context)
{
...
void
BhattackdsrRouting::DoDispose (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  m_node = 0;
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
...
void
BhattackdsrRouting::SetNode (Ptr<Node> node)
{
...
Ptr<Node>
BhattackdsrRouting::GetNode () const
{
  NS_LOG_FUNCTION_NOARGS ();
  return m_node;
}

void BhattackdsrRouting::SetRouteCache (Ptr<bhattackdsr::BhattackdsrRouteCache> r)
{
  // / Set the route cache to use
  m_routeCache = r;
}

Ptr<bhattackdsr::BhattackdsrRouteCache>
BhattackdsrRouting::GetRouteCache () const
{
  // / Get the route cache to use
  return m_routeCache;
}

void BhattackdsrRouting::SetRequestTable (Ptr<bhattackdsr::BhattackdsrRreqTable> q)
{
  // / Set the request table to use
  m_rreqTable = q;
}

Ptr<bhattackdsr::BhattackdsrRreqTable>
BhattackdsrRouting::GetRequestTable () const
{
  // / Get the request table to use
  return m_rreqTable;
}

void BhattackdsrRouting::SetPassiveBuffer (Ptr<bhattackdsr::BhattackdsrPassiveBuffer> p)
{
  // / Set the request table to use
  m_passiveBuffer = p;
}

Ptr<bhattackdsr::BhattackdsrPassiveBuffer>
BhattackdsrRouting::GetPassiveBuffer () const
{
  // / Get the request table to use
...
Ptr<Node>
BhattackdsrRouting::GetNodeWithAddress (Ipv4Address ipv4Address)
{
...
bool BhattackdsrRouting::IsLinkCache ()
{
  return m_routeCache->IsLinkCache ();
}

void BhattackdsrRouting::UseExtends (BhattackdsrRouteCacheEntry::IP_VECTOR rt)
{
  m_routeCache->UseExtends (rt);
}

bool BhattackdsrRouting::LookupRoute (Ipv4Address id, BhattackdsrRouteCacheEntry & rt)
{
  return m_routeCache->LookupRoute (id, rt);
}

bool BhattackdsrRouting::AddRoute_Link (BhattackdsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address source)
{
...
bool BhattackdsrRouting::AddRoute (BhattackdsrRouteCacheEntry & rt)
{
...
void BhattackdsrRouting::DeleteAllRoutesIncludeLink (Ipv4Address errorSrc, Ipv4Address unreachNode, Ipv4Address node)
{
  m_routeCache->DeleteAllRoutesIncludeLink (errorSrc, unreachNode, node);
}

bool BhattackdsrRouting::UpdateRouteEntry (Ipv4Address dst)
{
  return m_routeCache->UpdateRouteEntry (dst);
}

bool BhattackdsrRouting::FindSourceEntry (Ipv4Address src, Ipv4Address dst, uint16_t id)
{
  return m_rreqTable->FindSourceEntry (src, dst, id);
}

Ipv4Address
BhattackdsrRouting::GetIPfromMAC (Mac48Address address)
{
...
void BhattackdsrRouting::PrintVector (std::vector<Ipv4Address>& vec)
{
...
Ipv4Address BhattackdsrRouting::SearchNextHop (Ipv4Address ipv4Address, std::vector<Ipv4Address>& vec)
{
  NS_LOG_FUNCTION (this << ipv4Address);
  Ipv4Address nextHop;
...
Ptr<Ipv4Route>
BhattackdsrRouting::SetRoute (Ipv4Address nextHop, Ipv4Address srcAddress)
{
...
int
BhattackdsrRouting::GetProtocolNumber (void) const
{
...
uint16_t
BhattackdsrRouting::GetIDfromIP (Ipv4Address address)
{
  int32_t nNodes = NodeList::GetNNodes ();
  for (int32_t i = 0; i < nNodes; ++i)
    {
...
Ipv4Address
BhattackdsrRouting::GetIPfromID (uint16_t id)
{
  if (id >= 256)
    {
...
uint32_t
BhattackdsrRouting::GetPriority (BhattackdsrMessageType messageType)
{
...
void BhattackdsrRouting::SendBuffTimerExpire ()
{
      ...
      if (findRoute)
        {
          ...
          Ptr<Packet> bhattackdsrPacket = packet->Copy ();
          bhattackdsrPacket->RemoveHeader (bhattackdsrRoutingHeader);
          uint32_t offset = bhattackdsrRoutingHeader.GetBhattackdsrOptionsOffset ();
          ...
          if (optionType == 3)
            {
              Ptr<bhattackdsr::BhattackdsrOptions> bhattackdsrOption;
              ...
              if (errorType == 1) // This is the Route Error Option
                {
                  ...
                  if (nextHop == "0.0.0.0")
                    {
                      PacketNewRoute (bhattackdsrPacket, m_mainAddress, destination, protocol);
                      return;
                    }
                  ...
                  bhattackdsrRoutingHeader.SetSourceId (GetIDfromIP (m_mainAddress));
                  bhattackdsrRoutingHeader.SetDestId (255);
                  bhattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 4);
                  bhattackdsrRoutingHeader.AddBhattackdsrOption (newUnreach);
                  bhattackdsrRoutingHeader.AddBhattackdsrOption (sourceRoute);

                  Ptr<Packet> newPacket = Create<Packet> ();
                  newPacket->AddHeader (bhattackdsrRoutingHeader); // Add the routing header with rerr and sourceRoute attached to it
                  ...
                  std::map<uint32_t, Ptr<bhattackdsr::BhattackdsrNetworkQueue> >::iterator i = m_priorityQueue.find (priority);
                  Ptr<bhattackdsr::BhattackdsrNetworkQueue> bhattackdsrNetworkQueue = i->second;
                  ...
                  /// \todo New BhattackdsrNetworkQueueEntry
                  BhattackdsrNetworkQueueEntry newEntry (newPacket, m_mainAddress, nextHop, Simulator::Now (), m_ipv4Route);

                  if (bhattackdsrNetworkQueue->Enqueue (newEntry))
                    {
                      Scheduler (priority);
                    }
                  else
                    {
                      NS_LOG_INFO ("Packet dropped as bhattackdsr network queue is full");
                    }
                }
            }
          else
            {
              bhattackdsrRoutingHeader.SetNextHeader (protocol);
              bhattackdsrRoutingHeader.SetMessageType (2);
              bhattackdsrRoutingHeader.SetSourceId (GetIDfromIP (m_mainAddress));
              bhattackdsrRoutingHeader.SetDestId (GetIDfromIP (destination));
              ...
              bhattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 2);
              bhattackdsrRoutingHeader.AddBhattackdsrOption (sourceRoute);
              cleanP->AddHeader (bhattackdsrRoutingHeader);
              Ptr<const Packet> mtP = cleanP->Copy ();
              // Put the data packet in the maintenance queue for data packet retransmission
              BhattackdsrMaintainBuffEntry newEntry (/*Packet=*/ mtP, /*Ipv4Address=*/ m_mainAddress, /*nextHop=*/ nextHop,
              ...
              Simulator::Schedule (m_sendBuffInterval, &BhattackdsrRouting::SendBuffTimerExpire, this);
              return;
            }
        }
      else
...
bool BhattackdsrRouting::PromiscReceive (Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol, const Address &from,
                                 const Address &to, NetDevice::PacketType packetType)
{
  ...
  if (ipv4Header.GetProtocol () != BhattackdsrRouting::PROT_NUMBER)
    {
      return false;
    }
  // Remove the bhattackdsr routing header here
  Ptr<Packet> pktMinusBhattackdsrHdr = pktMinusIpHdr->Copy ();
  BhattackdsrRoutingHeader bhattackdsrRouting;
  pktMinusBhattackdsrHdr->RemoveHeader (bhattackdsrRouting);
...
    *さらに処理します。それ以外の場合は、無視します
  ...
  if (bhattackdsrRouting.GetMessageType () == 2 && ourAddress == m_mainAddress)
    {
      NS_LOG_DEBUG ("data packet receives " << packet->GetUid ());
      Ipv4Address sourceIp = GetIPfromID (bhattackdsrRouting.GetSourceId ());
      Ipv4Address destinationIp = GetIPfromID ( bhattackdsrRouting.GetDestId ());
      ...
      BhattackdsrMaintainBuffEntry newEntry;
      ...
      bhattackdsr->CancelLinkPacketTimer (newEntry);
    }
  ...
  if (packetType == NetDevice::PACKET_OTHERHOST)
    {
      ...
      uint8_t offset = bhattackdsrRouting.GetBhattackdsrOptionsOffset ();        // オプションヘッダーのオフセット（この場合は4バイト）を取得します
      uint8_t nextHeader = bhattackdsrRouting.GetNextHeader ();
      uint32_t sourceId = bhattackdsrRouting.GetSourceId ();
      ...
      if (optionType == 96)        // This is the source route option
        {
          ...
          bhattackdsrOption = GetOption (optionType);       // Get the relative DSR option and demux to the process function
         //オーバーヒアリング機能
          NS_LOG_DEBUG (Simulator::Now ().GetSeconds () <<
                        ...
                        " and packet : " << *pktMinusBhattackdsrHdr);
...
*/
          ...
          bhattackdsrOption->Process (pktMinusIpHdr, pktMinusBhattackdsrHdr, m_mainAddress, source, ipv4Header, nextHeader, isPromisc, promiscSource);
          return true;
...
void
BhattackdsrRouting::PacketNewRoute (Ptr<Packet> packet,
                            Ipv4Address source,
  ...
  if (!findRoute)
    {
      ...
      BhattackdsrSendBuffEntry newEntry (p, destination, m_sendBufferTimeout, protocol);     // 送信バッファの新しいエントリを作成します
...
void
BhattackdsrRouting::SendUnreachError (Ipv4Address unreachNode, Ipv4Address destination, Ipv4Address originalDst, uint8_t salvage, uint8_t protocol)
{
  ...
  BhattackdsrOptionRerrUnreachHeader rerrUnreachHeader;
  ...
  if (!findRoute)
    {
      if (destination == m_mainAddress)
      ...
      else
        {
          ...
          bhattackdsrRoutingHeader.SetPayloadLength (rerrLength + 2);
          bhattackdsrRoutingHeader.AddBhattackdsrOption (rerrUnreachHeader);
          newPacket->AddHeader (bhattackdsrRoutingHeader);
          Ptr<Packet> p = newPacket->Copy ();
          // Save the error packet in the error buffer
          BhattackdsrErrorBuffEntry newEntry (p, destination, m_mainAddress, unreachNode, m_sendBufferTimeout, protocol);
  ...
  else
    {
      ...
      std::map<uint32_t, Ptr<bhattackdsr::BhattackdsrNetworkQueue> >::iterator i = m_priorityQueue.find (priority);
      Ptr<bhattackdsr::BhattackdsrNetworkQueue> bhattackdsrNetworkQueue = i->second;
      NS_LOG_DEBUG ("Will be inserting into priority queue " << bhattackdsrNetworkQueue << " number: " << priority);
...
void
BhattackdsrRouting::ForwardErrPacket (BhattackdsrOptionRerrUnreachHeader &rerr,
                              BhattackdsrOptionSRHeader &sourceRoute,
                              Ipv4Address nextHop,
                              uint8_t protocol,
                              Ptr<Ipv4Route> route)
{
  ...
  NS_ASSERT_MSG (!m_downTarget.IsNull (), "Error, BhattackdsrRouting cannot send downward");
  BhattackdsrRoutingHeader bhattackdsrRoutingHeader;
  bhattackdsrRoutingHeader.SetNextHeader (protocol);
  bhattackdsrRoutingHeader.SetMessageType (1);
  bhattackdsrRoutingHeader.SetSourceId (GetIDfromIP (rerr.GetErrorSrc ()));
  bhattackdsrRoutingHeader.SetDestId (GetIDfromIP (rerr.GetErrorDst ()));

  uint8_t length = (sourceRoute.GetLength () + rerr.GetLength ());
  bhattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 4);
  bhattackdsrRoutingHeader.AddBhattackdsrOption (rerr);
  bhattackdsrRoutingHeader.AddBhattackdsrOption (sourceRoute);
  Ptr<Packet> packet = Create<Packet> ();
  packet->AddHeader (bhattackdsrRoutingHeader);
  Ptr<NetDevice> dev = m_ip->GetNetDevice (m_ip->GetInterfaceForAddress (m_mainAddress));
  ...
  /// \todo New BhattackdsrNetworkQueueEntry
  BhattackdsrNetworkQueueEntry newEntry (packet, m_mainAddress, nextHop, Simulator::Now (), route);
...
void
BhattackdsrRouting::Send (Ptr<Packet> packet,
                  Ipv4Address source,
                  Ipv4Address destination,
                  uint8_t protocol,
                  Ptr<Ipv4Route> route)
      ...
      if (!findRoute)
        {
          ...
          Ptr<Packet> p = packet->Copy ();
          BhattackdsrSendBuffEntry newEntry (p, destination, m_sendBufferTimeout, protocol);     // Create a new entry for send buffer
      ...
      else
        {
          ...
          // Put the data packet in the maintenance queue for data packet retransmission
          BhattackdsrMaintainBuffEntry newEntry (/*Packet=*/ mtP, /*ourAddress=*/ m_mainAddress, /*nextHop=*/ nextHop,
                                                  /*source=*/ source, /*destination=*/ destination, /*ackId=*/ 0,
                                                  /*SegsLeft=*/ nodeList.size () - 2, /*expire time=*/ m_maxMaintainTime);
          bool result = m_maintainBuffer.Enqueue (newEntry);       // Enqueue the packet the the maintenance buffer
          if (result)
          ...
          if (m_sendBuffer.GetSize () != 0 && m_sendBuffer.Find (destination))
            {
              ...
              Simulator::Schedule (MilliSeconds (m_uniformRandomVariable->GetInteger (0,100)),
                                   &BhattackdsrRouting::SendPacketFromBuffer, this, sourceRoute, nextHop, protocol);
            }
        }
    }
}
...
uint16_t
BhattackdsrRouting::AddAckReqHeader (Ptr<Packet>& packet, Ipv4Address nextHop)
{
  NS_LOG_FUNCTION (this << packet << nextHop);
  // This packet is used to peek option type
  Ptr<Packet> bhattackdsrP = packet->Copy ();
  Ptr<Packet> tmpP = packet->Copy ();

  BhattackdsrRoutingHeader bhattackdsrRoutingHeader;
  bhattackdsrP->RemoveHeader (bhattackdsrRoutingHeader);          // Remove the DSR header in whole
  uint8_t protocol = bhattackdsrRoutingHeader.GetNextHeader ();
  uint32_t sourceId = bhattackdsrRoutingHeader.GetSourceId ();
  uint32_t destinationId = bhattackdsrRoutingHeader.GetDestId ();
  uint32_t offset = bhattackdsrRoutingHeader.GetBhattackdsrOptionsOffset ();
...
//  outputfile2 <<" numberaddress "<< numberAddress <<  "\n" ;
  ...
  tmpP->RemoveHeader (sourceRoute);               // this is a clean packet without any bhattackdsr involved headers
  ...
  BhattackdsrRoutingHeader newBhattackdsrRoutingHeader;
  newBhattackdsrRoutingHeader.SetNextHeader (protocol);
  newBhattackdsrRoutingHeader.SetMessageType (2);
  newBhattackdsrRoutingHeader.SetSourceId (sourceId);
  newBhattackdsrRoutingHeader.SetDestId (destinationId);
  newBhattackdsrRoutingHeader.SetPayloadLength (length + 4);
  newBhattackdsrRoutingHeader.AddBhattackdsrOption (sourceRoute);
  newBhattackdsrRoutingHeader.AddBhattackdsrOption (ackReq);
  bhattackdsrP->AddHeader (newBhattackdsrRoutingHeader);
  // give the bhattackdsrP value to packet and then return
  packet = bhattackdsrP;
  return m_ackId;
}

void
BhattackdsrRouting::SendPacket (Ptr<Packet> packet, Ipv4Address source, Ipv4Address nextHop, uint8_t protocol)
{
...
//  printf("Sendpacketcounter = %d\n",Sendpacketcounter );
  ...
  /// \todo New BhattackdsrNetworkQueueEntry
  BhattackdsrNetworkQueueEntry newEntry (packet, source, nextHop, Simulator::Now (), m_ipv4Route);
...
void
BhattackdsrRouting::Scheduler (uint32_t priority)
{
  NS_LOG_FUNCTION (this);
...
void
BhattackdsrRouting::PriorityScheduler (uint32_t priority, bool continueWithFirst)
{
  ...
  for (uint32_t i = priority; numPriorities < m_numPriorityQueues; numPriorities++)
    {
      std::map<uint32_t, Ptr<BhattackdsrNetworkQueue> >::iterator q = m_priorityQueue.find (i);
      Ptr<bhattackdsr::BhattackdsrNetworkQueue> bhattackdsrNetworkQueue = q->second;
      uint32_t queueSize = bhattackdsrNetworkQueue->GetSize ();
      if (queueSize == 0)
        {
      ...
      else
        {
          ...
          for (std::map<uint32_t, Ptr<bhattackdsr::BhattackdsrNetworkQueue> >::iterator j = m_priorityQueue.begin (); j != m_priorityQueue.end (); j++)
            {
          ...
          BhattackdsrNetworkQueueEntry newEntry;
          bhattackdsrNetworkQueue->Dequeue (newEntry);
          if (SendRealDown (newEntry))
            {
              NS_LOG_LOGIC ("Packet sent by Bhattackdsr. Calling PriorityScheduler after some time");
              ...
              Simulator::Schedule (MicroSeconds (m_uniformRandomVariable->GetInteger (0, 1000)),
                                   &BhattackdsrRouting::PriorityScheduler,this, i, false);
            }
          else
            {
              // packet was dropped by Bhattackdsr. Call scheduler immediately so that we can
              // send another packet immediately.
              NS_LOG_LOGIC ("Packet dropped by Bhattackdsr. Calling PriorityScheduler immediately");
              Simulator::Schedule (Seconds (0), &BhattackdsrRouting::PriorityScheduler, this, i, false);
            }
...
void
BhattackdsrRouting::IncreaseRetransTimer ()
{
  NS_LOG_FUNCTION (this);
  ...
  std::vector<BhattackdsrNetworkQueueEntry> newNetworkQueue = bhattackdsrNetworkQueue->GetQueue ();
  for (std::vector<BhattackdsrNetworkQueueEntry>::iterator i = newNetworkQueue.begin (); i != newNetworkQueue.end (); i++)
    {
...
bool
BhattackdsrRouting::SendRealDown (BhattackdsrNetworkQueueEntry & newEntry)
{
  NS_LOG_FUNCTION (this);
...
void
BhattackdsrRouting::SendPacketFromBuffer (BhattackdsrOptionSRHeader const &sourceRoute, Ipv4Address nextHop, uint8_t protocol)
{
  ...
  if (m_sendBuffer.Find (destination))
    {
      ...
      */
      ...
      BhattackdsrSendBuffEntry entry;
      if (m_sendBuffer.Dequeue (destination, entry))
        {
          ...
          p->AddHeader (bhattackdsrRoutingHeader);
   ...
   */
    ...
    {
      BhattackdsrErrorBuffEntry entry;
              ...
              if (errorType == 1)
                {
                  ...
                  std::vector<Ipv4Address> nodeList = sourceRoute.GetNodesAddress ();
                  BhattackdsrRoutingHeader newRoutingHeader;
                  ...
                  newRoutingHeader.AddBhattackdsrOption (newUnreach);
                  newRoutingHeader.AddBhattackdsrOption (sourceRoute);
                  /// ルートを見つけて使用すると、リンクキャッシュへのUseExtends
                  if (m_routeCache->IsLinkCache ())
                    {
                      m_routeCache->UseExtends (nodeList);
...
bool
BhattackdsrRouting::PassiveEntryCheck (Ptr<Packet> packet, Ipv4Address source, Ipv4Address destination, uint8_t segsLeft,
                               uint16_t fragmentOffset, uint16_t identification, bool saveEntry)
{
  ...
  BhattackdsrPassiveBuffEntry newEntry;
  newEntry.SetPacket (p);
  ...
  if (m_passiveBuffer->AllEqual (newEntry) && (!saveEntry))
    {
      ...
      BhattackdsrMaintainBuffEntry mbEntry;
      mbEntry.SetPacket (p);
...
bool
BhattackdsrRouting::CancelPassiveTimer (Ptr<Packet> packet, Ipv4Address source, Ipv4Address destination,
                                uint8_t segsLeft)
{
  NS_LOG_FUNCTION (this << packet << source << destination << (uint32_t)segsLeft);
...
void
BhattackdsrRouting::CallCancelPacketTimer (uint16_t ackId, Ipv4Header const& ipv4Header, Ipv4Address realSrc, Ipv4Address realDst)
{
  ...
  BhattackdsrMaintainBuffEntry newEntry (/*Packet=*/ mainP, /*ourAddress=*/ sender, /*nextHop=*/ receiver,
...
void
BhattackdsrRouting::CancelPacketAllTimer (BhattackdsrMaintainBuffEntry & mb)
{
  NS_LOG_FUNCTION (this);
...
void
BhattackdsrRouting::CancelLinkPacketTimer (BhattackdsrMaintainBuffEntry & mb)
{
  NS_LOG_FUNCTION (this);
  LinkKey linkKey;
...
void
BhattackdsrRouting::CancelNetworkPacketTimer (BhattackdsrMaintainBuffEntry & mb)
{
  NS_LOG_FUNCTION (this);
  NetworkKey networkKey;
...
void
BhattackdsrRouting::CancelPassivePacketTimer (BhattackdsrMaintainBuffEntry & mb)
{
  NS_LOG_FUNCTION (this);
  PassiveKey passiveKey;
  passiveKey.m_ackId = 0;
...
void
BhattackdsrRouting::CancelPacketTimerNextHop (Ipv4Address nextHop, uint8_t protocol)
{
  NS_LOG_FUNCTION (this << nextHop << (uint32_t)protocol);

  BhattackdsrMaintainBuffEntry entry;
  ...
  if (m_maintainBuffer.Dequeue (nextHop, entry))
    {
      ...
      Ptr<Packet> bhattackdsrP = entry.GetPacket ()->Copy ();
      Ptr<Packet> p = bhattackdsrP->Copy ();
      Ptr<Packet> packet = bhattackdsrP->Copy ();
      BhattackdsrRoutingHeader bhattackdsrRoutingHeader;
      bhattackdsrP->RemoveHeader (bhattackdsrRoutingHeader);          // Remove the bhattackdsr header in whole
      uint32_t offset = bhattackdsrRoutingHeader.GetBhattackdsrOptionsOffset ();
      ...
      if (m_maintainBuffer.GetSize () && m_maintainBuffer.Find (nextHop))
        {
          ...
          Simulator::Schedule (MilliSeconds (m_uniformRandomVariable->GetInteger (0,100)),
                               &BhattackdsrRouting::CancelPacketTimerNextHop,this,nextHop,protocol);
        }
    }
  else
    {
...
void
BhattackdsrRouting::SalvagePacket (Ptr<const Packet> packet, Ipv4Address source, Ipv4Address dst, uint8_t protocol)
{
  ...
  BhattackdsrRoutingHeader bhattackdsrRoutingHeader;
  p->RemoveHeader (bhattackdsrRoutingHeader);
  // Remove offset of bhattackdsr routing header
  uint8_t offset = bhattackdsrRoutingHeader.GetBhattackdsrOptionsOffset ();
  ...
  if (findRoute && (salvage < m_maxSalvageCount))
    {
      ...
      newBhattackdsrRoutingHeader.SetNextHeader (protocol);
      newBhattackdsrRoutingHeader.SetMessageType (2);
      newBhattackdsrRoutingHeader.SetSourceId (GetIDfromIP (source));
      newBhattackdsrRoutingHeader.SetDestId (GetIDfromIP (dst));
      ...
      newBhattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 2);
      newBhattackdsrRoutingHeader.AddBhattackdsrOption (sourceRoute);
      p->AddHeader (newBhattackdsrRoutingHeader);
      ...
      /// \todo New BhattackdsrNetworkQueueEntry
      BhattackdsrNetworkQueueEntry newEntry (p, m_mainAddress, nextHop, Simulator::Now (), m_ipv4Route);
...
void
BhattackdsrRouting::ScheduleLinkPacketRetry (BhattackdsrMaintainBuffEntry & mb,
                                     uint8_t protocol)
{
  ...
  m_linkAckTimer[linkKey].SetFunction (&BhattackdsrRouting::LinkScheduleTimerExpire, this);
  m_linkAckTimer[linkKey].Remove ();
...
void
BhattackdsrRouting::SchedulePassivePacketRetry (BhattackdsrMaintainBuffEntry & mb,
                                        uint8_t protocol)
{
  NS_LOG_FUNCTION (this << (uint32_t)protocol);
  ...
  m_passiveAckTimer[passiveKey].SetFunction (&BhattackdsrRouting::PassiveScheduleTimerExpire, this);
  m_passiveAckTimer[passiveKey].Remove ();
...
void
BhattackdsrRouting::ScheduleNetworkPacketRetry (BhattackdsrMaintainBuffEntry & mb,
                                        bool isFirst,
                                        uint8_t protocol)
{
  Ptr<Packet> p = Create<Packet> ();
  Ptr<Packet> bhattackdsrP = Create<Packet> ();
  ...
  if (isFirst)
    {
      ...
      m_addressForwardTimer[networkKey].SetFunction (&BhattackdsrRouting::NetworkScheduleTimerExpire, this);
      m_addressForwardTimer[networkKey].Remove ();
  ...
  else
    {
      ...
      p = mb.GetPacket ()->Copy ();
      bhattackdsrP = mb.GetPacket ()->Copy ();
      ...
      NS_LOG_DEBUG ("The packet with bhattackdsr header " << bhattackdsrP->GetSize ());
      networkKey.m_ackId = mb.GetAckId ();
      networkKey.m_ourAdd = mb.GetOurAdd ();
      networkKey.m_nextHop = mb.GetNextHop ();
      networkKey.m_source = mb.GetSrc ();
...
void
BhattackdsrRouting::LinkScheduleTimerExpire  (BhattackdsrMaintainBuffEntry & mb,
                                      uint8_t protocol)
{
  NS_LOG_FUNCTION (this << (uint32_t)protocol);
  Ipv4Address nextHop = mb.GetNextHop ();
...
void
BhattackdsrRouting::PassiveScheduleTimerExpire  (BhattackdsrMaintainBuffEntry & mb,
                                         uint8_t protocol)
{
  NS_LOG_FUNCTION (this << (uint32_t)protocol);
  Ipv4Address nextHop = mb.GetNextHop ();
...
int64_t
BhattackdsrRouting::AssignStreams (int64_t stream)
{
...
void
BhattackdsrRouting::NetworkScheduleTimerExpire  (BhattackdsrMaintainBuffEntry & mb,
                                         uint8_t protocol)
{
  Ptr<Packet> p = mb.GetPacket ()->Copy ();
  Ipv4Address source = mb.GetSrc ();
...
void
BhattackdsrRouting::ForwardPacket (Ptr<const Packet> packet,
                           BhattackdsrOptionSRHeader &sourceRoute,
...
void
BhattackdsrRouting::SendInitialRequest (Ipv4Address source,
                                Ipv4Address destination,
                                uint8_t protocol)
{
  ...
  BhattackdsrOptionRreqHeader rreqHeader;                                  // has an alignment of 4n+0
  ...
  bhattackdsrRoutingHeader.AddBhattackdsrOption (rreqHeader);                      // Add the rreqHeader to the bhattackdsr extension header
...
void
BhattackdsrRouting::SendErrorRequest (BhattackdsrOptionRerrUnreachHeader &rerr, uint8_t protocol)
{
  NS_LOG_FUNCTION (this << (uint32_t)protocol);
  NS_ASSERT_MSG (!m_downTarget.IsNull (), "Error, BhattackdsrRouting cannot send downward");
  ...
  if (m_routeCache->LookupRoute (dst, toDst))
  ...
  else
    {
      ...
      bhattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 4);
      dstP->AddHeader (bhattackdsrRoutingHeader);
...
void
BhattackdsrRouting::CancelRreqTimer (Ipv4Address dst, bool isRemove)
{
...
void
BhattackdsrRouting::ScheduleRreqRetry (Ptr<Packet> packet, std::vector<Ipv4Address> address, bool nonProp, uint32_t requestId, uint8_t protocol)
{
  ...
  if (nonProp)
    {
      ...
      m_nonPropReqTimer[dst].SetFunction (&BhattackdsrRouting::RouteRequestTimerExpire, this);
      m_nonPropReqTimer[dst].Remove ();
  ...
  else
    {
      ...
      address.push_back (source);
      address.push_back (dst);
      m_addressReqTimer[dst].SetFunction (&BhattackdsrRouting::RouteRequestTimerExpire, this);
      m_addressReqTimer[dst].Remove ();
...
void
BhattackdsrRouting::RouteRequestTimerExpire (Ptr<Packet> packet, std::vector<Ipv4Address> address, uint32_t requestId, uint8_t protocol)
{
  NS_LOG_FUNCTION (this << packet << requestId << (uint32_t)protocol);
  // Get a clean packet without bhattackdsr header
  Ptr<Packet> bhattackdsrP = packet->Copy ();
  BhattackdsrRoutingHeader bhattackdsrRoutingHeader;
  bhattackdsrP->RemoveHeader (bhattackdsrRoutingHeader);          // Remove the bhattackdsr header in whole
  ...
  if (m_routeCache->LookupRoute (dst, toDst))
    {
      ...
      if (nextHop == "0.0.0.0")
        {
          NS_LOG_DEBUG ("Error next hop address");
          PacketNewRoute (bhattackdsrP, source, dst, protocol);
          return;
        }
      SetRoute (nextHop, m_mainAddress);
      CancelRreqTimer (dst, true);
...
void
BhattackdsrRouting::SendRequest (Ptr<Packet> packet,
                         Ipv4Address source)
{
  ...
  /// \todo New BhattackdsrNetworkQueueEntry
  BhattackdsrNetworkQueueEntry newEntry (packet, source, m_broadcast, Simulator::Now (), 0);
  if (bhattackdsrNetworkQueue->Enqueue (newEntry))
    {
      Scheduler (priority);
    }
...
void
BhattackdsrRouting::ScheduleInterRequest (Ptr<Packet> packet)
{
   ...
   */
  Simulator::Schedule (MilliSeconds (m_uniformRandomVariable->GetInteger (0, m_broadcastJitter)), &BhattackdsrRouting::SendRequest, this,
                       packet, m_mainAddress);
}

void
BhattackdsrRouting::SendGratuitousReply (Ipv4Address source, Ipv4Address srcAddress, std::vector<Ipv4Address> &nodeList, uint8_t protocol)
{
  ...
  if (!(m_graReply.FindAndUpdate (source, srcAddress, m_gratReplyHoldoff)))     // Find the gratuitous reply entry
    {
      ...
      bhattackdsrRoutingHeader.SetNextHeader (protocol);
      bhattackdsrRoutingHeader.SetMessageType (1);
      bhattackdsrRoutingHeader.SetSourceId (GetIDfromIP (replySrc));
...
void
BhattackdsrRouting::SendReply (Ptr<Packet> packet,
                       Ipv4Address source,
                       Ipv4Address nextHop,
                       Ptr<Ipv4Route> route)
{
  ...
  /// \todo New BhattackdsrNetworkQueueEntry
  BhattackdsrNetworkQueueEntry newEntry (packet, source, nextHop, Simulator::Now (), route);
  if (bhattackdsrNetworkQueue->Enqueue (newEntry))
    {
      Scheduler (priority);
    }
...
void
BhattackdsrRouting::ScheduleInitialReply (Ptr<Packet> packet,
                                  Ipv4Address source,
                                  Ipv4Address nextHop,
                                  Ptr<Ipv4Route> route)
{
  NS_LOG_FUNCTION (this << packet << source << nextHop);
  Simulator::ScheduleNow (&BhattackdsrRouting::SendReply, this,
                          packet, source, nextHop, route);
}

void
BhattackdsrRouting::ScheduleCachedReply (Ptr<Packet> packet,
                                 Ipv4Address source,
                                 Ipv4Address destination,
                                 Ptr<Ipv4Route> route,
                                 double hops)
{
  ...
  Simulator::Schedule (Time (2 * m_nodeTraversalTime * (hops - 1 + m_uniformRandomVariable->GetValue (0,1))), &BhattackdsrRouting::SendReply, this, packet, source, destination, route);
}

void
BhattackdsrRouting::SendAck   (uint16_t ackId,
                       Ipv4Address destination,
                       Ipv4Address realSrc,
                       Ipv4Address realDst,
                       uint8_t protocol,
                       Ptr<Ipv4Route> route)
{
  ...
  bhattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 2);
  bhattackdsrRoutingHeader.AddBhattackdsrOption (ack);
  ...
  NS_LOG_LOGIC ("Will be inserting into priority queue " << bhattackdsrNetworkQueue << " number: " << priority);

  //m_downTarget (packet, m_mainAddress, destination, GetProtocolNumber (), route);

  /// \todo New BhattackdsrNetworkQueueEntry
  BhattackdsrNetworkQueueEntry newEntry (packet, m_mainAddress, destination, Simulator::Now (), route);
  if (bhattackdsrNetworkQueue->Enqueue (newEntry))
    {
      Scheduler (priority);
    }
...
BhattackdsrRouting::Receive (Ptr<Packet> p,
                     Ipv4Header const &ip,
                     Ptr<Ipv4Interface> incomingInterface)
{
   ...
   */
  BhattackdsrRoutingHeader bhattackdsrRoutingHeader;
  packet->RemoveHeader (bhattackdsrRoutingHeader);          // Remove the DSR header in whole
  ...
  uint32_t offset = bhattackdsrRoutingHeader.GetBhattackdsrOptionsOffset ();        // Get the offset for option header, 8 bytes in this case
  ...
  bhattackdsrOption = GetOption (optionType);       // Get the relative bhattackdsr option and demux to the process function
  ...
  if (optionType == 1)        // This is the request option
    {
      ...
      bhattackdsrOption = GetOption (optionType);
      optionLength = bhattackdsrOption->Process (p, packet, m_mainAddress, source, ip, protocol, isPromisc, promiscSource);
      ...
      if (optionLength == 0)
      ...
      else
        {
          if (segmentsLeft == 0)
            {
              // / Get the next header
              uint8_t nextHeader = bhattackdsrRoutingHeader.GetNextHeader ();
  ...
  else
    {
      ...
      BhattackdsrOptionRerrUnsupportHeader rerrUnsupportHeader;
...
void
BhattackdsrRouting::SetDownTarget (DownTargetCallback callback)
{
  m_downTarget = callback;
}

void
BhattackdsrRouting::SetDownTarget6 (DownTargetCallback6 callback)
{
...
BhattackdsrRouting::GetDownTarget (void) const
{
  return m_downTarget;
}

IpL4Protocol::DownTargetCallback6
BhattackdsrRouting::GetDownTarget6 (void) const
{
  NS_FATAL_ERROR ("Unimplemented");
  return MakeNullCallback<void,Ptr<Packet>, Ipv6Address, Ipv6Address, uint8_t, Ptr<Ipv6Route> > ();
}

void BhattackdsrRouting::Insert (Ptr<bhattackdsr::BhattackdsrOptions> option)
{
  m_options.push_back (option);
}

Ptr<bhattackdsr::BhattackdsrOptions> BhattackdsrRouting::GetOption (int optionNumber)
{
  for (BhattackdsrOptionList_t::iterator i = m_options.begin (); i != m_options.end (); ++i)
    {

...
/**
 * \class BhattackdsrLinkStab
 * \brief BhattackdsrLinkStab class (DSR link stability)
 */
class BhattackdsrLinkStab
{
public:
   ...
   */
  BhattackdsrLinkStab (Time linkStab = Simulator::Now ());
  /**
   * \brief Destructor
   */
  virtual ~BhattackdsrLinkStab ();
...
/**
 * \class BhattackdsrNodeStab
 * \brief BhattackdsrNodeStab class (DSR node stability)
 */
class BhattackdsrNodeStab
{
public:
   ...
   */
  BhattackdsrNodeStab (Time nodeStab = Simulator::Now ());
  virtual ~BhattackdsrNodeStab ();
...
/**
 * \class BhattackdsrRouteCacheEntry
 * \brief BhattackdsrRouteCacheEntry class for entries in the route cache
 */
class BhattackdsrRouteCacheEntry
{
public:
   ...
   */
  BhattackdsrRouteCacheEntry (IP_VECTOR const  & ip = IP_VECTOR (), Ipv4Address dst = Ipv4Address (), Time exp = Simulator::Now ());
  virtual ~BhattackdsrRouteCacheEntry ();
   ...
   */
  bool operator== (BhattackdsrRouteCacheEntry const & o) const
  {
 ...
 */
class BhattackdsrRouteCache : public Object
{
public:
  /**
  ...
  BhattackdsrRouteCache ();
  virtual ~BhattackdsrRouteCache ();

  /**
   * \brief Remove the aged route cache entries when the route cache is full
   * \param rtVector the route cache to scan.
   */
  void RemoveLastEntry (std::list<BhattackdsrRouteCacheEntry> & rtVector);
  /**
   * \brief Define the vector of route entries.
   */
  typedef std::list<BhattackdsrRouteCacheEntry::IP_VECTOR> routeVector;
   ...
   */
  void PrintRouteVector (std::list<BhattackdsrRouteCacheEntry> route);
  /**
   ...
   * \return true if same
   */
  bool FindSameRoute (BhattackdsrRouteCacheEntry & rt, std::list<BhattackdsrRouteCacheEntry> & rtVector);
  /**
...
private:
   ...
   * \return
   */
  BhattackdsrRouteCache & operator= (BhattackdsrRouteCache const &);
  BhattackdsrRouteCacheEntry::IP_VECTOR m_vector;               ///< The route vector to save the ip addresses for intermediate nodes.
  uint32_t m_maxCacheLen;                               ///< The maximum number of packets that we allow a routing protocol to buffer.
  Time     RouteCacheTimeout;                           ///< The maximum period of time that bhattackdsr is allowed to for an unused route.
   ...
   */
  typedef std::list<BhattackdsrRouteCacheEntry> routeEntryVector;
  ...
  std::map<Ipv4Address, BhattackdsrRouteCacheEntry::IP_VECTOR> m_bestRoutesTable_link;     ///< for link route cache
  std::map<Link, BhattackdsrLinkStab> m_linkCache;                                         ///< The data structure to store link info
  std::map<Ipv4Address, BhattackdsrNodeStab> m_nodeCache;                                  ///< The data structure to store node info
  /**
   ...
   */
  bool LookupRoute_Link (Ipv4Address id, BhattackdsrRouteCacheEntry & rt);
  /**
...
public:
   ...
   * \return true if the link is cached
   */
  bool AddRoute_Link (BhattackdsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address node);
  /**

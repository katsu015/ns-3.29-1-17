 ...
 */
class BhattackdsrPassiveBuffEntry
{
public:
  /**
   * Construct a BhattackdsrPassiveBuffEntry with the given parameters
   *
   * \param pa packet
   ...
   */
  BhattackdsrPassiveBuffEntry (Ptr<const Packet> pa = 0, Ipv4Address d = Ipv4Address (), Ipv4Address s = Ipv4Address (),
   ...
   */
  bool operator== (BhattackdsrPassiveBuffEntry const & o) const
  {
...
/**
 * \ingroup bhattackdsr
 * \class BhattackdsrPassiveBuffer
 * \brief DSR passive buffer
 */
/************************************************************************************************************************/
class BhattackdsrPassiveBuffer  : public Object
{
public:
  /**
  ...
  BhattackdsrPassiveBuffer ();
  virtual ~BhattackdsrPassiveBuffer ();

  /// Push entry in queue, if there is no entry with the same packet and destination address in queue.
  /// \param entry Buffer Entry
  /// \return true on success adding the Entry.
  bool Enqueue (BhattackdsrPassiveBuffEntry & entry);
  /// Return first found (the earliest) entry for given destination
  /// \param [in] dst Entry destination
  /// \param [out] entry The Entry found (if any).
  /// \return true on success
  bool Dequeue (Ipv4Address dst, BhattackdsrPassiveBuffEntry & entry);
 ...
 /// \return true if an Entry was found and removed.
  bool AllEqual (BhattackdsrPassiveBuffEntry & entry);
  /// Number of entries
...
private:
  ...
  std::vector<BhattackdsrPassiveBuffEntry> m_passiveBuffer;
  /// Remove all expired entries
  void Purge ();
  /// Notify that packet is dropped from queue by timeout
  /// \param en BuffEntry Buffer entry
  /// \param reason Drop reason
  void Drop (BhattackdsrPassiveBuffEntry en, std::string reason);
  /// Notify that packet is dropped from queue by timeout
  /// \param en BuffEntry Buffer entry
  /// \param reason Drop reason
  void DropLink (BhattackdsrPassiveBuffEntry en, std::string reason);
  ...
  static bool LinkEqual (BhattackdsrPassiveBuffEntry en, const std::vector<Ipv4Address> link)
  {

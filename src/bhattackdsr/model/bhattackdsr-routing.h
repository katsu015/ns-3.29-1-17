...
#include "bhattackdsr-maintain-buff.h"
#include "bhattackdsr-passive-buff.h"
#include "bhattackdsr-option-header.h"
#include "bhattackdsr-fs-header.h"
#include "bhattackdsr-rsendbuff.h"
#include "bhattackdsr-errorbuff.h"
#include "bhattackdsr-gratuitous-reply-table.h"
...
class BhattackdsrOptions;
/**
 * \class BhattackdsrRouting
 * \brief Bhattackdsr Routing base
 */
class BhattackdsrRouting : public IpL4Protocol
{
public:
  /**
  ...
  /**
    * \brief Define the bhattackdsr protocol number.
    */
   ...
   */
  void SetRouteCache (Ptr<bhattackdsr::BhattackdsrRouteCache> r);
  /**
   * \brief Get the route cache.
   * \return the route cache
   */
  Ptr<bhattackdsr::BhattackdsrRouteCache> GetRouteCache () const;
  /**
   * \brief Set the node.
   * \param r the request table to set
   */
  void SetRequestTable (Ptr<bhattackdsr::BhattackdsrRreqTable> r);
  /**
    * \brief Get the request table.
    * \return the request table
    */
  Ptr<bhattackdsr::BhattackdsrRreqTable> GetRequestTable () const;
  /**
   * \brief Set the node.
   * \param r the passive buffer to set
   */
  void SetPassiveBuffer (Ptr<bhattackdsr::BhattackdsrPassiveBuffer> r);
  /**
    * \brief Get the passive buffer
    * \return the passive buffer
    */
  Ptr<bhattackdsr::BhattackdsrPassiveBuffer> GetPassiveBuffer () const;

  /// functions used to direct to route cache
  //\{
  bool IsLinkCache ();
  void UseExtends (BhattackdsrRouteCacheEntry::IP_VECTOR rt);
  bool LookupRoute (Ipv4Address id, BhattackdsrRouteCacheEntry & rt);
  bool AddRoute_Link (BhattackdsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address source);
  bool AddRoute (BhattackdsrRouteCacheEntry & rt);
  ...
  /**
    * \brief Get the bhattackdsr protocol number.
   ...
   */
  uint32_t GetPriority (BhattackdsrMessageType messageType);
  /**
   ...
   */
  void ForwardErrPacket (BhattackdsrOptionRerrUnreachHeader &rerr,
   ...
   */
  bool SendRealDown (BhattackdsrNetworkQueueEntry & newEntry);
  /**
   ...
   * \param protocol number
   */
  void SendPacketFromBuffer (BhattackdsrOptionSRHeader const &sourceRoute,
                             Ipv4Address nextHop,
                             uint8_t protocol);
  /**
  ...
  */
  void CancelPacketAllTimer (BhattackdsrMaintainBuffEntry & mb);
  /**
   ...
   */
  void CancelNetworkPacketTimer (BhattackdsrMaintainBuffEntry & mb);
  /**
   * \brief Cancel the passive packet retransmission timer for a specific maintenance entry
   * \param mb maintian byffer entry
   */
  void CancelPassivePacketTimer (BhattackdsrMaintainBuffEntry & mb);
  /**
   * \brief Cancel the link packet retransmission timer for a specific maintenance entry
   * \param mb maintian byffer entry
   */
  void CancelLinkPacketTimer (BhattackdsrMaintainBuffEntry & mb);
  /**
   ...
   */
  void ScheduleLinkPacketRetry   (BhattackdsrMaintainBuffEntry & mb,
                                  uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void SchedulePassivePacketRetry   (BhattackdsrMaintainBuffEntry & mb,
                                     uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void ScheduleNetworkPacketRetry   (BhattackdsrMaintainBuffEntry & mb,
                                     bool isFirst,
                                     uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void LinkScheduleTimerExpire  (BhattackdsrMaintainBuffEntry & mb,
                                 uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void NetworkScheduleTimerExpire  (BhattackdsrMaintainBuffEntry & mb,
                                    uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void PassiveScheduleTimerExpire  (BhattackdsrMaintainBuffEntry & mb,
                                    uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void SendErrorRequest (BhattackdsrOptionRerrUnreachHeader &rerr, uint8_t protocol);
  /**
  ...
  /**
   * \brief Insert a new Bhattackdsr Option.
   * \param option the option to insert
   */
  void Insert (Ptr<bhattackdsr::BhattackdsrOptions> option);
  /**
   * \brief Get the option corresponding to optionNumber.
   * \param optionNumber the option number of the option to retrieve
   * \return a matching Bhattackdsr option
   */
  Ptr<bhattackdsr::BhattackdsrOptions> GetOption (int optionNumber);
  /**
 ...
 *          */
  ...
  TracedCallback <const BhattackdsrOptionSRHeader &> m_txPacketTrace; ///< packet trace callback
...
private:
   ...
   * \param protocol The protocol we receive, need to verify it is bhattackdsr protocol
   * \param from The from address we received the packet
   * \param to The address this packet is destined for
   * \param packetType The bhattackdsr packet type, 0 is for control packet, 1 for data packet
   ...
   */
  typedef std::list<Ptr<BhattackdsrOptions> > BhattackdsrOptionList_t;
  /**
   * \brief List of DSR Options supported.
   */
  BhattackdsrOptionList_t m_options;
  ...
  BhattackdsrSendBuffer m_sendBuffer;                           ///< The send buffer

  BhattackdsrErrorBuffer m_errorBuffer;                         ///< The error buffer to save the error messages
  ...
  BhattackdsrMaintainBuffer m_maintainBuffer;                   ///< The declaration of maintain buffer
  ...
  Ptr<bhattackdsr::BhattackdsrRouteCache> m_routeCache;                 ///< A "drop-front" queue used by the routing layer to cache routes found.

  Ptr<bhattackdsr::BhattackdsrRreqTable> m_rreqTable;                   ///< A "drop-front" queue used by the routing layer to cache route request sent.

  Ptr<bhattackdsr::BhattackdsrPassiveBuffer> m_passiveBuffer;           ///< A "drop-front" queue used by the routing layer to cache route request sent.
  ...
  std::map<uint32_t, Ptr<bhattackdsr::BhattackdsrNetworkQueue> > m_priorityQueue;   ///< priority queues

  BhattackdsrGraReply m_graReply;                               ///< The gratuitous route reply.

  BhattackdsrNetworkQueue m_networkQueue;                       ///< The network queue.

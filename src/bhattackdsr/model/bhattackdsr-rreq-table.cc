...
NS_LOG_COMPONENT_DEFINE ("BhattackdsrRreqTable");
  
namespace bhattackdsr {

NS_OBJECT_ENSURE_REGISTERED (BhattackdsrRreqTable);

TypeId BhattackdsrRreqTable::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::bhattackdsr::BhattackdsrRreqTable")
    .SetParent<Object> ()
    .SetGroupName ("Bhattackdsr")
    .AddConstructor<BhattackdsrRreqTable> ()
  ;
  return tid;
}

BhattackdsrRreqTable::BhattackdsrRreqTable ()
...
void
BhattackdsrRreqTable::RemoveLeastExpire ()
{
...
void
BhattackdsrRreqTable::FindAndUpdate (Ipv4Address dst)
{
...
void
BhattackdsrRreqTable::RemoveRreqEntry (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
  std::map<Ipv4Address, RreqTableEntry >::const_iterator i =
    m_rreqDstMap.find (dst);
...
uint32_t
BhattackdsrRreqTable::GetRreqCnt (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
  std::map<Ipv4Address, RreqTableEntry >::const_iterator i =
    m_rreqDstMap.find (dst);
 ...
 */
uint32_t
BhattackdsrRreqTable::CheckUniqueRreqId (Ipv4Address dst)
{
...
uint32_t
BhattackdsrRreqTable::GetRreqSize ()
{
...
void
BhattackdsrRreqTable::Invalidate ()
{
...
BlackList*
BhattackdsrRreqTable::FindUnidirectional (Ipv4Address neighbor)
{
...
bool
BhattackdsrRreqTable::MarkLinkAsUnidirectional (Ipv4Address neighbor, Time blacklistTimeout)
{
...
void
BhattackdsrRreqTable::PurgeNeighbor ()
{
  /*
...
bool
BhattackdsrRreqTable::FindSourceEntry (Ipv4Address src, Ipv4Address dst, uint16_t id)
{
  NS_LOG_FUNCTION (this << src << dst << id);
  BhattackdsrReceivedRreqEntry rreqEntry;
  rreqEntry.SetDestination (dst);
  rreqEntry.SetIdentification (id);
  std::list<BhattackdsrReceivedRreqEntry> receivedRreqEntryList;
  /*
   * this function will return false if the entry is not found, true if duplicate entry find
   */
  std::map<Ipv4Address, std::list<BhattackdsrReceivedRreqEntry> >::const_iterator i = m_sourceRreqMap.find (src);
  if (i == m_sourceRreqMap.end ())
  ...
  else
    {
      ...
      for (std::list<BhattackdsrReceivedRreqEntry>::const_iterator j = receivedRreqEntryList.begin (); j != receivedRreqEntryList.end (); ++j)
        {

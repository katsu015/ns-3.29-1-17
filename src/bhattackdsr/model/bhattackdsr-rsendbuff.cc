...
NS_LOG_COMPONENT_DEFINE ("BhattackdsrSendBuffer");

namespace bhattackdsr {

uint32_t
BhattackdsrSendBuffer::GetSize ()
{
  Purge ();
...
bool
BhattackdsrSendBuffer::Enqueue (BhattackdsrSendBuffEntry & entry)
{
  Purge ();
  for (std::vector<BhattackdsrSendBuffEntry>::const_iterator i = m_sendBuffer.begin (); i
       != m_sendBuffer.end (); ++i)
    {
...
void
BhattackdsrSendBuffer::DropPacketWithDst (Ipv4Address dst)
{
   ...
   */
  for (std::vector<BhattackdsrSendBuffEntry>::iterator i = m_sendBuffer.begin (); i
       != m_sendBuffer.end (); ++i)
    {
      if (IsEqual (*i, dst))
        {
  ...
  m_sendBuffer.erase (std::remove_if (m_sendBuffer.begin (), m_sendBuffer.end (),
                                      std::bind2nd (std::ptr_fun (BhattackdsrSendBuffer::IsEqual), dst)), m_sendBuffer.end ());
}

bool
BhattackdsrSendBuffer::Dequeue (Ipv4Address dst, BhattackdsrSendBuffEntry & entry)
{
  Purge ();
  /*
   * Dequeue the entry with destination address dst
   */
  for (std::vector<BhattackdsrSendBuffEntry>::iterator i = m_sendBuffer.begin (); i != m_sendBuffer.end (); ++i)
    {
...
bool
BhattackdsrSendBuffer::Find (Ipv4Address dst)
{
  /*
...
struct IsExpired
{
   ...
   */
  bool
  operator() (BhattackdsrSendBuffEntry const & e) const
  {
...
void
BhattackdsrSendBuffer::Drop (BhattackdsrSendBuffEntry en, std::string reason)
{

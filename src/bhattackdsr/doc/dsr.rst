...
a packet buffer in bhattackdsr-rsendbuff.cc. The packet queue implements 
...
* The BhattackdsrFsHeader has added 3 fields: message type, source id, destination id, and these changes only for post-processing
...
* DSR works as a shim header between transport and network protocol, it needs its own forwarding mechanism, we are changing the packet transmission to hop-by-hop delivery, so we added two fields in bhattackdsr fixed header to notify packet delivery
...
To have a node run DSR, the easiest way would be to use the BhattackdsrHelper
and BhattackdsrMainHelpers in your simulation script. For instance:

.. sourcecode:: cpp

  BhattackdsrHelper bhattackdsr;
  BhattackdsrMainHelper bhattackdsrMain;
  bhattackdsrMain.Install (bhattackdsr, adhocNodes);

The example scripts inside ``src/bhattackdsr/examples/`` demonstrate the use of DSR based nodes in different scenarios. 
The helper source can be found inside ``src/bhattackdsr/helper/bhattackdsr-main-helper.{h,cc}`` 
and ``src/bhattackdsr/helper/bhattackdsr-helper.{h,cc}``

Examples
********

The example can be found in ``src/bhattackdsr/examples/``:

* bhattackdsr.cc use DSR as routing protocol within a traditional MANETs environment[3].
...
* Unit tests have been written to verify the internals of DSR. This can be found in ``src/bhattackdsr/test/bhattackdsr-test-suite.cc``. These tests verify whether the methods inside DSR module which deal with packet buffer, headers work correctly.
...
The model is not fully compliant with :rfc:`4728`. As an example, Bhattackdsr fixed size header
...
[1] Original paper: http://www.monarch.cs.rice.edu/monarch-papers/bhattackdsr-chapter00.pdf

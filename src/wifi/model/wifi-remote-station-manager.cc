...
/*
Ipv4Address
WifiRemoteStationManager::GetIPfromMAC (Mac48Address address)
{
...
{
    //std::cout << "/* ReportRtsOk */" << '\n';
  //  std::cout << "RTSok address" << address << '\n';
                                        ...
                                        uint32_t packetSize)
{
  ...
  if (longMpdu)
    ...
    /*
    for (u_int32_t i = 0; i < nodecount; i++) {
      std::cout << i << '\n';
    }

    if ("00:00:00:00:00:12" == address) {
      CTScount++;
      std::cout <<"id:17 " << CTScount << '\n';
    }
    if ("00:00:00:00:00:11" == address) {
      CTScount2++;
      std::cout <<"id:17 "<< CTScount2 << '\n';
    }
    if ("00:00:00:00:00:10" == address) {
      CTScount3++;
      std::cout <<"id:16 " << CTScount << '\n';
    }
    if ("00:00:00:00:00:0f" == address) {
      CTScount4++;
      std::cout <<"id:15 "<< CTScount2 << '\n';
    }
    if ("00:00:00:00:00:0e" == address) {
      CTScount5++;
      std::cout <<"id:14 " << CTScount << '\n';
    }
    if ("00:00:00:00:00:0d" == address) {
      CTScount6++;
      std::cout <<"id:13 "<< CTScount2 << '\n';
    }
    */
...
/*
  if ("00:00:00:00:00:12" == address) {
    std::cout << "countRTS:id:18" << '\n';
    RTScount++;
    std::cout << RTScount << '\n';
  }
  if ("00:00:00:00:00:11" == address) {
    std::cout << "countRTS:id:16" << '\n';
    RTScount++;
    std::cout << RTScount << '\n';
  }
  if ("00:00:00:00:00:10" == address) {
  ...
  if ("00:00:00:00:00:0e" == address) {
    std::cout << "countRTS:id:14" << '\n';
    RTScount++;
    std::cout << RTScount << '\n';
  }
  if ("00:00:00:00:00:0d" == address) {
    std::cout << "countRTS:id:13" << '\n';
    RTScount++;
    std::cout << RTScount << '\n';
  }
*/
//  std::cout << "NeedRTS" << '\n';
//  std::cout << "RTS アドレス" << address << '\n';
...
WifiRemoteStationManager::NeedCtsToSelf (WifiTxVector txVector)
{
  ...
  if (m_erpProtectionMode == CTS_TO_SELF
    ...
    {
      //BSS Basic Rateセットを検索します。使用モードがBasicセットにある場合、Cts To Selfは不要です。
      ...
      if (HasHtSupported ())
        {
          //BSS Basic MCSセットを検索します。使用モードが基本セットにある場合、Cts To Selfは不要です。
...
WifiRemoteStationManager::DoSetFragmentationThreshold (uint32_t threshold)
{
  ...
  if (threshold < 256)
    ...
    {
      /*
       * 各フラグメントの長さは、オクテットの偶数でなければなりません。ただし、MSDUまたは
        * MMPDU。偶数または奇数のオクテットのいずれかです。
       */
      if (threshold % 2 != 0)
        {
...
{
  /**
   * 標準には、選択するための比較的明確なルールがあります。
    *制御応答率（以下はIEEE 802.11-2012から引用されています。
    *セクション9.7）：
    *
    *送信STAの内容を計算できるようにするため
    *持続時間/ IDフィールド、受信フレームに応答するSTA
    * Control Responseフレーム（CTSまたはACK）、その他
    * BlockAck制御フレームよりも、
    *以下のBSSBasicRateSetパラメーター
    *フレーム交換の直前のフレームのレート
    *シーケンス（Annex Gで定義）および同じ
    *受信フレームとしての変調クラス（セクション9.7.8を参照）...
   */
  ...
  for (WifiModeListIterator i = m_bssBasicRateSet.begin (); i != m_bssBasicRateSet.end (); i++)
    {
      if ((!found || i->IsHigherDataRate (mode))
        ...
        {
          mode = *i;
          //潜在的に適切な送信レートが見つかりましたが、
           //継続してすべての基本料金を考慮する必要があります
           //私たちは正しいものを持っていると確信できます。
          found = true;
        }
    }
  if (HasHtSupported () || HasVhtSupported () || HasHeSupported ())
  ...
  //BSSBasicRateSetで適切なレートが見つかった場合、
   //完了し、そのモードを返すことができます。
  if (found)
    {
  ...
  /**
   * 適切な基本レートが見つからなかった場合、必須を検索します
    *料金。 標準（IEEE 802.11-2007、セクション9.6）は次のように述べています。
    *
    * ... BSSBasicRateSetパラメーターに含まれるレートが満たされない場合
    *これらの条件、その後に応答して送信される制御フレーム
    *受信したフレームは、最も高い必須で送信されます
    *のレート以下のPHYのレート
    *受信したフレーム、およびそれと同じ変調クラスのもの
    *受信したフレーム。 さらに、制御応答フレームは
    *受信したフレームと同じPHYオプションを使用して送信される、
    *を使用する要件と矛盾しない限り
    * BSSBasicRateSetパラメーター。
    *
    * \ todo最後の文は今のところ無視していることに注意してください。
    * PHYオプションの操作はまだありません。
   */
  for (uint8_t idx = 0; idx < m_wifiPhy->GetNModes (); idx++)
    {
      ...
      if (thismode.IsMandatory ()
        ...
        {
          mode = thismode;
          //上記のように; 潜在的に適切な送信が見つかりました
           //評価しますが、継続してすべてのことを考慮する必要があります
           //正しいレートを取得する前に必須レート。
          found = true;
        }
    }
  ...
  /**
   * 応答に適したレートがまだ見つからない場合は、
    *誰かがシミュレーション設定を台無しにしました。 これはおそらく
    * WifiPhyStandardが正しく設定されていない、またはそのレート
    *は、PHYでサポートされていません。
    * WifiRemoteStationManager（または子孫）設定。
    *
    *いずれにせよ、それは深刻です-標準に違反するか、
    *失敗し、後者を選択しました...
   */
  if (!found)
    {

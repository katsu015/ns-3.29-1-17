...
#include "sdattackdsr-maintain-buff.h"
#include "sdattackdsr-passive-buff.h"
#include "sdattackdsr-option-header.h"
#include "sdattackdsr-fs-header.h"
#include "sdattackdsr-rsendbuff.h"
#include "sdattackdsr-errorbuff.h"
#include "sdattackdsr-gratuitous-reply-table.h"
...
class SdattackdsrOptions;
/**
 * \class SdattackdsrRouting
 * \brief Sdattackdsr Routing base
 */
class SdattackdsrRouting : public IpL4Protocol
{
public:
  /**
  ...
  /**
    * \brief Define the sdattackdsr protocol number.
    */
   ...
   */
  void SetRouteCache (Ptr<sdattackdsr::SdattackdsrRouteCache> r);
  /**
   * \brief Get the route cache.
   * \return the route cache
   */
  Ptr<sdattackdsr::SdattackdsrRouteCache> GetRouteCache () const;
  /**
   * \brief Set the node.
   * \param r the request table to set
   */
  void SetRequestTable (Ptr<sdattackdsr::SdattackdsrRreqTable> r);
  /**
    * \brief Get the request table.
    * \return the request table
    */
  Ptr<sdattackdsr::SdattackdsrRreqTable> GetRequestTable () const;
  /**
   * \brief Set the node.
   * \param r the passive buffer to set
   */
  void SetPassiveBuffer (Ptr<sdattackdsr::SdattackdsrPassiveBuffer> r);
  /**
    * \brief Get the passive buffer
    * \return the passive buffer
    */
  Ptr<sdattackdsr::SdattackdsrPassiveBuffer> GetPassiveBuffer () const;

  /// functions used to direct to route cache
  //\{
  bool IsLinkCache ();
  void UseExtends (SdattackdsrRouteCacheEntry::IP_VECTOR rt);
  bool LookupRoute (Ipv4Address id, SdattackdsrRouteCacheEntry & rt);
  bool AddRoute_Link (SdattackdsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address source);
  bool AddRoute (SdattackdsrRouteCacheEntry & rt);
  ...
  /**
    * \brief Get the sdattackdsr protocol number.
   ...
   */
  uint32_t GetPriority (SdattackdsrMessageType messageType);
  /**
   ...
   */
  void ForwardErrPacket (SdattackdsrOptionRerrUnreachHeader &rerr,
   ...
   */
  bool SendRealDown (SdattackdsrNetworkQueueEntry & newEntry);
  /**
   ...
   * \param protocol number
   */
  void SendPacketFromBuffer (SdattackdsrOptionSRHeader const &sourceRoute,
                             Ipv4Address nextHop,
                             uint8_t protocol);
  /**
  ...
  */
  void CancelPacketAllTimer (SdattackdsrMaintainBuffEntry & mb);
  /**
   ...
   */
  void CancelNetworkPacketTimer (SdattackdsrMaintainBuffEntry & mb);
  /**
   * \brief Cancel the passive packet retransmission timer for a specific maintenance entry
   * \param mb maintian byffer entry
   */
  void CancelPassivePacketTimer (SdattackdsrMaintainBuffEntry & mb);
  /**
   * \brief Cancel the link packet retransmission timer for a specific maintenance entry
   * \param mb maintian byffer entry
   */
  void CancelLinkPacketTimer (SdattackdsrMaintainBuffEntry & mb);
  /**
   ...
   */
  void ScheduleLinkPacketRetry   (SdattackdsrMaintainBuffEntry & mb,
                                  uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void SchedulePassivePacketRetry   (SdattackdsrMaintainBuffEntry & mb,
                                     uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void ScheduleNetworkPacketRetry   (SdattackdsrMaintainBuffEntry & mb,
                                     bool isFirst,
                                     uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void LinkScheduleTimerExpire  (SdattackdsrMaintainBuffEntry & mb,
                                 uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void NetworkScheduleTimerExpire  (SdattackdsrMaintainBuffEntry & mb,
                                    uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void PassiveScheduleTimerExpire  (SdattackdsrMaintainBuffEntry & mb,
                                    uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void SendErrorRequest (SdattackdsrOptionRerrUnreachHeader &rerr, uint8_t protocol);
  /**
  ...
  /**
   * \brief Insert a new Sdattackdsr Option.
   * \param option the option to insert
   */
  void Insert (Ptr<sdattackdsr::SdattackdsrOptions> option);
  /**
   * \brief Get the option corresponding to optionNumber.
   * \param optionNumber the option number of the option to retrieve
   * \return a matching Sdattackdsr option
   */
  Ptr<sdattackdsr::SdattackdsrOptions> GetOption (int optionNumber);
  /**
 ...
 *          */
  ...
  TracedCallback <const SdattackdsrOptionSRHeader &> m_txPacketTrace; ///< packet trace callback
...
private:
   ...
   * \param protocol The protocol we receive, need to verify it is sdattackdsr protocol
   * \param from The from address we received the packet
   * \param to The address this packet is destined for
   * \param packetType The sdattackdsr packet type, 0 is for control packet, 1 for data packet
   ...
   */
  typedef std::list<Ptr<SdattackdsrOptions> > SdattackdsrOptionList_t;
  /**
   * \brief List of DSR Options supported.
   */
  SdattackdsrOptionList_t m_options;
  ...
  SdattackdsrSendBuffer m_sendBuffer;                           ///< The send buffer

  SdattackdsrErrorBuffer m_errorBuffer;                         ///< The error buffer to save the error messages
  ...
  SdattackdsrMaintainBuffer m_maintainBuffer;                   ///< The declaration of maintain buffer
  ...
  Ptr<sdattackdsr::SdattackdsrRouteCache> m_routeCache;                 ///< A "drop-front" queue used by the routing layer to cache routes found.

  Ptr<sdattackdsr::SdattackdsrRreqTable> m_rreqTable;                   ///< A "drop-front" queue used by the routing layer to cache route request sent.

  Ptr<sdattackdsr::SdattackdsrPassiveBuffer> m_passiveBuffer;           ///< A "drop-front" queue used by the routing layer to cache route request sent.
  ...
  std::map<uint32_t, Ptr<sdattackdsr::SdattackdsrNetworkQueue> > m_priorityQueue;   ///< priority queues

  SdattackdsrGraReply m_graReply;                               ///< The gratuitous route reply.

  SdattackdsrNetworkQueue m_networkQueue;                       ///< The network queue.

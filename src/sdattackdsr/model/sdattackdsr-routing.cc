...
#include "sdattackdsr-rreq-table.h"
#include "sdattackdsr-rcache.h"
#include "sdattackdsr-routing.h"
...
NS_OBJECT_ENSURE_REGISTERED (SdattackdsrRouting);

/* see http://www.iana.org/assignments/protocol-numbers */
const uint8_t SdattackdsrRouting::PROT_NUMBER = 48;
/*
 * The extension header is the fixed size sdattackdsr header, it is response for recognizing DSR option types
...
TypeId SdattackdsrRouting::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::sdattackdsr::SdattackdsrRouting")
    ...
    .AddAttribute ("EnableSubRoute",
                   ...
                   BooleanValue (true),
                   MakeBooleanAccessor (&SdattackdsrRouting::m_subRoute),
                   MakeBooleanChecker ())
    .AddAttribute ("RetransIncr",
                   "The increase time for retransmission timer "
                   "when facing network congestion",
                   TimeValue (MilliSeconds (20)),
                   MakeTimeAccessor (&SdattackdsrRouting::m_retransIncr),
                   MakeTimeChecker ())
    .AddAttribute ("MaxNetworkQueueSize",
                   "The max number of packet to save in the network queue.",
                   UintegerValue (400),
                   MakeUintegerAccessor (&SdattackdsrRouting::m_maxNetworkSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("MaxNetworkQueueDelay",
                   "The max time for a packet to stay in the network queue.",
                   TimeValue (Seconds (30.0)),
                   MakeTimeAccessor (&SdattackdsrRouting::m_maxNetworkDelay),
                   MakeTimeChecker ())
    .AddAttribute ("NumPriorityQueues",
                   "The max number of packet to save in the network queue.",
                   UintegerValue (2),
                   MakeUintegerAccessor (&SdattackdsrRouting::m_numPriorityQueues),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("LinkAcknowledgment",
                   "Enable Link layer acknowledgment mechanism",
                   BooleanValue (true),
                   MakeBooleanAccessor (&SdattackdsrRouting::m_linkAck),
                   MakeBooleanChecker ())
    .AddTraceSource ("Tx",
                     "Send DSR packet.",
                     MakeTraceSourceAccessor (&SdattackdsrRouting::m_txPacketTrace),
                     "ns3::sdattackdsr::SdattackdsrOptionSRHeader::TracedCallback")
    .AddTraceSource ("Drop",
                     "Drop DSR packet",
                     MakeTraceSourceAccessor (&SdattackdsrRouting::m_dropTrace),
                     "ns3::Packet::TracedCallback")
  ;
  return tid;
}

SdattackdsrRouting::SdattackdsrRouting ()
{
   ...
   */
  Ptr<sdattackdsr::SdattackdsrOptionPad1> pad1Option = CreateObject<sdattackdsr::SdattackdsrOptionPad1> ();
  Ptr<sdattackdsr::SdattackdsrOptionPadn> padnOption = CreateObject<sdattackdsr::SdattackdsrOptionPadn> ();
  Ptr<sdattackdsr::SdattackdsrOptionRreq> rreqOption = CreateObject<sdattackdsr::SdattackdsrOptionRreq> ();
  Ptr<sdattackdsr::SdattackdsrOptionRrep> rrepOption = CreateObject<sdattackdsr::SdattackdsrOptionRrep> ();
  Ptr<sdattackdsr::SdattackdsrOptionSR>   srOption = CreateObject<sdattackdsr::SdattackdsrOptionSR> ();
  Ptr<sdattackdsr::SdattackdsrOptionRerr>   rerrOption = CreateObject<sdattackdsr::SdattackdsrOptionRerr> ();
  Ptr<sdattackdsr::SdattackdsrOptionAckReq> ackReq = CreateObject<sdattackdsr::SdattackdsrOptionAckReq> ();
  Ptr<sdattackdsr::SdattackdsrOptionAck> ack = CreateObject<sdattackdsr::SdattackdsrOptionAck> ();
  ...
  m_sendBuffTimer.SetFunction (&SdattackdsrRouting::SendBuffTimerExpire, this);
...
void
SdattackdsrRouting::NotifyNewAggregate ()
{
  ...
  Simulator::ScheduleNow (&SdattackdsrRouting::Start, this);
}

void SdattackdsrRouting::Start ()
{
  ...
  for (uint32_t i = 0; i < m_numPriorityQueues; i++)
    {
      ...
      Ptr<sdattackdsr::SdattackdsrNetworkQueue> queue_i = CreateObject<sdattackdsr::SdattackdsrNetworkQueue> (m_maxNetworkSize,m_maxNetworkDelay);
      std::pair<std::map<uint32_t, Ptr<sdattackdsr::SdattackdsrNetworkQueue> >::iterator, bool> result_i = m_priorityQueue.insert (std::make_pair (i, queue_i));
  ...
  Ptr<sdattackdsr::SdattackdsrPassiveBuffer> passiveBuffer = CreateObject<sdattackdsr::SdattackdsrPassiveBuffer> ();
      ...
      for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
        {
          ...
          if (addr != loopback)
            {
              /*
               * Set sdattackdsr route cache
               */
              Ptr<sdattackdsr::SdattackdsrRouteCache> routeCache = CreateObject<sdattackdsr::SdattackdsrRouteCache> ();
              ...
              // routeCache->SetCallback (MakeCallback (&SdattackdsrRouting::SendRerrWhenBreaksLinkToNextHop, this));
              SetRouteCache (routeCache);
              // Set the main address as the current ip address
              m_mainAddress = addr;

              m_ipv4->GetNetDevice (1)->SetPromiscReceiveCallback (MakeCallback (&SdattackdsrRouting::PromiscReceive, this));
...
Ptr<NetDevice>
SdattackdsrRouting::GetNetDeviceFromContext (std::string context)
{
...
SdattackdsrRouting::GetElementsFromContext (std::string context)
{
...
void
SdattackdsrRouting::DoDispose (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  m_node = 0;
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
...
void
SdattackdsrRouting::SetNode (Ptr<Node> node)
{
...
Ptr<Node>
SdattackdsrRouting::GetNode () const
{
  NS_LOG_FUNCTION_NOARGS ();
  return m_node;
}

void SdattackdsrRouting::SetRouteCache (Ptr<sdattackdsr::SdattackdsrRouteCache> r)
{
  // / Set the route cache to use
  m_routeCache = r;
}

Ptr<sdattackdsr::SdattackdsrRouteCache>
SdattackdsrRouting::GetRouteCache () const
{
  // / Get the route cache to use
  return m_routeCache;
}

void SdattackdsrRouting::SetRequestTable (Ptr<sdattackdsr::SdattackdsrRreqTable> q)
{
  // / Set the request table to use
  m_rreqTable = q;
}

Ptr<sdattackdsr::SdattackdsrRreqTable>
SdattackdsrRouting::GetRequestTable () const
{
  // / Get the request table to use
  return m_rreqTable;
}

void SdattackdsrRouting::SetPassiveBuffer (Ptr<sdattackdsr::SdattackdsrPassiveBuffer> p)
{
  // / Set the request table to use
  m_passiveBuffer = p;
}

Ptr<sdattackdsr::SdattackdsrPassiveBuffer>
SdattackdsrRouting::GetPassiveBuffer () const
{
  // / Get the request table to use
...
Ptr<Node>
SdattackdsrRouting::GetNodeWithAddress (Ipv4Address ipv4Address)
{
...
bool SdattackdsrRouting::IsLinkCache ()
{
  return m_routeCache->IsLinkCache ();
}

void SdattackdsrRouting::UseExtends (SdattackdsrRouteCacheEntry::IP_VECTOR rt)
{
  m_routeCache->UseExtends (rt);
}

bool SdattackdsrRouting::LookupRoute (Ipv4Address id, SdattackdsrRouteCacheEntry & rt)
{
  return m_routeCache->LookupRoute (id, rt);
}

bool SdattackdsrRouting::AddRoute_Link (SdattackdsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address source)
{
...
bool SdattackdsrRouting::AddRoute (SdattackdsrRouteCacheEntry & rt)
{
...
void SdattackdsrRouting::DeleteAllRoutesIncludeLink (Ipv4Address errorSrc, Ipv4Address unreachNode, Ipv4Address node)
{
  m_routeCache->DeleteAllRoutesIncludeLink (errorSrc, unreachNode, node);
}

bool SdattackdsrRouting::UpdateRouteEntry (Ipv4Address dst)
{
  return m_routeCache->UpdateRouteEntry (dst);
}

bool SdattackdsrRouting::FindSourceEntry (Ipv4Address src, Ipv4Address dst, uint16_t id)
{
  return m_rreqTable->FindSourceEntry (src, dst, id);
}

Ipv4Address
SdattackdsrRouting::GetIPfromMAC (Mac48Address address)
{
...
void SdattackdsrRouting::PrintVector (std::vector<Ipv4Address>& vec)
{
...
Ipv4Address SdattackdsrRouting::SearchNextHop (Ipv4Address ipv4Address, std::vector<Ipv4Address>& vec)
{
  NS_LOG_FUNCTION (this << ipv4Address);
  Ipv4Address nextHop;
...
Ptr<Ipv4Route>
SdattackdsrRouting::SetRoute (Ipv4Address nextHop, Ipv4Address srcAddress)
{
...
int
SdattackdsrRouting::GetProtocolNumber (void) const
{
...
uint16_t
SdattackdsrRouting::GetIDfromIP (Ipv4Address address)
{
  int32_t nNodes = NodeList::GetNNodes ();
  for (int32_t i = 0; i < nNodes; ++i)
    {
...
Ipv4Address
SdattackdsrRouting::GetIPfromID (uint16_t id)
{
  if (id >= 256)
    {
...
uint32_t
SdattackdsrRouting::GetPriority (SdattackdsrMessageType messageType)
{
...
void SdattackdsrRouting::SendBuffTimerExpire ()
{
      ...
      if (findRoute)
        {
          ...
          Ptr<Packet> sdattackdsrPacket = packet->Copy ();
          sdattackdsrPacket->RemoveHeader (sdattackdsrRoutingHeader);
          uint32_t offset = sdattackdsrRoutingHeader.GetSdattackdsrOptionsOffset ();
          ...
          if (optionType == 3)
            {
              Ptr<sdattackdsr::SdattackdsrOptions> sdattackdsrOption;
              ...
              if (errorType == 1) // This is the Route Error Option
                {
                  ...
                  if (nextHop == "0.0.0.0")
                    {
                      PacketNewRoute (sdattackdsrPacket, m_mainAddress, destination, protocol);
                      return;
                    }
                  ...
                  sdattackdsrRoutingHeader.SetSourceId (GetIDfromIP (m_mainAddress));
                  sdattackdsrRoutingHeader.SetDestId (255);
                  sdattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 4);
                  sdattackdsrRoutingHeader.AddSdattackdsrOption (newUnreach);
                  sdattackdsrRoutingHeader.AddSdattackdsrOption (sourceRoute);

                  Ptr<Packet> newPacket = Create<Packet> ();
                  newPacket->AddHeader (sdattackdsrRoutingHeader); // Add the routing header with rerr and sourceRoute attached to it
                  ...
                  std::map<uint32_t, Ptr<sdattackdsr::SdattackdsrNetworkQueue> >::iterator i = m_priorityQueue.find (priority);
                  Ptr<sdattackdsr::SdattackdsrNetworkQueue> sdattackdsrNetworkQueue = i->second;
                  ...
                  /// \todo New SdattackdsrNetworkQueueEntry
                  SdattackdsrNetworkQueueEntry newEntry (newPacket, m_mainAddress, nextHop, Simulator::Now (), m_ipv4Route);

                  if (sdattackdsrNetworkQueue->Enqueue (newEntry))
                    {
                      Scheduler (priority);
                    }
                  else
                    {
                      NS_LOG_INFO ("Packet dropped as sdattackdsr network queue is full");
                    }
                }
            }
          else
            {
              sdattackdsrRoutingHeader.SetNextHeader (protocol);
              sdattackdsrRoutingHeader.SetMessageType (2);
              sdattackdsrRoutingHeader.SetSourceId (GetIDfromIP (m_mainAddress));
              sdattackdsrRoutingHeader.SetDestId (GetIDfromIP (destination));
              ...
              sdattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 2);
              sdattackdsrRoutingHeader.AddSdattackdsrOption (sourceRoute);
              cleanP->AddHeader (sdattackdsrRoutingHeader);
              Ptr<const Packet> mtP = cleanP->Copy ();
              // Put the data packet in the maintenance queue for data packet retransmission
              SdattackdsrMaintainBuffEntry newEntry (/*Packet=*/ mtP, /*Ipv4Address=*/ m_mainAddress, /*nextHop=*/ nextHop,
              ...
              Simulator::Schedule (m_sendBuffInterval, &SdattackdsrRouting::SendBuffTimerExpire, this);
              return;
            }
        }
      else
...
bool SdattackdsrRouting::PromiscReceive (Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol, const Address &from,
                                 const Address &to, NetDevice::PacketType packetType)
{
  ...
  if (ipv4Header.GetProtocol () != SdattackdsrRouting::PROT_NUMBER)
    {
      return false;
    }
  // Remove the sdattackdsr routing header here
  Ptr<Packet> pktMinusSdattackdsrHdr = pktMinusIpHdr->Copy ();
  SdattackdsrRoutingHeader sdattackdsrRouting;
  pktMinusSdattackdsrHdr->RemoveHeader (sdattackdsrRouting);
  ...
  if (sdattackdsrRouting.GetMessageType () == 2 && ourAddress == m_mainAddress)
    {
      NS_LOG_DEBUG ("data packet receives " << packet->GetUid ());
      Ipv4Address sourceIp = GetIPfromID (sdattackdsrRouting.GetSourceId ());
      Ipv4Address destinationIp = GetIPfromID ( sdattackdsrRouting.GetDestId ());
      ...
      SdattackdsrMaintainBuffEntry newEntry;
      ...
      sdattackdsr->CancelLinkPacketTimer (newEntry);
    }
  ...
  if (packetType == NetDevice::PACKET_OTHERHOST)
    {
      ...
      uint8_t offset = sdattackdsrRouting.GetSdattackdsrOptionsOffset ();        // Get the offset for option header, 4 bytes in this case
      uint8_t nextHeader = sdattackdsrRouting.GetNextHeader ();
      uint32_t sourceId = sdattackdsrRouting.GetSourceId ();
      ...
      if (optionType == 96)        // This is the source route option
        {
          ...
          sdattackdsrOption = GetOption (optionType);       // Get the relative DSR option and demux to the process function
          NS_LOG_DEBUG (Simulator::Now ().GetSeconds () <<
                        ...
                        " and packet : " << *pktMinusSdattackdsrHdr);

          bool isPromisc = true;                     // Set the boolean value isPromisc as true
          sdattackdsrOption->Process (pktMinusIpHdr, pktMinusSdattackdsrHdr, m_mainAddress, source, ipv4Header, nextHeader, isPromisc, promiscSource);
          return true;
...
void
SdattackdsrRouting::PacketNewRoute (Ptr<Packet> packet,
                            Ipv4Address source,
  ...
  if (!findRoute)
    {
      ...
      SdattackdsrSendBuffEntry newEntry (p, destination, m_sendBufferTimeout, protocol);     // Create a new entry for send buffer
...
void
SdattackdsrRouting::SendUnreachError (Ipv4Address unreachNode, Ipv4Address destination, Ipv4Address originalDst, uint8_t salvage, uint8_t protocol)
{
  ...
  SdattackdsrOptionRerrUnreachHeader rerrUnreachHeader;
  ...
  if (!findRoute)
    {
      if (destination == m_mainAddress)
      ...
      else
        {
          ...
          sdattackdsrRoutingHeader.SetPayloadLength (rerrLength + 2);
          sdattackdsrRoutingHeader.AddSdattackdsrOption (rerrUnreachHeader);
          newPacket->AddHeader (sdattackdsrRoutingHeader);
          Ptr<Packet> p = newPacket->Copy ();
          // Save the error packet in the error buffer
          SdattackdsrErrorBuffEntry newEntry (p, destination, m_mainAddress, unreachNode, m_sendBufferTimeout, protocol);
  ...
  else
    {
      ...
      std::map<uint32_t, Ptr<sdattackdsr::SdattackdsrNetworkQueue> >::iterator i = m_priorityQueue.find (priority);
      Ptr<sdattackdsr::SdattackdsrNetworkQueue> sdattackdsrNetworkQueue = i->second;
      NS_LOG_DEBUG ("Will be inserting into priority queue " << sdattackdsrNetworkQueue << " number: " << priority);
...
void
SdattackdsrRouting::ForwardErrPacket (SdattackdsrOptionRerrUnreachHeader &rerr,
                              SdattackdsrOptionSRHeader &sourceRoute,
                              Ipv4Address nextHop,
                              uint8_t protocol,
                              Ptr<Ipv4Route> route)
{
  ...
  NS_ASSERT_MSG (!m_downTarget.IsNull (), "Error, SdattackdsrRouting cannot send downward");
  SdattackdsrRoutingHeader sdattackdsrRoutingHeader;
  sdattackdsrRoutingHeader.SetNextHeader (protocol);
  sdattackdsrRoutingHeader.SetMessageType (1);
  sdattackdsrRoutingHeader.SetSourceId (GetIDfromIP (rerr.GetErrorSrc ()));
  sdattackdsrRoutingHeader.SetDestId (GetIDfromIP (rerr.GetErrorDst ()));

  uint8_t length = (sourceRoute.GetLength () + rerr.GetLength ());
  sdattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 4);
  sdattackdsrRoutingHeader.AddSdattackdsrOption (rerr);
  sdattackdsrRoutingHeader.AddSdattackdsrOption (sourceRoute);
  Ptr<Packet> packet = Create<Packet> ();
  packet->AddHeader (sdattackdsrRoutingHeader);
  Ptr<NetDevice> dev = m_ip->GetNetDevice (m_ip->GetInterfaceForAddress (m_mainAddress));
  ...
  /// \todo New SdattackdsrNetworkQueueEntry
  SdattackdsrNetworkQueueEntry newEntry (packet, m_mainAddress, nextHop, Simulator::Now (), route);
...
void
SdattackdsrRouting::Send (Ptr<Packet> packet,
                  Ipv4Address source,
                  Ipv4Address destination,
                  uint8_t protocol,
                  Ptr<Ipv4Route> route)
  ...
  else
    {
      ...
      if (!findRoute)
      ...
      else
        {
          ...
          // Put the data packet in the maintenance queue for data packet retransmission
          SdattackdsrMaintainBuffEntry newEntry (/*Packet=*/ mtP, /*ourAddress=*/ m_mainAddress, /*nextHop=*/ nextHop,
                                                  /*source=*/ source, /*destination=*/ destination, /*ackId=*/ 0,
                                                  /*SegsLeft=*/ nodeList.size () - 2, /*expire time=*/ m_maxMaintainTime);
          bool result = m_maintainBuffer.Enqueue (newEntry);       // Enqueue the packet the the maintenance buffer
          if (result)
          ...
          if (m_sendBuffer.GetSize () != 0 && m_sendBuffer.Find (destination))
            {
              ...
              Simulator::Schedule (MilliSeconds (m_uniformRandomVariable->GetInteger (0,100)),
                                   &SdattackdsrRouting::SendPacketFromBuffer, this, sourceRoute, nextHop, protocol);
            }
        }
    }
}
...
uint16_t
SdattackdsrRouting::AddAckReqHeader (Ptr<Packet>& packet, Ipv4Address nextHop)
{
  NS_LOG_FUNCTION (this << packet << nextHop);
  // This packet is used to peek option type
  Ptr<Packet> sdattackdsrP = packet->Copy ();
  Ptr<Packet> tmpP = packet->Copy ();

  SdattackdsrRoutingHeader sdattackdsrRoutingHeader;
  sdattackdsrP->RemoveHeader (sdattackdsrRoutingHeader);          // Remove the DSR header in whole
  uint8_t protocol = sdattackdsrRoutingHeader.GetNextHeader ();
  uint32_t sourceId = sdattackdsrRoutingHeader.GetSourceId ();
  uint32_t destinationId = sdattackdsrRoutingHeader.GetDestId ();
  uint32_t offset = sdattackdsrRoutingHeader.GetSdattackdsrOptionsOffset ();
  ...
  tmpP->RemoveHeader (sourceRoute);               // this is a clean packet without any sdattackdsr involved headers
  ...
  SdattackdsrRoutingHeader newSdattackdsrRoutingHeader;
  newSdattackdsrRoutingHeader.SetNextHeader (protocol);
  newSdattackdsrRoutingHeader.SetMessageType (2);
  newSdattackdsrRoutingHeader.SetSourceId (sourceId);
  newSdattackdsrRoutingHeader.SetDestId (destinationId);
  newSdattackdsrRoutingHeader.SetPayloadLength (length + 4);
  newSdattackdsrRoutingHeader.AddSdattackdsrOption (sourceRoute);
  newSdattackdsrRoutingHeader.AddSdattackdsrOption (ackReq);
  sdattackdsrP->AddHeader (newSdattackdsrRoutingHeader);
  // give the sdattackdsrP value to packet and then return
  packet = sdattackdsrP;
  return m_ackId;
}

void
SdattackdsrRouting::SendPacket (Ptr<Packet> packet, Ipv4Address source, Ipv4Address nextHop, uint8_t protocol)
{
  ...
  /// \todo New SdattackdsrNetworkQueueEntry
  SdattackdsrNetworkQueueEntry newEntry (packet, source, nextHop, Simulator::Now (), m_ipv4Route);
...
void
SdattackdsrRouting::Scheduler (uint32_t priority)
{
  NS_LOG_FUNCTION (this);
...
void
SdattackdsrRouting::PriorityScheduler (uint32_t priority, bool continueWithFirst)
{
  ...
  for (uint32_t i = priority; numPriorities < m_numPriorityQueues; numPriorities++)
    {
      std::map<uint32_t, Ptr<SdattackdsrNetworkQueue> >::iterator q = m_priorityQueue.find (i);
      Ptr<sdattackdsr::SdattackdsrNetworkQueue> sdattackdsrNetworkQueue = q->second;
      uint32_t queueSize = sdattackdsrNetworkQueue->GetSize ();
      if (queueSize == 0)
        {
      ...
      else
        {
          ...
          for (std::map<uint32_t, Ptr<sdattackdsr::SdattackdsrNetworkQueue> >::iterator j = m_priorityQueue.begin (); j != m_priorityQueue.end (); j++)
            {
          ...
          SdattackdsrNetworkQueueEntry newEntry;
          sdattackdsrNetworkQueue->Dequeue (newEntry);
          if (SendRealDown (newEntry))
            {
              NS_LOG_LOGIC ("Packet sent by Sdattackdsr. Calling PriorityScheduler after some time");
              ...
              Simulator::Schedule (MicroSeconds (m_uniformRandomVariable->GetInteger (0, 1000)),
                                   &SdattackdsrRouting::PriorityScheduler,this, i, false);
            }
          else
            {
              // packet was dropped by Sdattackdsr. Call scheduler immediately so that we can
              // send another packet immediately.
              NS_LOG_LOGIC ("Packet dropped by Sdattackdsr. Calling PriorityScheduler immediately");
              Simulator::Schedule (Seconds (0), &SdattackdsrRouting::PriorityScheduler, this, i, false);
            }
...
void
SdattackdsrRouting::IncreaseRetransTimer ()
{
  NS_LOG_FUNCTION (this);
  ...
  std::vector<SdattackdsrNetworkQueueEntry> newNetworkQueue = sdattackdsrNetworkQueue->GetQueue ();
  for (std::vector<SdattackdsrNetworkQueueEntry>::iterator i = newNetworkQueue.begin (); i != newNetworkQueue.end (); i++)
    {
...
bool
SdattackdsrRouting::SendRealDown (SdattackdsrNetworkQueueEntry & newEntry)
{
  NS_LOG_FUNCTION (this);
...
void
SdattackdsrRouting::SendPacketFromBuffer (SdattackdsrOptionSRHeader const &sourceRoute, Ipv4Address nextHop, uint8_t protocol)
{
  ...
  if (m_sendBuffer.Find (destination))
    {
      ...
      SdattackdsrSendBuffEntry entry;
      if (m_sendBuffer.Dequeue (destination, entry))
        {
          ...
          p->AddHeader (sdattackdsrRoutingHeader);
   ...
   */
    ...
    {
      SdattackdsrErrorBuffEntry entry;
              ...
              if (errorType == 1)
                {
                  ...
                  std::vector<Ipv4Address> nodeList = sourceRoute.GetNodesAddress ();
                  SdattackdsrRoutingHeader newRoutingHeader;
                  ...
                  newRoutingHeader.AddSdattackdsrOption (newUnreach);
                  newRoutingHeader.AddSdattackdsrOption (sourceRoute);
                  /// When found a route and use it, UseExtends to the link cache
                  if (m_routeCache->IsLinkCache ())
                    {
                      m_routeCache->UseExtends (nodeList);
...
bool
SdattackdsrRouting::PassiveEntryCheck (Ptr<Packet> packet, Ipv4Address source, Ipv4Address destination, uint8_t segsLeft,
                               uint16_t fragmentOffset, uint16_t identification, bool saveEntry)
{
  ...
  Ptr<Packet> p = packet->Copy ();
  // Here the segments left value need to plus one to check the earlier hop maintain buffer entry
  SdattackdsrPassiveBuffEntry newEntry;
  newEntry.SetPacket (p);
  ...
  if (m_passiveBuffer->AllEqual (newEntry) && (!saveEntry))
    {
      ...
      SdattackdsrMaintainBuffEntry mbEntry;
      mbEntry.SetPacket (p);
...
bool
SdattackdsrRouting::CancelPassiveTimer (Ptr<Packet> packet, Ipv4Address source, Ipv4Address destination,
                                uint8_t segsLeft)
{
  NS_LOG_FUNCTION (this << packet << source << destination << (uint32_t)segsLeft);
...
void
SdattackdsrRouting::CallCancelPacketTimer (uint16_t ackId, Ipv4Header const& ipv4Header, Ipv4Address realSrc, Ipv4Address realDst)
{
  ...
  SdattackdsrMaintainBuffEntry newEntry (/*Packet=*/ mainP, /*ourAddress=*/ sender, /*nextHop=*/ receiver,
...
void
SdattackdsrRouting::CancelPacketAllTimer (SdattackdsrMaintainBuffEntry & mb)
{
  NS_LOG_FUNCTION (this);
...
void
SdattackdsrRouting::CancelLinkPacketTimer (SdattackdsrMaintainBuffEntry & mb)
{
  NS_LOG_FUNCTION (this);
  LinkKey linkKey;
...
void
SdattackdsrRouting::CancelNetworkPacketTimer (SdattackdsrMaintainBuffEntry & mb)
{
  NS_LOG_FUNCTION (this);
  NetworkKey networkKey;
...
void
SdattackdsrRouting::CancelPassivePacketTimer (SdattackdsrMaintainBuffEntry & mb)
{
  NS_LOG_FUNCTION (this);
  PassiveKey passiveKey;
  passiveKey.m_ackId = 0;
...
void
SdattackdsrRouting::CancelPacketTimerNextHop (Ipv4Address nextHop, uint8_t protocol)
{
  NS_LOG_FUNCTION (this << nextHop << (uint32_t)protocol);

  SdattackdsrMaintainBuffEntry entry;
  ...
  if (m_maintainBuffer.Dequeue (nextHop, entry))
    {
      ...
      Ptr<Packet> sdattackdsrP = entry.GetPacket ()->Copy ();
      Ptr<Packet> p = sdattackdsrP->Copy ();
      Ptr<Packet> packet = sdattackdsrP->Copy ();
      SdattackdsrRoutingHeader sdattackdsrRoutingHeader;
      sdattackdsrP->RemoveHeader (sdattackdsrRoutingHeader);          // Remove the sdattackdsr header in whole
      uint32_t offset = sdattackdsrRoutingHeader.GetSdattackdsrOptionsOffset ();
      ...
      if (m_maintainBuffer.GetSize () && m_maintainBuffer.Find (nextHop))
        {
          ...
          Simulator::Schedule (MilliSeconds (m_uniformRandomVariable->GetInteger (0,100)),
                               &SdattackdsrRouting::CancelPacketTimerNextHop,this,nextHop,protocol);
        }
    }
  else
    {
...
void
SdattackdsrRouting::SalvagePacket (Ptr<const Packet> packet, Ipv4Address source, Ipv4Address dst, uint8_t protocol)
{
  ...
  SdattackdsrRoutingHeader sdattackdsrRoutingHeader;
  p->RemoveHeader (sdattackdsrRoutingHeader);
  // Remove offset of sdattackdsr routing header
  uint8_t offset = sdattackdsrRoutingHeader.GetSdattackdsrOptionsOffset ();
  ...
  if (findRoute && (salvage < m_maxSalvageCount))
    {
      ...
      newSdattackdsrRoutingHeader.SetNextHeader (protocol);
      newSdattackdsrRoutingHeader.SetMessageType (2);
      newSdattackdsrRoutingHeader.SetSourceId (GetIDfromIP (source));
      newSdattackdsrRoutingHeader.SetDestId (GetIDfromIP (dst));
      ...
      newSdattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 2);
      newSdattackdsrRoutingHeader.AddSdattackdsrOption (sourceRoute);
      p->AddHeader (newSdattackdsrRoutingHeader);
      ...
      /// \todo New SdattackdsrNetworkQueueEntry
      SdattackdsrNetworkQueueEntry newEntry (p, m_mainAddress, nextHop, Simulator::Now (), m_ipv4Route);
...
void
SdattackdsrRouting::ScheduleLinkPacketRetry (SdattackdsrMaintainBuffEntry & mb,
                                     uint8_t protocol)
{
  ...
  m_linkAckTimer[linkKey].SetFunction (&SdattackdsrRouting::LinkScheduleTimerExpire, this);
  m_linkAckTimer[linkKey].Remove ();
...
void
SdattackdsrRouting::SchedulePassivePacketRetry (SdattackdsrMaintainBuffEntry & mb,
                                        uint8_t protocol)
{
  NS_LOG_FUNCTION (this << (uint32_t)protocol);
  ...
  m_passiveAckTimer[passiveKey].SetFunction (&SdattackdsrRouting::PassiveScheduleTimerExpire, this);
  m_passiveAckTimer[passiveKey].Remove ();
...
void
SdattackdsrRouting::ScheduleNetworkPacketRetry (SdattackdsrMaintainBuffEntry & mb,
                                        bool isFirst,
                                        uint8_t protocol)
{
  Ptr<Packet> p = Create<Packet> ();
  Ptr<Packet> sdattackdsrP = Create<Packet> ();
  ...
  if (isFirst)
    {
      ...
      m_addressForwardTimer[networkKey].SetFunction (&SdattackdsrRouting::NetworkScheduleTimerExpire, this);
      m_addressForwardTimer[networkKey].Remove ();
  ...
  else
    {
      ...
      p = mb.GetPacket ()->Copy ();
      sdattackdsrP = mb.GetPacket ()->Copy ();
      ...
      NS_LOG_DEBUG ("The packet with sdattackdsr header " << sdattackdsrP->GetSize ());
      networkKey.m_ackId = mb.GetAckId ();
      networkKey.m_ourAdd = mb.GetOurAdd ();
      networkKey.m_nextHop = mb.GetNextHop ();
      networkKey.m_source = mb.GetSrc ();
...
void
SdattackdsrRouting::LinkScheduleTimerExpire  (SdattackdsrMaintainBuffEntry & mb,
                                      uint8_t protocol)
{
  NS_LOG_FUNCTION (this << (uint32_t)protocol);
  Ipv4Address nextHop = mb.GetNextHop ();
...
void
SdattackdsrRouting::PassiveScheduleTimerExpire  (SdattackdsrMaintainBuffEntry & mb,
                                         uint8_t protocol)
{
  NS_LOG_FUNCTION (this << (uint32_t)protocol);
  Ipv4Address nextHop = mb.GetNextHop ();
...
int64_t
SdattackdsrRouting::AssignStreams (int64_t stream)
{
...
void
SdattackdsrRouting::NetworkScheduleTimerExpire  (SdattackdsrMaintainBuffEntry & mb,
                                         uint8_t protocol)
{
  Ptr<Packet> p = mb.GetPacket ()->Copy ();
  Ipv4Address source = mb.GetSrc ();
...
void
SdattackdsrRouting::ForwardPacket (Ptr<const Packet> packet,
                           SdattackdsrOptionSRHeader &sourceRoute,
...
void
SdattackdsrRouting::SendInitialRequest (Ipv4Address source,
                                Ipv4Address destination,
                                uint8_t protocol)
{
  ...
  SdattackdsrOptionRreqHeader rreqHeader;                                  // has an alignment of 4n+0
  ...
  sdattackdsrRoutingHeader.AddSdattackdsrOption (rreqHeader);                      // Add the rreqHeader to the sdattackdsr extension header
...
void
SdattackdsrRouting::SendErrorRequest (SdattackdsrOptionRerrUnreachHeader &rerr, uint8_t protocol)
{
  NS_LOG_FUNCTION (this << (uint32_t)protocol);
  NS_ASSERT_MSG (!m_downTarget.IsNull (), "Error, SdattackdsrRouting cannot send downward");
  ...
  if (m_routeCache->LookupRoute (dst, toDst))
  ...
  else
    {
      ...
      sdattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 4);
      dstP->AddHeader (sdattackdsrRoutingHeader);
...
void
SdattackdsrRouting::CancelRreqTimer (Ipv4Address dst, bool isRemove)
{
...
void
SdattackdsrRouting::ScheduleRreqRetry (Ptr<Packet> packet, std::vector<Ipv4Address> address, bool nonProp, uint32_t requestId, uint8_t protocol)
{
  ...
  if (nonProp)
    {
      ...
      m_nonPropReqTimer[dst].SetFunction (&SdattackdsrRouting::RouteRequestTimerExpire, this);
      m_nonPropReqTimer[dst].Remove ();
  ...
  else
    {
      ...
      address.push_back (source);
      address.push_back (dst);
      m_addressReqTimer[dst].SetFunction (&SdattackdsrRouting::RouteRequestTimerExpire, this);
      m_addressReqTimer[dst].Remove ();
...
void
SdattackdsrRouting::RouteRequestTimerExpire (Ptr<Packet> packet, std::vector<Ipv4Address> address, uint32_t requestId, uint8_t protocol)
{
  NS_LOG_FUNCTION (this << packet << requestId << (uint32_t)protocol);
  // Get a clean packet without sdattackdsr header
  Ptr<Packet> sdattackdsrP = packet->Copy ();
  SdattackdsrRoutingHeader sdattackdsrRoutingHeader;
  sdattackdsrP->RemoveHeader (sdattackdsrRoutingHeader);          // Remove the sdattackdsr header in whole
  ...
  if (m_routeCache->LookupRoute (dst, toDst))
    {
      ...
      if (nextHop == "0.0.0.0")
        {
          NS_LOG_DEBUG ("Error next hop address");
          PacketNewRoute (sdattackdsrP, source, dst, protocol);
          return;
        }
      SetRoute (nextHop, m_mainAddress);
      CancelRreqTimer (dst, true);
...
void
SdattackdsrRouting::SendRequest (Ptr<Packet> packet,
                         Ipv4Address source)
{
  ...
  /// \todo New SdattackdsrNetworkQueueEntry
  SdattackdsrNetworkQueueEntry newEntry (packet, source, m_broadcast, Simulator::Now (), 0);
  if (sdattackdsrNetworkQueue->Enqueue (newEntry))
    {
      Scheduler (priority);
    }
...
void
SdattackdsrRouting::ScheduleInterRequest (Ptr<Packet> packet)
{
   ...
   */
  Simulator::Schedule (MilliSeconds (m_uniformRandomVariable->GetInteger (0, m_broadcastJitter)), &SdattackdsrRouting::SendRequest, this,
                       packet, m_mainAddress);
}

void
SdattackdsrRouting::SendGratuitousReply (Ipv4Address source, Ipv4Address srcAddress, std::vector<Ipv4Address> &nodeList, uint8_t protocol)
{
  ...
  if (!(m_graReply.FindAndUpdate (source, srcAddress, m_gratReplyHoldoff)))     // Find the gratuitous reply entry
    {
      ...
      sdattackdsrRoutingHeader.SetNextHeader (protocol);
      sdattackdsrRoutingHeader.SetMessageType (1);
      sdattackdsrRoutingHeader.SetSourceId (GetIDfromIP (replySrc));
...
void
SdattackdsrRouting::SendReply (Ptr<Packet> packet,
                       Ipv4Address source,
                       Ipv4Address nextHop,
                       Ptr<Ipv4Route> route)
{
  ...
  /// \todo New SdattackdsrNetworkQueueEntry
  SdattackdsrNetworkQueueEntry newEntry (packet, source, nextHop, Simulator::Now (), route);
  if (sdattackdsrNetworkQueue->Enqueue (newEntry))
    {
      Scheduler (priority);
    }
...
void
SdattackdsrRouting::ScheduleInitialReply (Ptr<Packet> packet,
                                  Ipv4Address source,
                                  Ipv4Address nextHop,
                                  Ptr<Ipv4Route> route)
{
  NS_LOG_FUNCTION (this << packet << source << nextHop);
  Simulator::ScheduleNow (&SdattackdsrRouting::SendReply, this,
                          packet, source, nextHop, route);
}

void
SdattackdsrRouting::ScheduleCachedReply (Ptr<Packet> packet,
                                 Ipv4Address source,
                                 Ipv4Address destination,
                                 Ptr<Ipv4Route> route,
                                 double hops)
{
  ...
  Simulator::Schedule (Time (2 * m_nodeTraversalTime * (hops - 1 + m_uniformRandomVariable->GetValue (0,1))), &SdattackdsrRouting::SendReply, this, packet, source, destination, route);
}

void
SdattackdsrRouting::SendAck   (uint16_t ackId,
                       Ipv4Address destination,
                       Ipv4Address realSrc,
                       Ipv4Address realDst,
                       uint8_t protocol,
                       Ptr<Ipv4Route> route)
{
  ...
  sdattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 2);
  sdattackdsrRoutingHeader.AddSdattackdsrOption (ack);
  ...
  NS_LOG_LOGIC ("Will be inserting into priority queue " << sdattackdsrNetworkQueue << " number: " << priority);

  //m_downTarget (packet, m_mainAddress, destination, GetProtocolNumber (), route);

  /// \todo New SdattackdsrNetworkQueueEntry
  SdattackdsrNetworkQueueEntry newEntry (packet, m_mainAddress, destination, Simulator::Now (), route);
  if (sdattackdsrNetworkQueue->Enqueue (newEntry))
    {
      Scheduler (priority);
    }
...
SdattackdsrRouting::Receive (Ptr<Packet> p,
                     Ipv4Header const &ip,
                     Ptr<Ipv4Interface> incomingInterface)
{
   ...
   */
  SdattackdsrRoutingHeader sdattackdsrRoutingHeader;
  packet->RemoveHeader (sdattackdsrRoutingHeader);          // Remove the DSR header in whole
  ...
  uint32_t offset = sdattackdsrRoutingHeader.GetSdattackdsrOptionsOffset ();        // Get the offset for option header, 8 bytes in this case
  ...
  sdattackdsrOption = GetOption (optionType);       // Get the relative sdattackdsr option and demux to the process function
  ...
  if (optionType == 1)        // This is the request option
    {
      ...
      sdattackdsrOption = GetOption (optionType);
      optionLength = sdattackdsrOption->Process (p, packet, m_mainAddress, source, ip, protocol, isPromisc, promiscSource);
      ...
      if (optionLength == 0)
      ...
      else
        {
          if (segmentsLeft == 0)
            {
              // / Get the next header
              uint8_t nextHeader = sdattackdsrRoutingHeader.GetNextHeader ();
  ...
  else
    {
      ...
      SdattackdsrOptionRerrUnsupportHeader rerrUnsupportHeader;
...
void
SdattackdsrRouting::SetDownTarget (DownTargetCallback callback)
{
  m_downTarget = callback;
}

void
SdattackdsrRouting::SetDownTarget6 (DownTargetCallback6 callback)
{
...
SdattackdsrRouting::GetDownTarget (void) const
{
  return m_downTarget;
}

IpL4Protocol::DownTargetCallback6
SdattackdsrRouting::GetDownTarget6 (void) const
{
  NS_FATAL_ERROR ("Unimplemented");
  return MakeNullCallback<void,Ptr<Packet>, Ipv6Address, Ipv6Address, uint8_t, Ptr<Ipv6Route> > ();
}

void SdattackdsrRouting::Insert (Ptr<sdattackdsr::SdattackdsrOptions> option)
{
  m_options.push_back (option);
}

Ptr<sdattackdsr::SdattackdsrOptions> SdattackdsrRouting::GetOption (int optionNumber)
{
  for (SdattackdsrOptionList_t::iterator i = m_options.begin (); i != m_options.end (); ++i)
    {

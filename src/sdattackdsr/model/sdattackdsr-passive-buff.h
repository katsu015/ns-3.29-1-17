 ...
 */
class SdattackdsrPassiveBuffEntry
{
public:
  /**
   * Construct a SdattackdsrPassiveBuffEntry with the given parameters
   *
   * \param pa packet
   ...
   */
  SdattackdsrPassiveBuffEntry (Ptr<const Packet> pa = 0, Ipv4Address d = Ipv4Address (), Ipv4Address s = Ipv4Address (),
   ...
   */
  bool operator== (SdattackdsrPassiveBuffEntry const & o) const
  {
...
/**
 * \ingroup sdattackdsr
 * \class SdattackdsrPassiveBuffer
 * \brief DSR passive buffer
 */
/************************************************************************************************************************/
class SdattackdsrPassiveBuffer  : public Object
{
public:
  /**
  ...
  SdattackdsrPassiveBuffer ();
  virtual ~SdattackdsrPassiveBuffer ();

  /// Push entry in queue, if there is no entry with the same packet and destination address in queue.
  /// \param entry Buffer Entry
  /// \return true on success adding the Entry.
  bool Enqueue (SdattackdsrPassiveBuffEntry & entry);
  /// Return first found (the earliest) entry for given destination
  /// \param [in] dst Entry destination
  /// \param [out] entry The Entry found (if any).
  /// \return true on success
  bool Dequeue (Ipv4Address dst, SdattackdsrPassiveBuffEntry & entry);
 ...
 /// \return true if an Entry was found and removed.
  bool AllEqual (SdattackdsrPassiveBuffEntry & entry);
  /// Number of entries
...
private:
  ...
  std::vector<SdattackdsrPassiveBuffEntry> m_passiveBuffer;
  /// Remove all expired entries
  void Purge ();
  /// Notify that packet is dropped from queue by timeout
  /// \param en BuffEntry Buffer entry
  /// \param reason Drop reason
  void Drop (SdattackdsrPassiveBuffEntry en, std::string reason);
  /// Notify that packet is dropped from queue by timeout
  /// \param en BuffEntry Buffer entry
  /// \param reason Drop reason
  void DropLink (SdattackdsrPassiveBuffEntry en, std::string reason);
  ...
  static bool LinkEqual (SdattackdsrPassiveBuffEntry en, const std::vector<Ipv4Address> link)
  {

...
NS_LOG_COMPONENT_DEFINE ("SdattackdsrRouteCache");

namespace sdattackdsr {

bool CompareRoutesBoth (const SdattackdsrRouteCacheEntry &a, const SdattackdsrRouteCacheEntry &b)
{
...
bool CompareRoutesHops (const SdattackdsrRouteCacheEntry &a, const SdattackdsrRouteCacheEntry &b)
{
  // compare based on hops
  return a.GetVector ().size () < b.GetVector ().size ();
}

bool CompareRoutesExpire (const SdattackdsrRouteCacheEntry &a, const SdattackdsrRouteCacheEntry &b)
{
...
SdattackdsrNodeStab::SdattackdsrNodeStab (Time nodeStab)
  : m_nodeStability (nodeStab + Simulator::Now ())
{
}

SdattackdsrNodeStab::~SdattackdsrNodeStab ()
{
}

SdattackdsrLinkStab::SdattackdsrLinkStab (Time linkStab)
  : m_linkStability (linkStab + Simulator::Now ())
{
}

SdattackdsrLinkStab::~SdattackdsrLinkStab ()
{
}

void SdattackdsrLinkStab::Print ( ) const
{
  NS_LOG_LOGIC ("LifeTime: " << GetLinkStability ().GetSeconds ());
}

typedef std::list<SdattackdsrRouteCacheEntry>::value_type route_pair;

SdattackdsrRouteCacheEntry::SdattackdsrRouteCacheEntry (IP_VECTOR const  & ip, Ipv4Address dst, Time exp)
...
SdattackdsrRouteCacheEntry::~SdattackdsrRouteCacheEntry ()
{
}

void
SdattackdsrRouteCacheEntry::Invalidate (Time badLinkLifetime)
{
  m_reqCount = 0;
...
void
SdattackdsrRouteCacheEntry::Print (std::ostream & os) const
{
  os << m_dst << "\t" << (m_expire - Simulator::Now ()).GetSeconds ()
     << "\t";
}

NS_OBJECT_ENSURE_REGISTERED (SdattackdsrRouteCache);

TypeId SdattackdsrRouteCache::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::sdattackdsr::SdattackdsrRouteCache")
    .SetParent<Object> ()
    .SetGroupName ("Sdattackdsr")
    .AddConstructor<SdattackdsrRouteCache> ()
  ;
  return tid;
}

SdattackdsrRouteCache::SdattackdsrRouteCache ()
  : m_vector (0),
    ...
    m_delay (MilliSeconds (100))
{
  ...
  m_ntimer.SetFunction (&SdattackdsrRouteCache::PurgeMac, this);
  m_txErrorCallback = MakeCallback (&SdattackdsrRouteCache::ProcessTxError, this);
}
...
void
SdattackdsrRouteCache::RemoveLastEntry (std::list<SdattackdsrRouteCacheEntry> & rtVector)
{
...
bool
SdattackdsrRouteCache::UpdateRouteEntry (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
  std::map<Ipv4Address, std::list<SdattackdsrRouteCacheEntry> >::const_iterator i =
  ...
  if (i == m_sortedRoutes.end ())
    ...
    {
      std::list<SdattackdsrRouteCacheEntry> rtVector = i->second;
      SdattackdsrRouteCacheEntry successEntry = rtVector.front ();
       ...
       */
      std::pair<std::map<Ipv4Address, std::list<SdattackdsrRouteCacheEntry> >::iterator, bool> result =
...
bool
SdattackdsrRouteCache::LookupRoute (Ipv4Address id, SdattackdsrRouteCacheEntry & rt)
{
  ...
  if (IsLinkCache ())
  ...
  else
    {
      ...
      std::map<Ipv4Address, std::list<SdattackdsrRouteCacheEntry> >::const_iterator i = m_sortedRoutes.find (id);
      if (i == m_sortedRoutes.end ())
        {
          NS_LOG_LOGIC ("No Direct Route to " << id << " found");
          for (std::map<Ipv4Address, std::list<SdattackdsrRouteCacheEntry> >::const_iterator j =
                 m_sortedRoutes.begin (); j != m_sortedRoutes.end (); ++j)
            {
              std::list<SdattackdsrRouteCacheEntry> rtVector = j->second; // The route cache vector linked with destination address
              /*
               * Loop through the possibly multiple routes within the route vector
               */
              for (std::list<SdattackdsrRouteCacheEntry>::const_iterator k = rtVector.begin (); k != rtVector.end (); ++k)
                {
                  // return the first route in the route vector
                  SdattackdsrRouteCacheEntry::IP_VECTOR routeVector = k->GetVector ();
                  SdattackdsrRouteCacheEntry::IP_VECTOR changeVector;

                  for (SdattackdsrRouteCacheEntry::IP_VECTOR::iterator l = routeVector.begin (); l != routeVector.end (); ++l)
                    {
                      if (*l != id)
                        {
                  ...
                  if ((changeVector.size () < routeVector.size ())  && (changeVector.size () > 1))
                    {
                      SdattackdsrRouteCacheEntry changeEntry; // Create the route entry
                      ...
                      std::list<SdattackdsrRouteCacheEntry> newVector;
      ...
      std::map<Ipv4Address, std::list<SdattackdsrRouteCacheEntry> >::const_iterator m = m_sortedRoutes.find (id);
       ...
       */
      std::list<SdattackdsrRouteCacheEntry> rtVector = m->second;
...
void
SdattackdsrRouteCache::SetCacheType (std::string type)
{
...
bool
SdattackdsrRouteCache::IsLinkCache ()
{
  NS_LOG_FUNCTION (this);
...
void
SdattackdsrRouteCache::RebuildBestRouteTable (Ipv4Address source)
{
          ...
          for (std::map<Ipv4Address, uint32_t>::const_iterator k = m_netGraph[tempip].begin (); k != m_netGraph[tempip].end (); ++k)
            {
               ...
               */
                ...
                {
                  std::map<Link, SdattackdsrLinkStab>::iterator oldlink = m_linkCache.find (Link (k->first, pre[k->first]));
                  std::map<Link, SdattackdsrLinkStab>::iterator newlink = m_linkCache.find (Link (k->first, tempip));
  ...
  for (std::map<Ipv4Address, Ipv4Address>::iterator i = pre.begin (); i != pre.end (); ++i)
    {
      ...
      SdattackdsrRouteCacheEntry::IP_VECTOR route;
      ...
      if (!i->second.IsBroadcast () && iptemp != source)
        {
          ...
          // Reverse the route
          SdattackdsrRouteCacheEntry::IP_VECTOR reverseroute;
          for (SdattackdsrRouteCacheEntry::IP_VECTOR::reverse_iterator j = route.rbegin (); j != route.rend (); ++j)
            {
...
bool
SdattackdsrRouteCache::LookupRoute_Link (Ipv4Address id, SdattackdsrRouteCacheEntry & rt)
{
  NS_LOG_FUNCTION (this << id);
  /// We need to purge the link node cache
  PurgeLinkNode ();
  std::map<Ipv4Address, SdattackdsrRouteCacheEntry::IP_VECTOR>::const_iterator i = m_bestRoutesTable_link.find (id);
  if (i == m_bestRoutesTable_link.end ())
  ...
  else
    {
      ...
      SdattackdsrRouteCacheEntry newEntry; // Create the route entry
...
void
SdattackdsrRouteCache::PurgeLinkNode ()
{
  NS_LOG_FUNCTION (this);
  for (std::map<Link, SdattackdsrLinkStab>::iterator i = m_linkCache.begin (); i != m_linkCache.end (); )
    {
      NS_LOG_DEBUG ("The link stability " << i->second.GetLinkStability ().GetSeconds ());
      std::map<Link, SdattackdsrLinkStab>::iterator itmp = i;
  ...
  for (std::map<Ipv4Address, SdattackdsrNodeStab>::iterator i = m_nodeCache.begin (); i != m_nodeCache.end (); )
    {
      NS_LOG_DEBUG ("The node stability " << i->second.GetNodeStability ().GetSeconds ());
      std::map<Ipv4Address, SdattackdsrNodeStab>::iterator itmp = i;
...
void
SdattackdsrRouteCache::UpdateNetGraph ()
{
  NS_LOG_FUNCTION (this);
  m_netGraph.clear ();
  for (std::map<Link, SdattackdsrLinkStab>::iterator i = m_linkCache.begin (); i != m_linkCache.end (); ++i)
    {
...
bool
SdattackdsrRouteCache::IncStability (Ipv4Address node)
{
  NS_LOG_FUNCTION (this << node);
  std::map<Ipv4Address, SdattackdsrNodeStab>::const_iterator i = m_nodeCache.find (node);
  if (i == m_nodeCache.end ())
    {
      NS_LOG_INFO ("The initial stability " << m_initStability.GetSeconds ());
      SdattackdsrNodeStab ns (m_initStability);
      m_nodeCache[node] = ns;
      return false;
    }
  else
    {
      ...
      SdattackdsrNodeStab ns (Time (i->second.GetNodeStability () * m_stabilityIncrFactor));
      m_nodeCache[node] = ns;
      return true;
    }
  return false;
...
bool
SdattackdsrRouteCache::DecStability (Ipv4Address node)
{
  NS_LOG_FUNCTION (this << node);
  std::map<Ipv4Address, SdattackdsrNodeStab>::const_iterator i = m_nodeCache.find (node);
  if (i == m_nodeCache.end ())
  ...
  else
    {
      ...
      SdattackdsrNodeStab ns (Time (i->second.GetNodeStability () / m_stabilityDecrFactor));
      m_nodeCache[node] = ns;
      return true;
    }
  return false;
...
bool
SdattackdsrRouteCache::AddRoute_Link (SdattackdsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address source)
{
  NS_LOG_FUNCTION (this << source);
  ...
  for (uint32_t i = 0; i < nodelist.size () - 1; i++)
    {
      SdattackdsrNodeStab ns;                /// This is the node stability
      ...
      SdattackdsrLinkStab stab;                /// Link stability
...
void
SdattackdsrRouteCache::UseExtends (SdattackdsrRouteCacheEntry::IP_VECTOR rt)
{
  NS_LOG_FUNCTION (this);
  /// Purge the link node cache first
  PurgeLinkNode ();
  ...
  for (SdattackdsrRouteCacheEntry::IP_VECTOR::iterator i = rt.begin (); i != rt.end () - 1; ++i)
    {
      Link link (*i, *(i + 1));
  ...
  for (SdattackdsrRouteCacheEntry::IP_VECTOR::iterator i = rt.begin (); i != rt.end (); ++i)
    {
...
bool
SdattackdsrRouteCache::AddRoute (SdattackdsrRouteCacheEntry & rt)
{
  NS_LOG_FUNCTION (this);
  Purge ();
  std::list<SdattackdsrRouteCacheEntry> rtVector;   // Declare the route cache entry vector
...
bool SdattackdsrRouteCache::FindSameRoute (SdattackdsrRouteCacheEntry & rt, std::list<SdattackdsrRouteCacheEntry> & rtVector)
{
  NS_LOG_FUNCTION (this);
  for (std::list<SdattackdsrRouteCacheEntry>::iterator i = rtVector.begin (); i != rtVector.end (); ++i)
    {
      // return the first route in the route vector
      SdattackdsrRouteCacheEntry::IP_VECTOR routeVector = i->GetVector ();
      SdattackdsrRouteCacheEntry::IP_VECTOR newVector = rt.GetVector ();
...
bool
SdattackdsrRouteCache::DeleteRoute (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
...
void
SdattackdsrRouteCache::DeleteAllRoutesIncludeLink (Ipv4Address errorSrc, Ipv4Address unreachNode, Ipv4Address node)
{
  ...
  if (IsLinkCache ())
    {
      ...
      std::map<Ipv4Address, SdattackdsrNodeStab>::iterator i = m_nodeCache.find (errorSrc);
      if (i == m_nodeCache.end ())
        {
  ...
  else
    {
       ...
       */
      for (std::map<Ipv4Address, std::list<SdattackdsrRouteCacheEntry> >::iterator j =
             m_sortedRoutes.begin (); j != m_sortedRoutes.end (); )
        {
          std::map<Ipv4Address, std::list<SdattackdsrRouteCacheEntry> >::iterator jtmp = j;
          Ipv4Address address = j->first;
          std::list<SdattackdsrRouteCacheEntry> rtVector = j->second;
          /*
           * Loop all the routes for a single destination
           */
          for (std::list<SdattackdsrRouteCacheEntry>::iterator k = rtVector.begin (); k != rtVector.end (); )
            {
              // return the first route in the route vector
              SdattackdsrRouteCacheEntry::IP_VECTOR routeVector = k->GetVector ();
              SdattackdsrRouteCacheEntry::IP_VECTOR changeVector;
               ...
               */
              for (SdattackdsrRouteCacheEntry::IP_VECTOR::iterator i = routeVector.begin (); i != routeVector.end (); ++i)
                {
                  if (*i != errorSrc)
                    {
                  ...
                  if (m_subRoute)
                    {
                       ...
                       */
                      k = rtVector.erase (k);
                      SdattackdsrRouteCacheEntry changeEntry;
                      changeEntry.SetVector (changeVector);
...
void
SdattackdsrRouteCache::PrintVector (std::vector<Ipv4Address>& vec)
{
  NS_LOG_FUNCTION (this);
  /*
...
void
SdattackdsrRouteCache::PrintRouteVector (std::list<SdattackdsrRouteCacheEntry> route)
{
  NS_LOG_FUNCTION (this);
  for (std::list<SdattackdsrRouteCacheEntry>::iterator i = route.begin (); i != route.end (); i++)
    {
...
SdattackdsrRouteCache::Purge ()
{
  ...
  for (std::map<Ipv4Address, std::list<SdattackdsrRouteCacheEntry> >::iterator i =
         m_sortedRoutes.begin (); i != m_sortedRoutes.end (); )
    {
      // Loop of route cache entry with the route size
      std::map<Ipv4Address, std::list<SdattackdsrRouteCacheEntry> >::iterator itmp = i;
      /*
      ...
      if (rtVector.size ())
        {
          for (std::list<SdattackdsrRouteCacheEntry>::iterator j = rtVector.begin (); j != rtVector.end (); )
            {
...
void
SdattackdsrRouteCache::Print (std::ostream &os)
{
  NS_LOG_FUNCTION (this);
  Purge ();
  os << "\nDSR Route Cache\n"
     << "Destination\tGateway\t\tInterface\tFlag\tExpire\tHops\n";
  for (std::list<SdattackdsrRouteCacheEntry>::const_iterator i =
 ...
 */
uint16_t
SdattackdsrRouteCache::CheckUniqueAckId (Ipv4Address nextHop)
{
  NS_LOG_FUNCTION (this);
...
uint16_t
SdattackdsrRouteCache::GetAckSize ()
{
 ...
 */
bool
SdattackdsrRouteCache::IsNeighbor (Ipv4Address addr)
{
  NS_LOG_FUNCTION (this);
...
Time
SdattackdsrRouteCache::GetExpireTime (Ipv4Address addr)
{
  NS_LOG_FUNCTION (this);
  PurgeMac ();
...
void
SdattackdsrRouteCache::UpdateNeighbor (std::vector<Ipv4Address> nodeList, Time expire)
{
  NS_LOG_FUNCTION (this);
...
void
SdattackdsrRouteCache::AddNeighbor (std::vector<Ipv4Address> nodeList, Ipv4Address ownAddress, Time expire)
{
...
struct CloseNeighbor
{
   ...
   *
   * \param nb SdattackdsrRouteCache::Neighbor entry
   * \return true if expired or closed, false otherwise
   */
  bool operator() (const SdattackdsrRouteCache::Neighbor & nb) const
  {
...
void
SdattackdsrRouteCache::PurgeMac ()
{
  if (m_nb.empty ())
    {
      return;
...
void
SdattackdsrRouteCache::ScheduleTimer ()
{
  m_ntimer.Cancel ();
  m_ntimer.Schedule ();
}
...
void
SdattackdsrRouteCache::AddArpCache (Ptr<ArpCache> a)
{
  m_arp.push_back (a);
}

void
SdattackdsrRouteCache::DelArpCache (Ptr<ArpCache> a)
{
  m_arp.erase (std::remove (m_arp.begin (), m_arp.end (), a), m_arp.end ());
}

Mac48Address
SdattackdsrRouteCache::LookupMacAddress (Ipv4Address addr)
{
  Mac48Address hwaddr;
...
void
SdattackdsrRouteCache::ProcessTxError (WifiMacHeader const & hdr)
{

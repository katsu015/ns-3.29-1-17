...
NS_LOG_COMPONENT_DEFINE ("SdattackdsrRreqTable");
  
namespace sdattackdsr {

NS_OBJECT_ENSURE_REGISTERED (SdattackdsrRreqTable);

TypeId SdattackdsrRreqTable::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::sdattackdsr::SdattackdsrRreqTable")
    .SetParent<Object> ()
    .SetGroupName ("Sdattackdsr")
    .AddConstructor<SdattackdsrRreqTable> ()
  ;
  return tid;
}

SdattackdsrRreqTable::SdattackdsrRreqTable ()
...
void
SdattackdsrRreqTable::RemoveLeastExpire ()
{
...
void
SdattackdsrRreqTable::FindAndUpdate (Ipv4Address dst)
{
...
void
SdattackdsrRreqTable::RemoveRreqEntry (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
  std::map<Ipv4Address, RreqTableEntry >::const_iterator i =
    m_rreqDstMap.find (dst);
...
uint32_t
SdattackdsrRreqTable::GetRreqCnt (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
  std::map<Ipv4Address, RreqTableEntry >::const_iterator i =
    m_rreqDstMap.find (dst);
 ...
 */
uint32_t
SdattackdsrRreqTable::CheckUniqueRreqId (Ipv4Address dst)
{
...
uint32_t
SdattackdsrRreqTable::GetRreqSize ()
{
...
void
SdattackdsrRreqTable::Invalidate ()
{
...
BlackList*
SdattackdsrRreqTable::FindUnidirectional (Ipv4Address neighbor)
{
...
bool
SdattackdsrRreqTable::MarkLinkAsUnidirectional (Ipv4Address neighbor, Time blacklistTimeout)
{
...
void
SdattackdsrRreqTable::PurgeNeighbor ()
{
  /*
...
bool
SdattackdsrRreqTable::FindSourceEntry (Ipv4Address src, Ipv4Address dst, uint16_t id)
{
  NS_LOG_FUNCTION (this << src << dst << id);
  SdattackdsrReceivedRreqEntry rreqEntry;
  rreqEntry.SetDestination (dst);
  rreqEntry.SetIdentification (id);
  std::list<SdattackdsrReceivedRreqEntry> receivedRreqEntryList;
  /*
   * this function will return false if the entry is not found, true if duplicate entry find
   */
  std::map<Ipv4Address, std::list<SdattackdsrReceivedRreqEntry> >::const_iterator i = m_sourceRreqMap.find (src);
  if (i == m_sourceRreqMap.end ())
  ...
  else
    {
      ...
      for (std::list<SdattackdsrReceivedRreqEntry>::const_iterator j = receivedRreqEntryList.begin (); j != receivedRreqEntryList.end (); ++j)
        {

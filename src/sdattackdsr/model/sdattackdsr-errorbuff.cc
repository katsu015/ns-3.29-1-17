...
NS_LOG_COMPONENT_DEFINE ("SdattackdsrErrorBuffer");

namespace sdattackdsr {

uint32_t
SdattackdsrErrorBuffer::GetSize ()
{
  Purge ();
...
bool
SdattackdsrErrorBuffer::Enqueue (SdattackdsrErrorBuffEntry & entry)
{
  Purge ();
  for (std::vector<SdattackdsrErrorBuffEntry>::const_iterator i = m_errorBuffer.begin (); i
       != m_errorBuffer.end (); ++i)
    {
...
void
SdattackdsrErrorBuffer::DropPacketForErrLink (Ipv4Address source, Ipv4Address nextHop)
{
   ...
   */
  for (std::vector<SdattackdsrErrorBuffEntry>::iterator i = m_errorBuffer.begin (); i
       != m_errorBuffer.end (); ++i)
    {
      if (LinkEqual (*i, link))
        {
  ...
  m_errorBuffer.erase (std::remove_if (m_errorBuffer.begin (), m_errorBuffer.end (),
                                       std::bind2nd (std::ptr_fun (SdattackdsrErrorBuffer::LinkEqual), link)), m_errorBuffer.end ());
}

bool
SdattackdsrErrorBuffer::Dequeue (Ipv4Address dst, SdattackdsrErrorBuffEntry & entry)
{
  Purge ();
  /*
   * Dequeue the entry with destination address dst
   */
  for (std::vector<SdattackdsrErrorBuffEntry>::iterator i = m_errorBuffer.begin (); i != m_errorBuffer.end (); ++i)
    {
...
bool
SdattackdsrErrorBuffer::Find (Ipv4Address dst)
{
  /*
...
struct IsExpired
{
   ...
   */
  bool
  operator() (SdattackdsrErrorBuffEntry const & e) const
  {
...
void
SdattackdsrErrorBuffer::Drop (SdattackdsrErrorBuffEntry en, std::string reason)
{
...
void
SdattackdsrErrorBuffer::DropLink (SdattackdsrErrorBuffEntry en, std::string reason)
{

 ...
 */
class SdattackdsrNetworkQueueEntry
{
public:
  /**
   * Construct a SdattackdsrNetworkQueueEntry with the given parameters
   *
   * \param pa packet
   ...
   * \param r Route
   */
  SdattackdsrNetworkQueueEntry (Ptr<const Packet> pa = 0, Ipv4Address s = Ipv4Address (), Ipv4Address n = Ipv4Address (),
   ...
   */
  bool operator== (SdattackdsrNetworkQueueEntry const & o) const
  {
...
class SdattackdsrNetworkQueue : public Object
{
public:
  /**
  ...
  SdattackdsrNetworkQueue ();
  /**
   * Construct a SdattackdsrNetworkQueue with the given
   * maximum length and maximum delay.
   *
   * \param maxLen Maximum queue size
   * \param maxDelay Maximum entry lifetime in the queue
   */
  SdattackdsrNetworkQueue (uint32_t maxLen, Time maxDelay);
  ~SdattackdsrNetworkQueue ();
   ...
   */
  bool FindPacketWithNexthop (Ipv4Address nextHop, SdattackdsrNetworkQueueEntry & entry);
  /**
   ...
   *         false otherwise
   */
  bool Enqueue (SdattackdsrNetworkQueueEntry & entry);
  /**
   ...
   *         false otherwise
   */
  bool Dequeue (SdattackdsrNetworkQueueEntry & entry);
  /**
   * Number of entries
   *
   ...
   * \return the current queue entry
   */
  std::vector<SdattackdsrNetworkQueueEntry> & GetQueue ()
  {
    return m_sdattackdsrNetworkQueue;
  }
...
private:
   ...
   */
  void Cleanup (void);
  std::vector<SdattackdsrNetworkQueueEntry> m_sdattackdsrNetworkQueue; //!< Queue (vector) of entries

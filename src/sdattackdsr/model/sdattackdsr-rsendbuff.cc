...
NS_LOG_COMPONENT_DEFINE ("SdattackdsrSendBuffer");

namespace sdattackdsr {

uint32_t
SdattackdsrSendBuffer::GetSize ()
{
  Purge ();
...
bool
SdattackdsrSendBuffer::Enqueue (SdattackdsrSendBuffEntry & entry)
{
  Purge ();
  for (std::vector<SdattackdsrSendBuffEntry>::const_iterator i = m_sendBuffer.begin (); i
       != m_sendBuffer.end (); ++i)
    {
...
void
SdattackdsrSendBuffer::DropPacketWithDst (Ipv4Address dst)
{
   ...
   */
  for (std::vector<SdattackdsrSendBuffEntry>::iterator i = m_sendBuffer.begin (); i
       != m_sendBuffer.end (); ++i)
    {
      if (IsEqual (*i, dst))
        {
  ...
  m_sendBuffer.erase (std::remove_if (m_sendBuffer.begin (), m_sendBuffer.end (),
                                      std::bind2nd (std::ptr_fun (SdattackdsrSendBuffer::IsEqual), dst)), m_sendBuffer.end ());
}

bool
SdattackdsrSendBuffer::Dequeue (Ipv4Address dst, SdattackdsrSendBuffEntry & entry)
{
  Purge ();
  /*
   * Dequeue the entry with destination address dst
   */
  for (std::vector<SdattackdsrSendBuffEntry>::iterator i = m_sendBuffer.begin (); i != m_sendBuffer.end (); ++i)
    {
...
bool
SdattackdsrSendBuffer::Find (Ipv4Address dst)
{
  /*
...
struct IsExpired
{
   ...
   */
  bool
  operator() (SdattackdsrSendBuffEntry const & e) const
  {
...
void
SdattackdsrSendBuffer::Drop (SdattackdsrSendBuffEntry en, std::string reason)
{

 ...
 */
class SdattackdsrReceivedRreqEntry
{
public:
  /**
   * Construct a SdattackdsrReceivedRreqEntry with the given parameters
   *
   * \param d IPv4 address of the destination
   * \param i identification
   */
  SdattackdsrReceivedRreqEntry (Ipv4Address d = Ipv4Address (), uint16_t i = 0)
    : m_destination (d),
      m_identification (i)
  {
  }
   ...
   * \param o another SdattackdsrReceivedRreqEntry
   * \return true if equal
   */
  bool operator== (SdattackdsrReceivedRreqEntry const & o) const
  {
...
/**
 * \ingroup sdattackdsr
 * \brief maintain list of SdattackdsrRreqTable entry
 */
class SdattackdsrRreqTable  : public Object
{
public:
  /**
  ...
  SdattackdsrRreqTable ();
  virtual ~SdattackdsrRreqTable ();
...
private:
  ...
  /// Map of entries
  std::list<SdattackdsrReceivedRreqEntry> m_sourceRequests;
  ...
  std::map<Ipv4Address, std::list<SdattackdsrReceivedRreqEntry> > m_sourceRreqMap;

...
NS_LOG_COMPONENT_DEFINE ("SdattackdsrPassiveBuffer");

namespace sdattackdsr {

NS_OBJECT_ENSURE_REGISTERED (SdattackdsrPassiveBuffer);

TypeId SdattackdsrPassiveBuffer::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::sdattackdsr::SdattackdsrPassiveBuffer")
    .SetParent<Object> ()
    .SetGroupName ("Sdattackdsr")
    .AddConstructor<SdattackdsrPassiveBuffer> ()
  ;
  return tid;
}

SdattackdsrPassiveBuffer::SdattackdsrPassiveBuffer ()
{
}
...
uint32_t
SdattackdsrPassiveBuffer::GetSize ()
{
  Purge ();
...
bool
SdattackdsrPassiveBuffer::Enqueue (SdattackdsrPassiveBuffEntry & entry)
{
  Purge ();
  for (std::vector<SdattackdsrPassiveBuffEntry>::const_iterator i = m_passiveBuffer.begin (); i
...
bool
SdattackdsrPassiveBuffer::AllEqual (SdattackdsrPassiveBuffEntry & entry)
{
  for (std::vector<SdattackdsrPassiveBuffEntry>::iterator i = m_passiveBuffer.begin (); i
       != m_passiveBuffer.end (); ++i)
    {
//      NS_LOG_INFO ("packet id " << i->GetPacket ()->GetUid () << " " << entry.GetPacket ()->GetUid () << " source " << i->GetSource () << " " << entry.GetSource ()
//                                     << " dst " << i->GetDestination () << " " << entry.GetDestination () << " identification " << i->GetIdentification () << " "
...
bool
SdattackdsrPassiveBuffer::Dequeue (Ipv4Address dst, SdattackdsrPassiveBuffEntry & entry)
{
  Purge ();
  /*
   * Dequeue the entry with destination address dst
   */
  for (std::vector<SdattackdsrPassiveBuffEntry>::iterator i = m_passiveBuffer.begin (); i != m_passiveBuffer.end (); ++i)
    {
...
bool
SdattackdsrPassiveBuffer::Find (Ipv4Address dst)
{
  /*
...
struct IsExpired
{
   ...
   */
  bool
  operator() (SdattackdsrPassiveBuffEntry const & e) const
  {
...
void
SdattackdsrPassiveBuffer::Purge ()
{
  /*
...
void
SdattackdsrPassiveBuffer::Drop (SdattackdsrPassiveBuffEntry en, std::string reason)
{
...
void
SdattackdsrPassiveBuffer::DropLink (SdattackdsrPassiveBuffEntry en, std::string reason)
{

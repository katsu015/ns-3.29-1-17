...
/**
 * \class SdattackdsrLinkStab
 * \brief SdattackdsrLinkStab class (DSR link stability)
 */
class SdattackdsrLinkStab
{
public:
   ...
   */
  SdattackdsrLinkStab (Time linkStab = Simulator::Now ());
  /**
   * \brief Destructor
   */
  virtual ~SdattackdsrLinkStab ();
...
/**
 * \class SdattackdsrNodeStab
 * \brief SdattackdsrNodeStab class (DSR node stability)
 */
class SdattackdsrNodeStab
{
public:
   ...
   */
  SdattackdsrNodeStab (Time nodeStab = Simulator::Now ());
  virtual ~SdattackdsrNodeStab ();
...
/**
 * \class SdattackdsrRouteCacheEntry
 * \brief SdattackdsrRouteCacheEntry class for entries in the route cache
 */
class SdattackdsrRouteCacheEntry
{
public:
   ...
   */
  SdattackdsrRouteCacheEntry (IP_VECTOR const  & ip = IP_VECTOR (), Ipv4Address dst = Ipv4Address (), Time exp = Simulator::Now ());
  virtual ~SdattackdsrRouteCacheEntry ();
   ...
   */
  bool operator== (SdattackdsrRouteCacheEntry const & o) const
  {
 ...
 */
class SdattackdsrRouteCache : public Object
{
public:
  /**
  ...
  SdattackdsrRouteCache ();
  virtual ~SdattackdsrRouteCache ();

  /**
   * \brief Remove the aged route cache entries when the route cache is full
   * \param rtVector the route cache to scan.
   */
  void RemoveLastEntry (std::list<SdattackdsrRouteCacheEntry> & rtVector);
  /**
   * \brief Define the vector of route entries.
   */
  typedef std::list<SdattackdsrRouteCacheEntry::IP_VECTOR> routeVector;
   ...
   */
  void PrintRouteVector (std::list<SdattackdsrRouteCacheEntry> route);
  /**
   ...
   * \return true if same
   */
  bool FindSameRoute (SdattackdsrRouteCacheEntry & rt, std::list<SdattackdsrRouteCacheEntry> & rtVector);
  /**
...
private:
   ...
   * \return
   */
  SdattackdsrRouteCache & operator= (SdattackdsrRouteCache const &);
  SdattackdsrRouteCacheEntry::IP_VECTOR m_vector;               ///< The route vector to save the ip addresses for intermediate nodes.
  uint32_t m_maxCacheLen;                               ///< The maximum number of packets that we allow a routing protocol to buffer.
  Time     RouteCacheTimeout;                           ///< The maximum period of time that sdattackdsr is allowed to for an unused route.
   ...
   */
  typedef std::list<SdattackdsrRouteCacheEntry> routeEntryVector;
  ...
  std::map<Ipv4Address, SdattackdsrRouteCacheEntry::IP_VECTOR> m_bestRoutesTable_link;     ///< for link route cache
  std::map<Link, SdattackdsrLinkStab> m_linkCache;                                         ///< The data structure to store link info
  std::map<Ipv4Address, SdattackdsrNodeStab> m_nodeCache;                                  ///< The data structure to store node info
  /**
   ...
   */
  bool LookupRoute_Link (Ipv4Address id, SdattackdsrRouteCacheEntry & rt);
  /**
...
public:
   ...
   * \return true if the link is cached
   */
  bool AddRoute_Link (SdattackdsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address node);
  /**

 ...
 */
class SdattackdsrSendBuffEntry
{
public:
  /**
   * Construct SdattackdsrSendBuffEntry with the given parameters.
   *
   * \param pa packet
   ...
   */
  SdattackdsrSendBuffEntry (Ptr<const Packet> pa = 0, Ipv4Address d = Ipv4Address (),
   ...
   * \param o another SdattackdsrSendBuffEntry
   * \return true if equal
   */
  bool operator== (SdattackdsrSendBuffEntry const & o) const
  {
 ...
 */
/************************************************************************************************************************/
class SdattackdsrSendBuffer
{
public:
  /**
   * Default constructor
   */
  SdattackdsrSendBuffer ()
  {
  }
  /**
   ...
   *
   * \param entry SdattackdsrSendBuffEntry to put in the queue
   * \return true if successfully enqueued,
   *         false otherwise
   */
  bool Enqueue (SdattackdsrSendBuffEntry & entry);
  /**
   ...
   *         false otherwise
   */
  bool Dequeue (Ipv4Address dst, SdattackdsrSendBuffEntry & entry);
  /**
   ...
   * \return a pointer to the internal queue
   */
  std::vector<SdattackdsrSendBuffEntry> & GetBuffer ()
  {
    return m_sendBuffer;
  }

private:
  std::vector<SdattackdsrSendBuffEntry> m_sendBuffer;                   ///< The send buffer to cache unsent packet
  void Purge ();                                                ///< Remove all expired entries

  /// Notify that packet is dropped from queue by timeout
  /// \param en BuffEntry Buffer entry
  /// \param reason Drop reason
  void Drop (SdattackdsrSendBuffEntry en, std::string reason);
   ...
   *         false otherwise
   */
  static bool IsEqual (SdattackdsrSendBuffEntry en, const Ipv4Address dst)
  {

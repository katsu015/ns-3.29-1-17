...
NS_LOG_COMPONENT_DEFINE ("RushattackdsrMaintainBuffer");

namespace rushattackdsr {

uint32_t
RushattackdsrMaintainBuffer::GetSize ()
{
  Purge ();
...
bool
RushattackdsrMaintainBuffer::Enqueue (RushattackdsrMaintainBuffEntry & entry)
{
  Purge ();
  for (std::vector<RushattackdsrMaintainBuffEntry>::const_iterator i = m_maintainBuffer.begin (); i
...
void
RushattackdsrMaintainBuffer::DropPacketWithNextHop (Ipv4Address nextHop)
{
  ...
  m_maintainBuffer.erase (std::remove_if (m_maintainBuffer.begin (), m_maintainBuffer.end (),
                                          std::bind2nd (std::ptr_fun (RushattackdsrMaintainBuffer::IsEqual), nextHop)), m_maintainBuffer.end ());
}

bool
RushattackdsrMaintainBuffer::Dequeue (Ipv4Address nextHop, RushattackdsrMaintainBuffEntry & entry)
{
  Purge ();
  for (std::vector<RushattackdsrMaintainBuffEntry>::iterator i = m_maintainBuffer.begin (); i != m_maintainBuffer.end (); ++i)
    {
...
bool
RushattackdsrMaintainBuffer::Find (Ipv4Address nextHop)
{
  for (std::vector<RushattackdsrMaintainBuffEntry>::const_iterator i = m_maintainBuffer.begin (); i
       != m_maintainBuffer.end (); ++i)
    {
...
bool
RushattackdsrMaintainBuffer::AllEqual (RushattackdsrMaintainBuffEntry & entry)
{
  for (std::vector<RushattackdsrMaintainBuffEntry>::iterator i = m_maintainBuffer.begin (); i
       != m_maintainBuffer.end (); ++i)
    {
...
bool
RushattackdsrMaintainBuffer::NetworkEqual (RushattackdsrMaintainBuffEntry & entry)
{
  for (std::vector<RushattackdsrMaintainBuffEntry>::iterator i = m_maintainBuffer.begin (); i
       != m_maintainBuffer.end (); ++i)
    {
...
bool
RushattackdsrMaintainBuffer::PromiscEqual (RushattackdsrMaintainBuffEntry & entry)
{
...
bool
RushattackdsrMaintainBuffer::LinkEqual (RushattackdsrMaintainBuffEntry & entry)
{
  NS_LOG_DEBUG ("The maintenance buffer size " << m_maintainBuffer.size ());
  for (std::vector<RushattackdsrMaintainBuffEntry>::iterator i = m_maintainBuffer.begin (); i
       != m_maintainBuffer.end (); ++i)
...
struct IsExpired
{
   ...
   */
  bool
  operator() (RushattackdsrMaintainBuffEntry const & e) const
  {
...
void
RushattackdsrMaintainBuffer::Purge ()
{

...
/**
 * \class RushattackdsrHeader
 * \brief Header for Rushattackdsr Routing.
 */

/**
* \ingroup rushattackdsr
* \brief Rushattackdsr fixed size header Format
  \verbatim
   |      0        |      1        |      2        |      3        |
...
/**
* \ingroup rushattackdsr
* \brief The modified version of Rushattackdsr fixed size header Format
  \verbatim
   |      0        |      1        |      2        |      3        |
   0 1 2 3 4 5 6 7 0 1 2 3 4 5 6 7 0 1 2 3 4 5 6 7 0 1 2 3 4 5 6 7
   +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  ...
  \endverbatim
*/
class RushattackdsrFsHeader : public Header
{
public:
  /**
   ...
   * \brief Constructor.
   */
  RushattackdsrFsHeader ();
  /**
   * \brief Destructor.
   */
  virtual ~RushattackdsrFsHeader ();
  /**
...
/**
 * \class RushattackdsrOptionField
 * \brief Option field for an RushattackdsrFsHeader
 * Enables adding options to an RushattackdsrFsHeader
 *
 * Implementor's note: Make sure to add the result of
 * OptionField::GetSerializedSize () to your RushattackdsrFsHeader::GetSerializedSize ()
 * return value. Call OptionField::Serialize and OptionField::Deserialize at the
 * end of your corresponding RushattackdsrFsHeader methods.
 */
class RushattackdsrOptionField
{
public:
   ...
   */
  RushattackdsrOptionField (uint32_t optionsOffset);
  /**
   * \brief Destructor.
   */
  ~RushattackdsrOptionField ();
  /**
   * \brief Get the serialized size of the packet.
   * \return size
   */
   ...
   */
  void AddRushattackdsrOption (RushattackdsrOptionHeader const& option);
  /**
   ...
   */
  uint32_t GetRushattackdsrOptionsOffset ();
  /**
   * \brief Get the buffer.
   * \return buffer
   */
  Buffer GetRushattackdsrOptionBuffer ();
...
private:
   ...
   */
  uint32_t CalculatePad (RushattackdsrOptionHeader::Alignment alignment) const;
  /**
   * \brief Data payload.
   */
  Buffer m_optionData;
...
/**
 * \class RushattackdsrRoutingHeader
 * \brief Header of Rushattackdsr Routing
 */
class RushattackdsrRoutingHeader : public RushattackdsrFsHeader,
                         public RushattackdsrOptionField
{
public:
  /**
   * \brief Get the type identificator.
   ...
   * \brief Constructor.
   */
  RushattackdsrRoutingHeader ();
  /**
   * \brief Destructor.
   */
  virtual ~RushattackdsrRoutingHeader ();
  /**
   * \brief Print some information about the packet.
   * \param os output stream
   * \return info about this packet
...
static inline std::ostream & operator<< (std::ostream& os, const RushattackdsrRoutingHeader & rushattackdsr)
{
  rushattackdsr.Print (os);
  return os;
}

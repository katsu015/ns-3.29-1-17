...
/**
 * \class RushattackdsrLinkStab
 * \brief RushattackdsrLinkStab class (DSR link stability)
 */
class RushattackdsrLinkStab
{
public:
   ...
   */
  RushattackdsrLinkStab (Time linkStab = Simulator::Now ());
  /**
   * \brief Destructor
   */
  virtual ~RushattackdsrLinkStab ();
...
/**
 * \class RushattackdsrNodeStab
 * \brief RushattackdsrNodeStab class (DSR node stability)
 */
class RushattackdsrNodeStab
{
public:
   ...
   */
  RushattackdsrNodeStab (Time nodeStab = Simulator::Now ());
  virtual ~RushattackdsrNodeStab ();
...
/**
 * \class RushattackdsrRouteCacheEntry
 * \brief RushattackdsrRouteCacheEntry class for entries in the route cache
 */
class RushattackdsrRouteCacheEntry
{
public:
   ...
   */
  RushattackdsrRouteCacheEntry (IP_VECTOR const  & ip = IP_VECTOR (), Ipv4Address dst = Ipv4Address (), Time exp = Simulator::Now ());
  virtual ~RushattackdsrRouteCacheEntry ();
   ...
   */
  bool operator== (RushattackdsrRouteCacheEntry const & o) const
  {
 ...
 */
class RushattackdsrRouteCache : public Object
{
public:
  /**
  ...
  RushattackdsrRouteCache ();
  virtual ~RushattackdsrRouteCache ();

  /**
   * \brief Remove the aged route cache entries when the route cache is full
   * \param rtVector the route cache to scan.
   */
  void RemoveLastEntry (std::list<RushattackdsrRouteCacheEntry> & rtVector);
  /**
   * \brief Define the vector of route entries.
   */
  typedef std::list<RushattackdsrRouteCacheEntry::IP_VECTOR> routeVector;
   ...
   */
  void PrintRouteVector (std::list<RushattackdsrRouteCacheEntry> route);
  /**
   ...
   * \return true if same
   */
  bool FindSameRoute (RushattackdsrRouteCacheEntry & rt, std::list<RushattackdsrRouteCacheEntry> & rtVector);
  /**
...
private:
   ...
   * \return
   */
  RushattackdsrRouteCache & operator= (RushattackdsrRouteCache const &);
  RushattackdsrRouteCacheEntry::IP_VECTOR m_vector;               ///< The route vector to save the ip addresses for intermediate nodes.
  uint32_t m_maxCacheLen;                               ///< The maximum number of packets that we allow a routing protocol to buffer.
  Time     RouteCacheTimeout;                           ///< The maximum period of time that rushattackdsr is allowed to for an unused route.
   ...
   */
  typedef std::list<RushattackdsrRouteCacheEntry> routeEntryVector;
  ...
  std::map<Ipv4Address, RushattackdsrRouteCacheEntry::IP_VECTOR> m_bestRoutesTable_link;     ///< for link route cache
  std::map<Link, RushattackdsrLinkStab> m_linkCache;                                         ///< The data structure to store link info
  std::map<Ipv4Address, RushattackdsrNodeStab> m_nodeCache;                                  ///< The data structure to store node info
  /**
   ...
   */
  bool LookupRoute_Link (Ipv4Address id, RushattackdsrRouteCacheEntry & rt);
  /**
...
public:
   ...
   * \return true if the link is cached
   */
  bool AddRoute_Link (RushattackdsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address node);
  /**

 ...
 */
class RushattackdsrNetworkQueueEntry
{
public:
  /**
   * Construct a RushattackdsrNetworkQueueEntry with the given parameters
   *
   * \param pa packet
   ...
   * \param r Route
   */
  RushattackdsrNetworkQueueEntry (Ptr<const Packet> pa = 0, Ipv4Address s = Ipv4Address (), Ipv4Address n = Ipv4Address (),
   ...
   */
  bool operator== (RushattackdsrNetworkQueueEntry const & o) const
  {
...
class RushattackdsrNetworkQueue : public Object
{
public:
  /**
  ...
  RushattackdsrNetworkQueue ();
  /**
   * Construct a RushattackdsrNetworkQueue with the given
   * maximum length and maximum delay.
   *
   * \param maxLen Maximum queue size
   * \param maxDelay Maximum entry lifetime in the queue
   */
  RushattackdsrNetworkQueue (uint32_t maxLen, Time maxDelay);
  ~RushattackdsrNetworkQueue ();
   ...
   */
  bool FindPacketWithNexthop (Ipv4Address nextHop, RushattackdsrNetworkQueueEntry & entry);
  /**
   ...
   *         false otherwise
   */
  bool Enqueue (RushattackdsrNetworkQueueEntry & entry);
  /**
   ...
   *         false otherwise
   */
  bool Dequeue (RushattackdsrNetworkQueueEntry & entry);
  /**
   * Number of entries
   *
   ...
   * \return the current queue entry
   */
  std::vector<RushattackdsrNetworkQueueEntry> & GetQueue ()
  {
    return m_rushattackdsrNetworkQueue;
  }
...
private:
   ...
   */
  void Cleanup (void);
  std::vector<RushattackdsrNetworkQueueEntry> m_rushattackdsrNetworkQueue; //!< Queue (vector) of entries

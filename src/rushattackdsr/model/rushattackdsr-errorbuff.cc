...
NS_LOG_COMPONENT_DEFINE ("RushattackdsrErrorBuffer");

namespace rushattackdsr {

uint32_t
RushattackdsrErrorBuffer::GetSize ()
{
  Purge ();
...
bool
RushattackdsrErrorBuffer::Enqueue (RushattackdsrErrorBuffEntry & entry)
{
  Purge ();
  for (std::vector<RushattackdsrErrorBuffEntry>::const_iterator i = m_errorBuffer.begin (); i
       != m_errorBuffer.end (); ++i)
    {
...
void
RushattackdsrErrorBuffer::DropPacketForErrLink (Ipv4Address source, Ipv4Address nextHop)
{
   ...
   */
  for (std::vector<RushattackdsrErrorBuffEntry>::iterator i = m_errorBuffer.begin (); i
       != m_errorBuffer.end (); ++i)
    {
      if (LinkEqual (*i, link))
        {
  ...
  m_errorBuffer.erase (std::remove_if (m_errorBuffer.begin (), m_errorBuffer.end (),
                                       std::bind2nd (std::ptr_fun (RushattackdsrErrorBuffer::LinkEqual), link)), m_errorBuffer.end ());
}

bool
RushattackdsrErrorBuffer::Dequeue (Ipv4Address dst, RushattackdsrErrorBuffEntry & entry)
{
  Purge ();
  /*
   * Dequeue the entry with destination address dst
   */
  for (std::vector<RushattackdsrErrorBuffEntry>::iterator i = m_errorBuffer.begin (); i != m_errorBuffer.end (); ++i)
    {
...
bool
RushattackdsrErrorBuffer::Find (Ipv4Address dst)
{
  /*
...
struct IsExpired
{
   ...
   */
  bool
  operator() (RushattackdsrErrorBuffEntry const & e) const
  {
...
void
RushattackdsrErrorBuffer::Purge ()
{
  /*
...
void
RushattackdsrErrorBuffer::Drop (RushattackdsrErrorBuffEntry en, std::string reason)
{
...
void
RushattackdsrErrorBuffer::DropLink (RushattackdsrErrorBuffEntry en, std::string reason)
{

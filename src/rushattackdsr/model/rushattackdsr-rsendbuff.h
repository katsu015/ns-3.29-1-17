 ...
 */
class RushattackdsrSendBuffEntry
{
public:
  /**
   * Construct RushattackdsrSendBuffEntry with the given parameters.
   *
   * \param pa packet
   ...
   */
  RushattackdsrSendBuffEntry (Ptr<const Packet> pa = 0, Ipv4Address d = Ipv4Address (),
   ...
   * \param o another RushattackdsrSendBuffEntry
   * \return true if equal
   */
  bool operator== (RushattackdsrSendBuffEntry const & o) const
  {
 ...
 */
/************************************************************************************************************************/
class RushattackdsrSendBuffer
{
public:
  /**
   * Default constructor
   */
  RushattackdsrSendBuffer ()
  {
  }
  /**
   ...
   *
   * \param entry RushattackdsrSendBuffEntry to put in the queue
   * \return true if successfully enqueued,
   *         false otherwise
   */
  bool Enqueue (RushattackdsrSendBuffEntry & entry);
  /**
   ...
   *         false otherwise
   */
  bool Dequeue (Ipv4Address dst, RushattackdsrSendBuffEntry & entry);
  /**
   ...
   * \return a pointer to the internal queue
   */
  std::vector<RushattackdsrSendBuffEntry> & GetBuffer ()
  {
    return m_sendBuffer;
  }

private:
  std::vector<RushattackdsrSendBuffEntry> m_sendBuffer;                   ///< The send buffer to cache unsent packet
  void Purge ();                                                ///< Remove all expired entries

  /// Notify that packet is dropped from queue by timeout
  /// \param en BuffEntry Buffer entry
  /// \param reason Drop reason
  void Drop (RushattackdsrSendBuffEntry en, std::string reason);
   ...
   *         false otherwise
   */
  static bool IsEqual (RushattackdsrSendBuffEntry en, const Ipv4Address dst)
  {

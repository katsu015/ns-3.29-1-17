...
NS_LOG_COMPONENT_DEFINE ("RushattackdsrPassiveBuffer");

namespace rushattackdsr {

NS_OBJECT_ENSURE_REGISTERED (RushattackdsrPassiveBuffer);

TypeId RushattackdsrPassiveBuffer::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::rushattackdsr::RushattackdsrPassiveBuffer")
    .SetParent<Object> ()
    .SetGroupName ("Rushattackdsr")
    .AddConstructor<RushattackdsrPassiveBuffer> ()
  ;
  return tid;
}

RushattackdsrPassiveBuffer::RushattackdsrPassiveBuffer ()
{
}
...
uint32_t
RushattackdsrPassiveBuffer::GetSize ()
{
  Purge ();
...
bool
RushattackdsrPassiveBuffer::Enqueue (RushattackdsrPassiveBuffEntry & entry)
{
  Purge ();
  for (std::vector<RushattackdsrPassiveBuffEntry>::const_iterator i = m_passiveBuffer.begin (); i
...
bool
RushattackdsrPassiveBuffer::AllEqual (RushattackdsrPassiveBuffEntry & entry)
{
  for (std::vector<RushattackdsrPassiveBuffEntry>::iterator i = m_passiveBuffer.begin (); i
       != m_passiveBuffer.end (); ++i)
    {
//      NS_LOG_INFO ("packet id " << i->GetPacket ()->GetUid () << " " << entry.GetPacket ()->GetUid () << " source " << i->GetSource () << " " << entry.GetSource ()
//                                     << " dst " << i->GetDestination () << " " << entry.GetDestination () << " identification " << i->GetIdentification () << " "
...
bool
RushattackdsrPassiveBuffer::Dequeue (Ipv4Address dst, RushattackdsrPassiveBuffEntry & entry)
{
  Purge ();
  /*
   * Dequeue the entry with destination address dst
   */
  for (std::vector<RushattackdsrPassiveBuffEntry>::iterator i = m_passiveBuffer.begin (); i != m_passiveBuffer.end (); ++i)
    {
...
bool
RushattackdsrPassiveBuffer::Find (Ipv4Address dst)
{
  /*
...
struct IsExpired
{
   ...
   */
  bool
  operator() (RushattackdsrPassiveBuffEntry const & e) const
  {
...
void
RushattackdsrPassiveBuffer::Purge ()
{
  /*
...
void
RushattackdsrPassiveBuffer::Drop (RushattackdsrPassiveBuffEntry en, std::string reason)
{
...
void
RushattackdsrPassiveBuffer::DropLink (RushattackdsrPassiveBuffEntry en, std::string reason)
{

...
NS_LOG_COMPONENT_DEFINE ("RushattackdsrRouteCache");

namespace rushattackdsr {

bool CompareRoutesBoth (const RushattackdsrRouteCacheEntry &a, const RushattackdsrRouteCacheEntry &b)
{
...
bool CompareRoutesHops (const RushattackdsrRouteCacheEntry &a, const RushattackdsrRouteCacheEntry &b)
{
  // compare based on hops
  return a.GetVector ().size () < b.GetVector ().size ();
}

bool CompareRoutesExpire (const RushattackdsrRouteCacheEntry &a, const RushattackdsrRouteCacheEntry &b)
{
...
RushattackdsrNodeStab::RushattackdsrNodeStab (Time nodeStab)
  : m_nodeStability (nodeStab + Simulator::Now ())
{
}

RushattackdsrNodeStab::~RushattackdsrNodeStab ()
{
}

RushattackdsrLinkStab::RushattackdsrLinkStab (Time linkStab)
  : m_linkStability (linkStab + Simulator::Now ())
{
}

RushattackdsrLinkStab::~RushattackdsrLinkStab ()
{
}

void RushattackdsrLinkStab::Print ( ) const
{
  NS_LOG_LOGIC ("LifeTime: " << GetLinkStability ().GetSeconds ());
}

typedef std::list<RushattackdsrRouteCacheEntry>::value_type route_pair;

RushattackdsrRouteCacheEntry::RushattackdsrRouteCacheEntry (IP_VECTOR const  & ip, Ipv4Address dst, Time exp)
...
RushattackdsrRouteCacheEntry::~RushattackdsrRouteCacheEntry ()
{
}

void
RushattackdsrRouteCacheEntry::Invalidate (Time badLinkLifetime)
{
  m_reqCount = 0;
...
void
RushattackdsrRouteCacheEntry::Print (std::ostream & os) const
{
  os << m_dst << "\t" << (m_expire - Simulator::Now ()).GetSeconds ()
     << "\t";
}

NS_OBJECT_ENSURE_REGISTERED (RushattackdsrRouteCache);

TypeId RushattackdsrRouteCache::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::rushattackdsr::RushattackdsrRouteCache")
    .SetParent<Object> ()
    .SetGroupName ("Rushattackdsr")
    .AddConstructor<RushattackdsrRouteCache> ()
  ;
  return tid;
}

RushattackdsrRouteCache::RushattackdsrRouteCache ()
  : m_vector (0),
    ...
    m_delay (MilliSeconds (100))
{
  ...
  m_ntimer.SetFunction (&RushattackdsrRouteCache::PurgeMac, this);
  m_txErrorCallback = MakeCallback (&RushattackdsrRouteCache::ProcessTxError, this);
}
...
void
RushattackdsrRouteCache::RemoveLastEntry (std::list<RushattackdsrRouteCacheEntry> & rtVector)
{
...
bool
RushattackdsrRouteCache::UpdateRouteEntry (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
  std::map<Ipv4Address, std::list<RushattackdsrRouteCacheEntry> >::const_iterator i =
  ...
  if (i == m_sortedRoutes.end ())
    ...
    {
      std::list<RushattackdsrRouteCacheEntry> rtVector = i->second;
      RushattackdsrRouteCacheEntry successEntry = rtVector.front ();
       ...
       */
      std::pair<std::map<Ipv4Address, std::list<RushattackdsrRouteCacheEntry> >::iterator, bool> result =
...
bool
RushattackdsrRouteCache::LookupRoute (Ipv4Address id, RushattackdsrRouteCacheEntry & rt)
{
  ...
  if (IsLinkCache ())
  ...
  else
    {
      ...
      std::map<Ipv4Address, std::list<RushattackdsrRouteCacheEntry> >::const_iterator i = m_sortedRoutes.find (id);
      if (i == m_sortedRoutes.end ())
        {
          NS_LOG_LOGIC ("No Direct Route to " << id << " found");
          for (std::map<Ipv4Address, std::list<RushattackdsrRouteCacheEntry> >::const_iterator j =
                 m_sortedRoutes.begin (); j != m_sortedRoutes.end (); ++j)
            {
              std::list<RushattackdsrRouteCacheEntry> rtVector = j->second; // The route cache vector linked with destination address
              /*
               * Loop through the possibly multiple routes within the route vector
               */
              for (std::list<RushattackdsrRouteCacheEntry>::const_iterator k = rtVector.begin (); k != rtVector.end (); ++k)
                {
                  // return the first route in the route vector
                  RushattackdsrRouteCacheEntry::IP_VECTOR routeVector = k->GetVector ();
                  RushattackdsrRouteCacheEntry::IP_VECTOR changeVector;

                  for (RushattackdsrRouteCacheEntry::IP_VECTOR::iterator l = routeVector.begin (); l != routeVector.end (); ++l)
                    {
                      if (*l != id)
                        {
                  ...
                  if ((changeVector.size () < routeVector.size ())  && (changeVector.size () > 1))
                    {
                      RushattackdsrRouteCacheEntry changeEntry; // Create the route entry
                      ...
                      std::list<RushattackdsrRouteCacheEntry> newVector;
      ...
      std::map<Ipv4Address, std::list<RushattackdsrRouteCacheEntry> >::const_iterator m = m_sortedRoutes.find (id);
       ...
       */
      std::list<RushattackdsrRouteCacheEntry> rtVector = m->second;
...
void
RushattackdsrRouteCache::SetCacheType (std::string type)
{
...
bool
RushattackdsrRouteCache::IsLinkCache ()
{
  NS_LOG_FUNCTION (this);
...
void
RushattackdsrRouteCache::RebuildBestRouteTable (Ipv4Address source)
{
          ...
          for (std::map<Ipv4Address, uint32_t>::const_iterator k = m_netGraph[tempip].begin (); k != m_netGraph[tempip].end (); ++k)
            {
               ...
               */
                ...
                {
                  std::map<Link, RushattackdsrLinkStab>::iterator oldlink = m_linkCache.find (Link (k->first, pre[k->first]));
                  std::map<Link, RushattackdsrLinkStab>::iterator newlink = m_linkCache.find (Link (k->first, tempip));
  ...
  for (std::map<Ipv4Address, Ipv4Address>::iterator i = pre.begin (); i != pre.end (); ++i)
    {
      ...
      RushattackdsrRouteCacheEntry::IP_VECTOR route;
      ...
      if (!i->second.IsBroadcast () && iptemp != source)
        {
          ...
          // Reverse the route
          RushattackdsrRouteCacheEntry::IP_VECTOR reverseroute;
          for (RushattackdsrRouteCacheEntry::IP_VECTOR::reverse_iterator j = route.rbegin (); j != route.rend (); ++j)
            {
...
bool
RushattackdsrRouteCache::LookupRoute_Link (Ipv4Address id, RushattackdsrRouteCacheEntry & rt)
{
  NS_LOG_FUNCTION (this << id);
  /// We need to purge the link node cache
  PurgeLinkNode ();
  std::map<Ipv4Address, RushattackdsrRouteCacheEntry::IP_VECTOR>::const_iterator i = m_bestRoutesTable_link.find (id);
  if (i == m_bestRoutesTable_link.end ())
  ...
  else
    {
      ...
      RushattackdsrRouteCacheEntry newEntry; // Create the route entry
...
void
RushattackdsrRouteCache::PurgeLinkNode ()
{
  NS_LOG_FUNCTION (this);
  for (std::map<Link, RushattackdsrLinkStab>::iterator i = m_linkCache.begin (); i != m_linkCache.end (); )
    {
      NS_LOG_DEBUG ("The link stability " << i->second.GetLinkStability ().GetSeconds ());
      std::map<Link, RushattackdsrLinkStab>::iterator itmp = i;
  ...
  for (std::map<Ipv4Address, RushattackdsrNodeStab>::iterator i = m_nodeCache.begin (); i != m_nodeCache.end (); )
    {
      NS_LOG_DEBUG ("The node stability " << i->second.GetNodeStability ().GetSeconds ());
      std::map<Ipv4Address, RushattackdsrNodeStab>::iterator itmp = i;
...
void
RushattackdsrRouteCache::UpdateNetGraph ()
{
  NS_LOG_FUNCTION (this);
  m_netGraph.clear ();
  for (std::map<Link, RushattackdsrLinkStab>::iterator i = m_linkCache.begin (); i != m_linkCache.end (); ++i)
    {
...
bool
RushattackdsrRouteCache::IncStability (Ipv4Address node)
{
  NS_LOG_FUNCTION (this << node);
  std::map<Ipv4Address, RushattackdsrNodeStab>::const_iterator i = m_nodeCache.find (node);
  if (i == m_nodeCache.end ())
    {
      NS_LOG_INFO ("The initial stability " << m_initStability.GetSeconds ());
      RushattackdsrNodeStab ns (m_initStability);
      m_nodeCache[node] = ns;
      return false;
    }
  else
    {
      ...
      RushattackdsrNodeStab ns (Time (i->second.GetNodeStability () * m_stabilityIncrFactor));
      m_nodeCache[node] = ns;
      return true;
    }
  return false;
...
bool
RushattackdsrRouteCache::DecStability (Ipv4Address node)
{
  NS_LOG_FUNCTION (this << node);
  std::map<Ipv4Address, RushattackdsrNodeStab>::const_iterator i = m_nodeCache.find (node);
  if (i == m_nodeCache.end ())
  ...
  else
    {
      ...
      RushattackdsrNodeStab ns (Time (i->second.GetNodeStability () / m_stabilityDecrFactor));
      m_nodeCache[node] = ns;
      return true;
    }
  return false;
...
bool
RushattackdsrRouteCache::AddRoute_Link (RushattackdsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address source)
{
  NS_LOG_FUNCTION (this << source);
  ...
  for (uint32_t i = 0; i < nodelist.size () - 1; i++)
    {
      RushattackdsrNodeStab ns;                /// This is the node stability
      ...
      RushattackdsrLinkStab stab;                /// Link stability
...
void
RushattackdsrRouteCache::UseExtends (RushattackdsrRouteCacheEntry::IP_VECTOR rt)
{
  NS_LOG_FUNCTION (this);
  /// Purge the link node cache first
  PurgeLinkNode ();
  ...
  for (RushattackdsrRouteCacheEntry::IP_VECTOR::iterator i = rt.begin (); i != rt.end () - 1; ++i)
    {
      Link link (*i, *(i + 1));
  ...
  for (RushattackdsrRouteCacheEntry::IP_VECTOR::iterator i = rt.begin (); i != rt.end (); ++i)
    {
...
bool
RushattackdsrRouteCache::AddRoute (RushattackdsrRouteCacheEntry & rt)
{
  NS_LOG_FUNCTION (this);
  Purge ();
  std::list<RushattackdsrRouteCacheEntry> rtVector;   // Declare the route cache entry vector
...
bool RushattackdsrRouteCache::FindSameRoute (RushattackdsrRouteCacheEntry & rt, std::list<RushattackdsrRouteCacheEntry> & rtVector)
{
  NS_LOG_FUNCTION (this);
  for (std::list<RushattackdsrRouteCacheEntry>::iterator i = rtVector.begin (); i != rtVector.end (); ++i)
    {
      // return the first route in the route vector
      RushattackdsrRouteCacheEntry::IP_VECTOR routeVector = i->GetVector ();
      RushattackdsrRouteCacheEntry::IP_VECTOR newVector = rt.GetVector ();
...
bool
RushattackdsrRouteCache::DeleteRoute (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
...
void
RushattackdsrRouteCache::DeleteAllRoutesIncludeLink (Ipv4Address errorSrc, Ipv4Address unreachNode, Ipv4Address node)
{
  ...
  if (IsLinkCache ())
    {
      ...
      std::map<Ipv4Address, RushattackdsrNodeStab>::iterator i = m_nodeCache.find (errorSrc);
      if (i == m_nodeCache.end ())
        {
  ...
  else
    {
       ...
       */
      for (std::map<Ipv4Address, std::list<RushattackdsrRouteCacheEntry> >::iterator j =
             m_sortedRoutes.begin (); j != m_sortedRoutes.end (); )
        {
          std::map<Ipv4Address, std::list<RushattackdsrRouteCacheEntry> >::iterator jtmp = j;
          Ipv4Address address = j->first;
          std::list<RushattackdsrRouteCacheEntry> rtVector = j->second;
          /*
           * Loop all the routes for a single destination
           */
          for (std::list<RushattackdsrRouteCacheEntry>::iterator k = rtVector.begin (); k != rtVector.end (); )
            {
              // return the first route in the route vector
              RushattackdsrRouteCacheEntry::IP_VECTOR routeVector = k->GetVector ();
              RushattackdsrRouteCacheEntry::IP_VECTOR changeVector;
               ...
               */
              for (RushattackdsrRouteCacheEntry::IP_VECTOR::iterator i = routeVector.begin (); i != routeVector.end (); ++i)
                {
                  if (*i != errorSrc)
                    {
                  ...
                  if (m_subRoute)
                    {
                       ...
                       */
                      k = rtVector.erase (k);
                      RushattackdsrRouteCacheEntry changeEntry;
                      changeEntry.SetVector (changeVector);
...
void
RushattackdsrRouteCache::PrintVector (std::vector<Ipv4Address>& vec)
{
  NS_LOG_FUNCTION (this);
  /*
...
void
RushattackdsrRouteCache::PrintRouteVector (std::list<RushattackdsrRouteCacheEntry> route)
{
  NS_LOG_FUNCTION (this);
  for (std::list<RushattackdsrRouteCacheEntry>::iterator i = route.begin (); i != route.end (); i++)
    {
...
RushattackdsrRouteCache::Purge ()
{
  ...
  for (std::map<Ipv4Address, std::list<RushattackdsrRouteCacheEntry> >::iterator i =
         m_sortedRoutes.begin (); i != m_sortedRoutes.end (); )
    {
      // Loop of route cache entry with the route size
      std::map<Ipv4Address, std::list<RushattackdsrRouteCacheEntry> >::iterator itmp = i;
      /*
      ...
      if (rtVector.size ())
        {
          for (std::list<RushattackdsrRouteCacheEntry>::iterator j = rtVector.begin (); j != rtVector.end (); )
            {
...
void
RushattackdsrRouteCache::Print (std::ostream &os)
{
  NS_LOG_FUNCTION (this);
  Purge ();
  os << "\nDSR Route Cache\n"
     << "Destination\tGateway\t\tInterface\tFlag\tExpire\tHops\n";
  for (std::list<RushattackdsrRouteCacheEntry>::const_iterator i =
 ...
 */
uint16_t
RushattackdsrRouteCache::CheckUniqueAckId (Ipv4Address nextHop)
{
  NS_LOG_FUNCTION (this);
...
uint16_t
RushattackdsrRouteCache::GetAckSize ()
{
 ...
 */
bool
RushattackdsrRouteCache::IsNeighbor (Ipv4Address addr)
{
  NS_LOG_FUNCTION (this);
...
Time
RushattackdsrRouteCache::GetExpireTime (Ipv4Address addr)
{
  NS_LOG_FUNCTION (this);
  PurgeMac ();
...
void
RushattackdsrRouteCache::UpdateNeighbor (std::vector<Ipv4Address> nodeList, Time expire)
{
  NS_LOG_FUNCTION (this);
...
void
RushattackdsrRouteCache::AddNeighbor (std::vector<Ipv4Address> nodeList, Ipv4Address ownAddress, Time expire)
{
...
struct CloseNeighbor
{
   ...
   *
   * \param nb RushattackdsrRouteCache::Neighbor entry
   * \return true if expired or closed, false otherwise
   */
  bool operator() (const RushattackdsrRouteCache::Neighbor & nb) const
  {
...
void
RushattackdsrRouteCache::PurgeMac ()
{
  if (m_nb.empty ())
    {
      return;
...
void
RushattackdsrRouteCache::ScheduleTimer ()
{
  m_ntimer.Cancel ();
  m_ntimer.Schedule ();
}
...
void
RushattackdsrRouteCache::AddArpCache (Ptr<ArpCache> a)
{
  m_arp.push_back (a);
}

void
RushattackdsrRouteCache::DelArpCache (Ptr<ArpCache> a)
{
  m_arp.erase (std::remove (m_arp.begin (), m_arp.end (), a), m_arp.end ());
}

Mac48Address
RushattackdsrRouteCache::LookupMacAddress (Ipv4Address addr)
{
  Mac48Address hwaddr;
...
void
RushattackdsrRouteCache::ProcessTxError (WifiMacHeader const & hdr)
{

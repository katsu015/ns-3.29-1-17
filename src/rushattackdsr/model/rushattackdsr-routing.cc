...
#include "rushattackdsr-rreq-table.h"
#include "rushattackdsr-rcache.h"
#include "rushattackdsr-routing.h"
#include "rushattackdsr-fs-header.h"
#include "rushattackdsr-options.h"
...
NS_LOG_COMPONENT_DEFINE ("RushattackdsrRouting");

namespace rushattackdsr {

NS_OBJECT_ENSURE_REGISTERED (RushattackdsrRouting);

/* see http://www.iana.org/assignments/protocol-numbers */
const uint8_t RushattackdsrRouting::PROT_NUMBER = 48;
/*
 * The extension header is the fixed size rushattackdsr header, it is response for recognizing DSR option types
...
TypeId RushattackdsrRouting::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::rushattackdsr::RushattackdsrRouting")
    .SetParent<IpL4Protocol> ()
    .SetGroupName ("Rushattackdsr")
    .AddConstructor<RushattackdsrRouting> ()
    .AddAttribute ("RouteCache",
                   "The route cache for saving routes from "
                   "route discovery process.",
                   PointerValue (0),
                   MakePointerAccessor (&RushattackdsrRouting::SetRouteCache,
                                        &RushattackdsrRouting::GetRouteCache),
                   MakePointerChecker<RushattackdsrRouteCache> ())
    .AddAttribute ("RreqTable",
                   "The request table to manage route requests.",
                   PointerValue (0),
                   MakePointerAccessor (&RushattackdsrRouting::SetRequestTable,
                                        &RushattackdsrRouting::GetRequestTable),
                   MakePointerChecker<RushattackdsrRreqTable> ())
    .AddAttribute ("PassiveBuffer",
                   "The passive buffer to manage "
                   "promisucously received passive ack.",
                   PointerValue (0),
                   MakePointerAccessor (&RushattackdsrRouting::SetPassiveBuffer,
                                        &RushattackdsrRouting::GetPassiveBuffer),
                   MakePointerChecker<RushattackdsrPassiveBuffer> ())
    .AddAttribute ("MaxSendBuffLen",
                   "Maximum number of packets that can be stored "
                   "in send buffer.",
                   UintegerValue (64),
                   MakeUintegerAccessor (&RushattackdsrRouting::m_maxSendBuffLen),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("MaxSendBuffTime",
                   "Maximum time packets can be queued in the send buffer .",
                   TimeValue (Seconds (30)),
                   MakeTimeAccessor (&RushattackdsrRouting::m_sendBufferTimeout),
                   MakeTimeChecker ())
    .AddAttribute ("MaxMaintLen",
                   "Maximum number of packets that can be stored "
                   "in maintenance buffer.",
                   UintegerValue (50),
                   MakeUintegerAccessor (&RushattackdsrRouting::m_maxMaintainLen),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("MaxMaintTime",
                   "Maximum time packets can be queued in maintenance buffer.",
                   TimeValue (Seconds (30)),
                   MakeTimeAccessor (&RushattackdsrRouting::m_maxMaintainTime),
                   MakeTimeChecker ())
    .AddAttribute ("MaxCacheLen",
                   "Maximum number of route entries that can be stored "
                   "in route cache.",
                   UintegerValue (64),
                   MakeUintegerAccessor (&RushattackdsrRouting::m_maxCacheLen),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("RouteCacheTimeout",
                   "Maximum time the route cache can be queued in "
                   "route cache.",
                   TimeValue (Seconds (300)),
                   MakeTimeAccessor (&RushattackdsrRouting::m_maxCacheTime),
                   MakeTimeChecker ())
    .AddAttribute ("MaxEntriesEachDst",
                   "Maximum number of route entries for a "
                   "single destination to respond.",
                   UintegerValue (20),
                   MakeUintegerAccessor (&RushattackdsrRouting::m_maxEntriesEachDst),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("SendBuffInterval",
                   "How often to check send buffer for packet with route.",
                   TimeValue (Seconds (500)),
                   MakeTimeAccessor (&RushattackdsrRouting::m_sendBuffInterval),
                   MakeTimeChecker ())
    .AddAttribute ("NodeTraversalTime",
                   "The time it takes to traverse two neighboring nodes.",
                   TimeValue (MilliSeconds (40)),
                   MakeTimeAccessor (&RushattackdsrRouting::m_nodeTraversalTime),
                   MakeTimeChecker ())
    .AddAttribute ("RreqRetries",
                   "Maximum number of retransmissions for "
                   "request discovery of a route.",
                   UintegerValue (16),
                   MakeUintegerAccessor (&RushattackdsrRouting::m_rreqRetries),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("MaintenanceRetries",
                   "Maximum number of retransmissions for "
                   "data packets from maintenance buffer.",
                   UintegerValue (2),
                   MakeUintegerAccessor (&RushattackdsrRouting::m_maxMaintRexmt),
                   MakeUintegerChecker<uint32_t> ())
    ...
    .AddAttribute ("EnableSubRoute",
                   ...
                   BooleanValue (true),
                   MakeBooleanAccessor (&RushattackdsrRouting::m_subRoute),
                   MakeBooleanChecker ())
    .AddAttribute ("RetransIncr",
                   "The increase time for retransmission timer "
                   "when facing network congestion",
                   TimeValue (MilliSeconds (20)),
                   MakeTimeAccessor (&RushattackdsrRouting::m_retransIncr),
                   MakeTimeChecker ())
    .AddAttribute ("MaxNetworkQueueSize",
                   "The max number of packet to save in the network queue.",
                   UintegerValue (400),
                   MakeUintegerAccessor (&RushattackdsrRouting::m_maxNetworkSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("MaxNetworkQueueDelay",
                   "The max time for a packet to stay in the network queue.",
                   TimeValue (Seconds (30.0)),
                   MakeTimeAccessor (&RushattackdsrRouting::m_maxNetworkDelay),
                   MakeTimeChecker ())
    .AddAttribute ("NumPriorityQueues",
                   "The max number of packet to save in the network queue.",
                   UintegerValue (2),
                   MakeUintegerAccessor (&RushattackdsrRouting::m_numPriorityQueues),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("LinkAcknowledgment",
                   "Enable Link layer acknowledgment mechanism",
                   BooleanValue (true),
                   MakeBooleanAccessor (&RushattackdsrRouting::m_linkAck),
                   MakeBooleanChecker ())
    .AddTraceSource ("Tx",
                     "Send DSR packet.",
                     MakeTraceSourceAccessor (&RushattackdsrRouting::m_txPacketTrace),
                     "ns3::rushattackdsr::RushattackdsrOptionSRHeader::TracedCallback")
    .AddTraceSource ("Drop",
                     "Drop DSR packet",
                     MakeTraceSourceAccessor (&RushattackdsrRouting::m_dropTrace),
                     "ns3::Packet::TracedCallback")
  ;
  return tid;
}

RushattackdsrRouting::RushattackdsrRouting ()
{
   ...
   */
  Ptr<rushattackdsr::RushattackdsrOptionPad1> pad1Option = CreateObject<rushattackdsr::RushattackdsrOptionPad1> ();
  Ptr<rushattackdsr::RushattackdsrOptionPadn> padnOption = CreateObject<rushattackdsr::RushattackdsrOptionPadn> ();
  Ptr<rushattackdsr::RushattackdsrOptionRreq> rreqOption = CreateObject<rushattackdsr::RushattackdsrOptionRreq> ();
  Ptr<rushattackdsr::RushattackdsrOptionRrep> rrepOption = CreateObject<rushattackdsr::RushattackdsrOptionRrep> ();
  Ptr<rushattackdsr::RushattackdsrOptionSR>   srOption = CreateObject<rushattackdsr::RushattackdsrOptionSR> ();
  Ptr<rushattackdsr::RushattackdsrOptionRerr>   rerrOption = CreateObject<rushattackdsr::RushattackdsrOptionRerr> ();
  Ptr<rushattackdsr::RushattackdsrOptionAckReq> ackReq = CreateObject<rushattackdsr::RushattackdsrOptionAckReq> ();
  Ptr<rushattackdsr::RushattackdsrOptionAck> ack = CreateObject<rushattackdsr::RushattackdsrOptionAck> ();
  ...
  m_sendBuffTimer.SetFunction (&RushattackdsrRouting::SendBuffTimerExpire, this);
...
void
RushattackdsrRouting::NotifyNewAggregate ()
{
  ...
  Simulator::ScheduleNow (&RushattackdsrRouting::Start, this);
}

void RushattackdsrRouting::Start ()
{
  ...
  for (uint32_t i = 0; i < m_numPriorityQueues; i++)
    {
      ...
      Ptr<rushattackdsr::RushattackdsrNetworkQueue> queue_i = CreateObject<rushattackdsr::RushattackdsrNetworkQueue> (m_maxNetworkSize,m_maxNetworkDelay);
      std::pair<std::map<uint32_t, Ptr<rushattackdsr::RushattackdsrNetworkQueue> >::iterator, bool> result_i = m_priorityQueue.insert (std::make_pair (i, queue_i));
      NS_ASSERT_MSG (result_i.second, "Error in creating queues");
    }
  Ptr<rushattackdsr::RushattackdsrRreqTable> rreqTable = CreateObject<rushattackdsr::RushattackdsrRreqTable> ();
  ...
  Ptr<rushattackdsr::RushattackdsrPassiveBuffer> passiveBuffer = CreateObject<rushattackdsr::RushattackdsrPassiveBuffer> ();
      ...
      for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
        {
          ...
          if (addr != loopback)
            {
              /*
               * Set rushattackdsr route cache
               */
              Ptr<rushattackdsr::RushattackdsrRouteCache> routeCache = CreateObject<rushattackdsr::RushattackdsrRouteCache> ();
              ...
              // routeCache->SetCallback (MakeCallback (&RushattackdsrRouting::SendRerrWhenBreaksLinkToNextHop, this));
              SetRouteCache (routeCache);
              // Set the main address as the current ip address
              m_mainAddress = addr;

              m_ipv4->GetNetDevice (1)->SetPromiscReceiveCallback (MakeCallback (&RushattackdsrRouting::PromiscReceive, this));
...
Ptr<NetDevice>
RushattackdsrRouting::GetNetDeviceFromContext (std::string context)
{
...
RushattackdsrRouting::GetElementsFromContext (std::string context)
{
...
void
RushattackdsrRouting::DoDispose (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  m_node = 0;
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
...
void
RushattackdsrRouting::SetNode (Ptr<Node> node)
{
...
Ptr<Node>
RushattackdsrRouting::GetNode () const
{
  NS_LOG_FUNCTION_NOARGS ();
  return m_node;
}

void RushattackdsrRouting::SetRouteCache (Ptr<rushattackdsr::RushattackdsrRouteCache> r)
{
  // / Set the route cache to use
  m_routeCache = r;
}

Ptr<rushattackdsr::RushattackdsrRouteCache>
RushattackdsrRouting::GetRouteCache () const
{
  // / Get the route cache to use
  return m_routeCache;
}

void RushattackdsrRouting::SetRequestTable (Ptr<rushattackdsr::RushattackdsrRreqTable> q)
{
  // / Set the request table to use
  m_rreqTable = q;
}

Ptr<rushattackdsr::RushattackdsrRreqTable>
RushattackdsrRouting::GetRequestTable () const
{
  // / Get the request table to use
  return m_rreqTable;
}

void RushattackdsrRouting::SetPassiveBuffer (Ptr<rushattackdsr::RushattackdsrPassiveBuffer> p)
{
  // / Set the request table to use
  m_passiveBuffer = p;
}

Ptr<rushattackdsr::RushattackdsrPassiveBuffer>
RushattackdsrRouting::GetPassiveBuffer () const
{
  // / Get the request table to use
...
Ptr<Node>
RushattackdsrRouting::GetNodeWithAddress (Ipv4Address ipv4Address)
{
...
bool RushattackdsrRouting::IsLinkCache ()
{
  return m_routeCache->IsLinkCache ();
}

void RushattackdsrRouting::UseExtends (RushattackdsrRouteCacheEntry::IP_VECTOR rt)
{
  m_routeCache->UseExtends (rt);
}

bool RushattackdsrRouting::LookupRoute (Ipv4Address id, RushattackdsrRouteCacheEntry & rt)
{
  return m_routeCache->LookupRoute (id, rt);
}

bool RushattackdsrRouting::AddRoute_Link (RushattackdsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address source)
{
...
bool RushattackdsrRouting::AddRoute (RushattackdsrRouteCacheEntry & rt)
{
...
void RushattackdsrRouting::DeleteAllRoutesIncludeLink (Ipv4Address errorSrc, Ipv4Address unreachNode, Ipv4Address node)
{
  m_routeCache->DeleteAllRoutesIncludeLink (errorSrc, unreachNode, node);
}

bool RushattackdsrRouting::UpdateRouteEntry (Ipv4Address dst)
{
  return m_routeCache->UpdateRouteEntry (dst);
}

bool RushattackdsrRouting::FindSourceEntry (Ipv4Address src, Ipv4Address dst, uint16_t id)
{
  return m_rreqTable->FindSourceEntry (src, dst, id);
}

Ipv4Address
RushattackdsrRouting::GetIPfromMAC (Mac48Address address)
{
...
void RushattackdsrRouting::PrintVector (std::vector<Ipv4Address>& vec)
{
...
Ipv4Address RushattackdsrRouting::SearchNextHop (Ipv4Address ipv4Address, std::vector<Ipv4Address>& vec)
{
  NS_LOG_FUNCTION (this << ipv4Address);
  Ipv4Address nextHop;
...
Ptr<Ipv4Route>
RushattackdsrRouting::SetRoute (Ipv4Address nextHop, Ipv4Address srcAddress)
{
...
int
RushattackdsrRouting::GetProtocolNumber (void) const
{
...
uint16_t
RushattackdsrRouting::GetIDfromIP (Ipv4Address address)
{
  int32_t nNodes = NodeList::GetNNodes ();
  for (int32_t i = 0; i < nNodes; ++i)
    {
...
Ipv4Address
RushattackdsrRouting::GetIPfromID (uint16_t id)
{
  if (id >= 256)
    {
...
uint32_t
RushattackdsrRouting::GetPriority (RushattackdsrMessageType messageType)
{
...
void RushattackdsrRouting::SendBuffTimerExpire ()
{
...
void RushattackdsrRouting::CheckSendBuffer ()
{
  NS_LOG_INFO (Simulator::Now ().GetSeconds ()
               << " Checking send buffer at " << m_mainAddress << " with size " << m_sendBuffer.GetSize ());


  for (std::vector<RushattackdsrSendBuffEntry>::iterator i = m_sendBuffer.GetBuffer ().begin (); i != m_sendBuffer.GetBuffer ().end (); )
    {
      NS_LOG_DEBUG ("Here we try to find the data packet in the send buffer");
      Ipv4Address destination = i->GetDestination ();
      RushattackdsrRouteCacheEntry toDst;
      ...
      if (findRoute)
        {
          ...
          RushattackdsrRoutingHeader rushattackdsrRoutingHeader;
          Ptr<Packet> copyP = packet->Copy ();
          Ptr<Packet> rushattackdsrPacket = packet->Copy ();
          rushattackdsrPacket->RemoveHeader (rushattackdsrRoutingHeader);
          uint32_t offset = rushattackdsrRoutingHeader.GetRushattackdsrOptionsOffset ();
          ...
          if (optionType == 3)
              ...
              if (errorType == 1) // This is the Route Error Option
                {
                  ...
                  RushattackdsrOptionSRHeader sourceRoute;
                  ...
                  if (nextHop == "0.0.0.0")
                    {
                      PacketNewRoute (rushattackdsrPacket, m_mainAddress, destination, protocol);
                      return;
                    }

                  SetRoute (nextHop, m_mainAddress);
                  uint8_t length = (sourceRoute.GetLength () + newUnreach.GetLength ());
                  rushattackdsrRoutingHeader.SetNextHeader (protocol);
                  rushattackdsrRoutingHeader.SetMessageType (1);
                  rushattackdsrRoutingHeader.SetSourceId (GetIDfromIP (m_mainAddress));
                  rushattackdsrRoutingHeader.SetDestId (255);
                  rushattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 4);
                  rushattackdsrRoutingHeader.AddRushattackdsrOption (newUnreach);
                  rushattackdsrRoutingHeader.AddRushattackdsrOption (sourceRoute);

                  Ptr<Packet> newPacket = Create<Packet> ();
                  newPacket->AddHeader (rushattackdsrRoutingHeader); // Add the routing header with rerr and sourceRoute attached to it
                  ...
                  std::map<uint32_t, Ptr<rushattackdsr::RushattackdsrNetworkQueue> >::iterator i = m_priorityQueue.find (priority);
                  Ptr<rushattackdsr::RushattackdsrNetworkQueue> rushattackdsrNetworkQueue = i->second;
                  ...
                  /// \todo New RushattackdsrNetworkQueueEntry
                  RushattackdsrNetworkQueueEntry newEntry (newPacket, m_mainAddress, nextHop, Simulator::Now (), m_ipv4Route);

                  if (rushattackdsrNetworkQueue->Enqueue (newEntry))
                    {
                      Scheduler (priority);
                    }
                  else
                    {
                      NS_LOG_INFO ("Packet dropped as rushattackdsr network queue is full");
                    }
                }
            }
          else
            {
              rushattackdsrRoutingHeader.SetNextHeader (protocol);
              rushattackdsrRoutingHeader.SetMessageType (2);
              rushattackdsrRoutingHeader.SetSourceId (GetIDfromIP (m_mainAddress));
              rushattackdsrRoutingHeader.SetDestId (GetIDfromIP (destination));
              ...
              rushattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 2);
              rushattackdsrRoutingHeader.AddRushattackdsrOption (sourceRoute);
              cleanP->AddHeader (rushattackdsrRoutingHeader);
              Ptr<const Packet> mtP = cleanP->Copy ();
              // Put the data packet in the maintenance queue for data packet retransmission
              RushattackdsrMaintainBuffEntry newEntry (/*Packet=*/ mtP, /*Ipv4Address=*/ m_mainAddress, /*nextHop=*/ nextHop,
              ...
              Simulator::Schedule (m_sendBuffInterval, &RushattackdsrRouting::SendBuffTimerExpire, this);
              return;
            }
        }
      else
...
bool RushattackdsrRouting::PromiscReceive (Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol, const Address &from,
                                 const Address &to, NetDevice::PacketType packetType)
{
  ...
  if (ipv4Header.GetProtocol () != RushattackdsrRouting::PROT_NUMBER)
    {
      return false;
    }
  // Remove the rushattackdsr routing header here
  Ptr<Packet> pktMinusRushattackdsrHdr = pktMinusIpHdr->Copy ();
  RushattackdsrRoutingHeader rushattackdsrRouting;
  pktMinusRushattackdsrHdr->RemoveHeader (rushattackdsrRouting);
  ...
  if (rushattackdsrRouting.GetMessageType () == 2 && ourAddress == m_mainAddress)
    {
      NS_LOG_DEBUG ("data packet receives " << packet->GetUid ());
      Ipv4Address sourceIp = GetIPfromID (rushattackdsrRouting.GetSourceId ());
      Ipv4Address destinationIp = GetIPfromID ( rushattackdsrRouting.GetDestId ());
      ...
      RushattackdsrMaintainBuffEntry newEntry;
      ...
      Ptr<rushattackdsr::RushattackdsrRouting> rushattackdsr = node->GetObject<rushattackdsr::RushattackdsrRouting> ();
      rushattackdsr->CancelLinkPacketTimer (newEntry);
    }
  ...
  if (packetType == NetDevice::PACKET_OTHERHOST)
    {
      ...
      uint8_t offset = rushattackdsrRouting.GetRushattackdsrOptionsOffset ();        // Get the offset for option header, 4 bytes in this case
      uint8_t nextHeader = rushattackdsrRouting.GetNextHeader ();
      uint32_t sourceId = rushattackdsrRouting.GetSourceId ();
      ...
      if (optionType == 96)        // This is the source route option
        {
          ...
          rushattackdsrOption = GetOption (optionType);       // Get the relative DSR option and demux to the process function
          NS_LOG_DEBUG (Simulator::Now ().GetSeconds () <<
                        ...
                        " and packet : " << *pktMinusRushattackdsrHdr);

          bool isPromisc = true;                     // Set the boolean value isPromisc as true
          rushattackdsrOption->Process (pktMinusIpHdr, pktMinusRushattackdsrHdr, m_mainAddress, source, ipv4Header, nextHeader, isPromisc, promiscSource);
          return true;
...
void
RushattackdsrRouting::PacketNewRoute (Ptr<Packet> packet,
                            Ipv4Address source,
  ...
  if (!findRoute)
    {
      ...
      RushattackdsrSendBuffEntry newEntry (p, destination, m_sendBufferTimeout, protocol);     // Create a new entry for send buffer
  ...
  else
    {
      ...
      rushattackdsrRoutingHeader.SetNextHeader (protocol);
      rushattackdsrRoutingHeader.SetMessageType (2);
      rushattackdsrRoutingHeader.SetSourceId (GetIDfromIP (source));
      rushattackdsrRoutingHeader.SetDestId (GetIDfromIP (destination));
...
void
RushattackdsrRouting::SendUnreachError (Ipv4Address unreachNode, Ipv4Address destination, Ipv4Address originalDst, uint8_t salvage, uint8_t protocol)
{
  ...
  RushattackdsrOptionRerrUnreachHeader rerrUnreachHeader;
  ...
  if (!findRoute)
    {
      if (destination == m_mainAddress)
      ...
      else
        {
          ...
          rushattackdsrRoutingHeader.SetPayloadLength (rerrLength + 2);
          rushattackdsrRoutingHeader.AddRushattackdsrOption (rerrUnreachHeader);
          newPacket->AddHeader (rushattackdsrRoutingHeader);
          Ptr<Packet> p = newPacket->Copy ();
          // Save the error packet in the error buffer
          RushattackdsrErrorBuffEntry newEntry (p, destination, m_mainAddress, unreachNode, m_sendBufferTimeout, protocol);
  ...
  else
    {
      ...
      std::map<uint32_t, Ptr<rushattackdsr::RushattackdsrNetworkQueue> >::iterator i = m_priorityQueue.find (priority);
      Ptr<rushattackdsr::RushattackdsrNetworkQueue> rushattackdsrNetworkQueue = i->second;
      NS_LOG_DEBUG ("Will be inserting into priority queue " << rushattackdsrNetworkQueue << " number: " << priority);
...
void
RushattackdsrRouting::ForwardErrPacket (RushattackdsrOptionRerrUnreachHeader &rerr,
                              RushattackdsrOptionSRHeader &sourceRoute,
                              Ipv4Address nextHop,
                              uint8_t protocol,
                              Ptr<Ipv4Route> route)
{
  ...
  NS_ASSERT_MSG (!m_downTarget.IsNull (), "Error, RushattackdsrRouting cannot send downward");
  RushattackdsrRoutingHeader rushattackdsrRoutingHeader;
  rushattackdsrRoutingHeader.SetNextHeader (protocol);
  rushattackdsrRoutingHeader.SetMessageType (1);
  rushattackdsrRoutingHeader.SetSourceId (GetIDfromIP (rerr.GetErrorSrc ()));
  rushattackdsrRoutingHeader.SetDestId (GetIDfromIP (rerr.GetErrorDst ()));

  uint8_t length = (sourceRoute.GetLength () + rerr.GetLength ());
  rushattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 4);
  rushattackdsrRoutingHeader.AddRushattackdsrOption (rerr);
  rushattackdsrRoutingHeader.AddRushattackdsrOption (sourceRoute);
  Ptr<Packet> packet = Create<Packet> ();
  packet->AddHeader (rushattackdsrRoutingHeader);
  Ptr<NetDevice> dev = m_ip->GetNetDevice (m_ip->GetInterfaceForAddress (m_mainAddress));
  ...
  /// \todo New RushattackdsrNetworkQueueEntry
  RushattackdsrNetworkQueueEntry newEntry (packet, m_mainAddress, nextHop, Simulator::Now (), route);
...
void
RushattackdsrRouting::Send (Ptr<Packet> packet,
                  Ipv4Address source,
                  Ipv4Address destination,
                  uint8_t protocol,
                  Ptr<Ipv4Route> route)
  ...
  else
    {
      ...
      if (!findRoute)
      ...
      else
        {
          ...
          // Put the data packet in the maintenance queue for data packet retransmission
          RushattackdsrMaintainBuffEntry newEntry (/*Packet=*/ mtP, /*ourAddress=*/ m_mainAddress, /*nextHop=*/ nextHop,
                                                  /*source=*/ source, /*destination=*/ destination, /*ackId=*/ 0,
                                                  /*SegsLeft=*/ nodeList.size () - 2, /*expire time=*/ m_maxMaintainTime);
          bool result = m_maintainBuffer.Enqueue (newEntry);       // Enqueue the packet the the maintenance buffer
          if (result)
          ...
          if (m_sendBuffer.GetSize () != 0 && m_sendBuffer.Find (destination))
            {
              ...
              Simulator::Schedule (MilliSeconds (m_uniformRandomVariable->GetInteger (0,100)),
                                   &RushattackdsrRouting::SendPacketFromBuffer, this, sourceRoute, nextHop, protocol);
            }
        }
    }
}
...
uint16_t
RushattackdsrRouting::AddAckReqHeader (Ptr<Packet>& packet, Ipv4Address nextHop)
{
  NS_LOG_FUNCTION (this << packet << nextHop);
  // This packet is used to peek option type
  Ptr<Packet> rushattackdsrP = packet->Copy ();
  Ptr<Packet> tmpP = packet->Copy ();

  RushattackdsrRoutingHeader rushattackdsrRoutingHeader;
  rushattackdsrP->RemoveHeader (rushattackdsrRoutingHeader);          // Remove the DSR header in whole
  uint8_t protocol = rushattackdsrRoutingHeader.GetNextHeader ();
  uint32_t sourceId = rushattackdsrRoutingHeader.GetSourceId ();
  uint32_t destinationId = rushattackdsrRoutingHeader.GetDestId ();
  uint32_t offset = rushattackdsrRoutingHeader.GetRushattackdsrOptionsOffset ();
  ...
  tmpP->RemoveHeader (sourceRoute);               // this is a clean packet without any rushattackdsr involved headers

  RushattackdsrOptionAckReqHeader ackReq;
  ...
  RushattackdsrRoutingHeader newRushattackdsrRoutingHeader;
  newRushattackdsrRoutingHeader.SetNextHeader (protocol);
  newRushattackdsrRoutingHeader.SetMessageType (2);
  newRushattackdsrRoutingHeader.SetSourceId (sourceId);
  newRushattackdsrRoutingHeader.SetDestId (destinationId);
  newRushattackdsrRoutingHeader.SetPayloadLength (length + 4);
  newRushattackdsrRoutingHeader.AddRushattackdsrOption (sourceRoute);
  newRushattackdsrRoutingHeader.AddRushattackdsrOption (ackReq);
  rushattackdsrP->AddHeader (newRushattackdsrRoutingHeader);
  // give the rushattackdsrP value to packet and then return
  packet = rushattackdsrP;
  return m_ackId;
}

void
RushattackdsrRouting::SendPacket (Ptr<Packet> packet, Ipv4Address source, Ipv4Address nextHop, uint8_t protocol)
{
  ...
  /// \todo New RushattackdsrNetworkQueueEntry
  RushattackdsrNetworkQueueEntry newEntry (packet, source, nextHop, Simulator::Now (), m_ipv4Route);
...
void
RushattackdsrRouting::Scheduler (uint32_t priority)
{
  NS_LOG_FUNCTION (this);
...
void
RushattackdsrRouting::PriorityScheduler (uint32_t priority, bool continueWithFirst)
{
  ...
  for (uint32_t i = priority; numPriorities < m_numPriorityQueues; numPriorities++)
    {
      std::map<uint32_t, Ptr<RushattackdsrNetworkQueue> >::iterator q = m_priorityQueue.find (i);
      Ptr<rushattackdsr::RushattackdsrNetworkQueue> rushattackdsrNetworkQueue = q->second;
      uint32_t queueSize = rushattackdsrNetworkQueue->GetSize ();
      if (queueSize == 0)
        {
      ...
      else
        {
          ...
          for (std::map<uint32_t, Ptr<rushattackdsr::RushattackdsrNetworkQueue> >::iterator j = m_priorityQueue.begin (); j != m_priorityQueue.end (); j++)
            {
          ...
          RushattackdsrNetworkQueueEntry newEntry;
          rushattackdsrNetworkQueue->Dequeue (newEntry);
          if (SendRealDown (newEntry))
            {
              NS_LOG_LOGIC ("Packet sent by Rushattackdsr. Calling PriorityScheduler after some time");
              ...
              Simulator::Schedule (MicroSeconds (m_uniformRandomVariable->GetInteger (0, 1000)),
                                   &RushattackdsrRouting::PriorityScheduler,this, i, false);
            }
          else
            {
              // packet was dropped by Rushattackdsr. Call scheduler immediately so that we can
              // send another packet immediately.
              NS_LOG_LOGIC ("Packet dropped by Rushattackdsr. Calling PriorityScheduler immediately");
              Simulator::Schedule (Seconds (0), &RushattackdsrRouting::PriorityScheduler, this, i, false);
            }
...
void
RushattackdsrRouting::IncreaseRetransTimer ()
{
  NS_LOG_FUNCTION (this);
  ...
  std::vector<RushattackdsrNetworkQueueEntry> newNetworkQueue = rushattackdsrNetworkQueue->GetQueue ();
  for (std::vector<RushattackdsrNetworkQueueEntry>::iterator i = newNetworkQueue.begin (); i != newNetworkQueue.end (); i++)
    {
...
bool
RushattackdsrRouting::SendRealDown (RushattackdsrNetworkQueueEntry & newEntry)
{
  NS_LOG_FUNCTION (this);
...
void
RushattackdsrRouting::SendPacketFromBuffer (RushattackdsrOptionSRHeader const &sourceRoute, Ipv4Address nextHop, uint8_t protocol)
{
  ...
  if (m_sendBuffer.Find (destination))
    {
      ...
      RushattackdsrSendBuffEntry entry;
      if (m_sendBuffer.Dequeue (destination, entry))
        {
          ...
          p->AddHeader (rushattackdsrRoutingHeader);
   ...
   */
    ...
    {
      RushattackdsrErrorBuffEntry entry;
              ...
              if (errorType == 1)
                {
                  ...
                  std::vector<Ipv4Address> nodeList = sourceRoute.GetNodesAddress ();
                  RushattackdsrRoutingHeader newRoutingHeader;
                  ...
                  newRoutingHeader.AddRushattackdsrOption (newUnreach);
                  newRoutingHeader.AddRushattackdsrOption (sourceRoute);
                  /// When found a route and use it, UseExtends to the link cache
                  if (m_routeCache->IsLinkCache ())
                    {
                      m_routeCache->UseExtends (nodeList);
...
bool
RushattackdsrRouting::PassiveEntryCheck (Ptr<Packet> packet, Ipv4Address source, Ipv4Address destination, uint8_t segsLeft,
                               uint16_t fragmentOffset, uint16_t identification, bool saveEntry)
{
  ...
  Ptr<Packet> p = packet->Copy ();
  // Here the segments left value need to plus one to check the earlier hop maintain buffer entry
  RushattackdsrPassiveBuffEntry newEntry;
  newEntry.SetPacket (p);
  ...
  if (m_passiveBuffer->AllEqual (newEntry) && (!saveEntry))
    {
      ...
      RushattackdsrMaintainBuffEntry mbEntry;
      mbEntry.SetPacket (p);
...
bool
RushattackdsrRouting::CancelPassiveTimer (Ptr<Packet> packet, Ipv4Address source, Ipv4Address destination,
                                uint8_t segsLeft)
{
  NS_LOG_FUNCTION (this << packet << source << destination << (uint32_t)segsLeft);
...
void
RushattackdsrRouting::CallCancelPacketTimer (uint16_t ackId, Ipv4Header const& ipv4Header, Ipv4Address realSrc, Ipv4Address realDst)
{
  ...
  RushattackdsrMaintainBuffEntry newEntry (/*Packet=*/ mainP, /*ourAddress=*/ sender, /*nextHop=*/ receiver,
...
void
RushattackdsrRouting::CancelPacketAllTimer (RushattackdsrMaintainBuffEntry & mb)
{
  NS_LOG_FUNCTION (this);
...
void
RushattackdsrRouting::CancelLinkPacketTimer (RushattackdsrMaintainBuffEntry & mb)
{
  NS_LOG_FUNCTION (this);
  LinkKey linkKey;
...
void
RushattackdsrRouting::CancelNetworkPacketTimer (RushattackdsrMaintainBuffEntry & mb)
{
  NS_LOG_FUNCTION (this);
  NetworkKey networkKey;
...
void
RushattackdsrRouting::CancelPassivePacketTimer (RushattackdsrMaintainBuffEntry & mb)
{
  NS_LOG_FUNCTION (this);
  PassiveKey passiveKey;
  passiveKey.m_ackId = 0;
...
void
RushattackdsrRouting::CancelPacketTimerNextHop (Ipv4Address nextHop, uint8_t protocol)
{
  NS_LOG_FUNCTION (this << nextHop << (uint32_t)protocol);

  RushattackdsrMaintainBuffEntry entry;
  ...
  if (m_maintainBuffer.Dequeue (nextHop, entry))
    {
      ...
      Ptr<Packet> rushattackdsrP = entry.GetPacket ()->Copy ();
      Ptr<Packet> p = rushattackdsrP->Copy ();
      Ptr<Packet> packet = rushattackdsrP->Copy ();
      RushattackdsrRoutingHeader rushattackdsrRoutingHeader;
      rushattackdsrP->RemoveHeader (rushattackdsrRoutingHeader);          // Remove the rushattackdsr header in whole
      uint32_t offset = rushattackdsrRoutingHeader.GetRushattackdsrOptionsOffset ();
      ...
      if (m_maintainBuffer.GetSize () && m_maintainBuffer.Find (nextHop))
        {
          ...
          Simulator::Schedule (MilliSeconds (m_uniformRandomVariable->GetInteger (0,100)),
                               &RushattackdsrRouting::CancelPacketTimerNextHop,this,nextHop,protocol);
        }
    }
  else
    {
...
void
RushattackdsrRouting::SalvagePacket (Ptr<const Packet> packet, Ipv4Address source, Ipv4Address dst, uint8_t protocol)
{
  ...
  RushattackdsrRoutingHeader rushattackdsrRoutingHeader;
  p->RemoveHeader (rushattackdsrRoutingHeader);
  // Remove offset of rushattackdsr routing header
  uint8_t offset = rushattackdsrRoutingHeader.GetRushattackdsrOptionsOffset ();
  ...
  if (findRoute && (salvage < m_maxSalvageCount))
    {
      ...
      newRushattackdsrRoutingHeader.SetNextHeader (protocol);
      newRushattackdsrRoutingHeader.SetMessageType (2);
      newRushattackdsrRoutingHeader.SetSourceId (GetIDfromIP (source));
      newRushattackdsrRoutingHeader.SetDestId (GetIDfromIP (dst));
      ...
      newRushattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 2);
      newRushattackdsrRoutingHeader.AddRushattackdsrOption (sourceRoute);
      p->AddHeader (newRushattackdsrRoutingHeader);
      ...
      /// \todo New RushattackdsrNetworkQueueEntry
      RushattackdsrNetworkQueueEntry newEntry (p, m_mainAddress, nextHop, Simulator::Now (), m_ipv4Route);
...
void
RushattackdsrRouting::ScheduleLinkPacketRetry (RushattackdsrMaintainBuffEntry & mb,
                                     uint8_t protocol)
{
  ...
  m_linkAckTimer[linkKey].SetFunction (&RushattackdsrRouting::LinkScheduleTimerExpire, this);
  m_linkAckTimer[linkKey].Remove ();
...
void
RushattackdsrRouting::SchedulePassivePacketRetry (RushattackdsrMaintainBuffEntry & mb,
                                        uint8_t protocol)
{
  NS_LOG_FUNCTION (this << (uint32_t)protocol);
  ...
  m_passiveAckTimer[passiveKey].SetFunction (&RushattackdsrRouting::PassiveScheduleTimerExpire, this);
  m_passiveAckTimer[passiveKey].Remove ();
...
void
RushattackdsrRouting::ScheduleNetworkPacketRetry (RushattackdsrMaintainBuffEntry & mb,
                                        bool isFirst,
                                        uint8_t protocol)
{
  Ptr<Packet> p = Create<Packet> ();
  Ptr<Packet> rushattackdsrP = Create<Packet> ();
  ...
  if (isFirst)
    {
      ...
      m_addressForwardTimer[networkKey].SetFunction (&RushattackdsrRouting::NetworkScheduleTimerExpire, this);
      m_addressForwardTimer[networkKey].Remove ();
  ...
  else
    {
      ...
      p = mb.GetPacket ()->Copy ();
      rushattackdsrP = mb.GetPacket ()->Copy ();
      ...
      NS_LOG_DEBUG ("The packet with rushattackdsr header " << rushattackdsrP->GetSize ());
      networkKey.m_ackId = mb.GetAckId ();
      networkKey.m_ourAdd = mb.GetOurAdd ();
      networkKey.m_nextHop = mb.GetNextHop ();
      networkKey.m_source = mb.GetSrc ();
...
void
RushattackdsrRouting::LinkScheduleTimerExpire  (RushattackdsrMaintainBuffEntry & mb,
                                      uint8_t protocol)
{
  NS_LOG_FUNCTION (this << (uint32_t)protocol);
  Ipv4Address nextHop = mb.GetNextHop ();
...
void
RushattackdsrRouting::PassiveScheduleTimerExpire  (RushattackdsrMaintainBuffEntry & mb,
                                         uint8_t protocol)
{
  NS_LOG_FUNCTION (this << (uint32_t)protocol);
  Ipv4Address nextHop = mb.GetNextHop ();
...
int64_t
RushattackdsrRouting::AssignStreams (int64_t stream)
{
...
void
RushattackdsrRouting::NetworkScheduleTimerExpire  (RushattackdsrMaintainBuffEntry & mb,
                                         uint8_t protocol)
{
  Ptr<Packet> p = mb.GetPacket ()->Copy ();
  Ipv4Address source = mb.GetSrc ();
...
void
RushattackdsrRouting::ForwardPacket (Ptr<const Packet> packet,
                           RushattackdsrOptionSRHeader &sourceRoute,
                           ...
                           Ptr<Ipv4Route> route)
{
  ...
  rushattackdsrRoutingHeader.SetMessageType (2);
  rushattackdsrRoutingHeader.SetSourceId (GetIDfromIP (source));
  rushattackdsrRoutingHeader.SetDestId (GetIDfromIP (targetAddress));
...
void
RushattackdsrRouting::SendInitialRequest (Ipv4Address source,
                                Ipv4Address destination,
                                uint8_t protocol)
{
  ...
  RushattackdsrOptionRreqHeader rreqHeader;                                  // has an alignment of 4n+0
  ...
  rushattackdsrRoutingHeader.AddRushattackdsrOption (rreqHeader);                      // Add the rreqHeader to the rushattackdsr extension header
...
void
RushattackdsrRouting::SendErrorRequest (RushattackdsrOptionRerrUnreachHeader &rerr, uint8_t protocol)
{
  NS_LOG_FUNCTION (this << (uint32_t)protocol);
  NS_ASSERT_MSG (!m_downTarget.IsNull (), "Error, RushattackdsrRouting cannot send downward");
  ...
  if (m_routeCache->LookupRoute (dst, toDst))
  ...
  else
    {
      ...
      rushattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 4);
      dstP->AddHeader (rushattackdsrRoutingHeader);
...
void
RushattackdsrRouting::CancelRreqTimer (Ipv4Address dst, bool isRemove)
{
...
void
RushattackdsrRouting::ScheduleRreqRetry (Ptr<Packet> packet, std::vector<Ipv4Address> address, bool nonProp, uint32_t requestId, uint8_t protocol)
{
  ...
  if (nonProp)
    {
      ...
      m_nonPropReqTimer[dst].SetFunction (&RushattackdsrRouting::RouteRequestTimerExpire, this);
      m_nonPropReqTimer[dst].Remove ();
  ...
  else
    {
      ...
      address.push_back (source);
      address.push_back (dst);
      m_addressReqTimer[dst].SetFunction (&RushattackdsrRouting::RouteRequestTimerExpire, this);
      m_addressReqTimer[dst].Remove ();
...
void
RushattackdsrRouting::RouteRequestTimerExpire (Ptr<Packet> packet, std::vector<Ipv4Address> address, uint32_t requestId, uint8_t protocol)
{
  NS_LOG_FUNCTION (this << packet << requestId << (uint32_t)protocol);
  // Get a clean packet without rushattackdsr header
  Ptr<Packet> rushattackdsrP = packet->Copy ();
  RushattackdsrRoutingHeader rushattackdsrRoutingHeader;
  rushattackdsrP->RemoveHeader (rushattackdsrRoutingHeader);          // Remove the rushattackdsr header in whole
  ...
  if (m_routeCache->LookupRoute (dst, toDst))
    {
      ...
      if (nextHop == "0.0.0.0")
        {
          NS_LOG_DEBUG ("Error next hop address");
          PacketNewRoute (rushattackdsrP, source, dst, protocol);
          return;
        }
      SetRoute (nextHop, m_mainAddress);
      CancelRreqTimer (dst, true);
...
void
RushattackdsrRouting::SendRequest (Ptr<Packet> packet,
                         Ipv4Address source)
{
  ...
  /// \todo New RushattackdsrNetworkQueueEntry
  RushattackdsrNetworkQueueEntry newEntry (packet, source, m_broadcast, Simulator::Now (), 0);
  if (rushattackdsrNetworkQueue->Enqueue (newEntry))
    {
      Scheduler (priority);
    }
...
void
RushattackdsrRouting::ScheduleInterRequest (Ptr<Packet> packet)
{
   ...
   */
  Simulator::Schedule (MilliSeconds (m_uniformRandomVariable->GetInteger (0, m_broadcastJitter)), &RushattackdsrRouting::SendRequest, this,
                       packet, m_mainAddress);
}

void
RushattackdsrRouting::SendGratuitousReply (Ipv4Address source, Ipv4Address srcAddress, std::vector<Ipv4Address> &nodeList, uint8_t protocol)
{
  ...
  if (!(m_graReply.FindAndUpdate (source, srcAddress, m_gratReplyHoldoff)))     // Find the gratuitous reply entry
    {
      ...
      RushattackdsrOptionRrepHeader rrep;
      ...
      rushattackdsrRoutingHeader.SetNextHeader (protocol);
      rushattackdsrRoutingHeader.SetMessageType (1);
      rushattackdsrRoutingHeader.SetSourceId (GetIDfromIP (replySrc));
      rushattackdsrRoutingHeader.SetDestId (GetIDfromIP (replyDst));

      uint8_t length = rrep.GetLength ();        // Get the length of the rrep header excluding the type header
      rushattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 2);
      rushattackdsrRoutingHeader.AddRushattackdsrOption (rrep);
      Ptr<Packet> newPacket = Create<Packet> ();
      newPacket->AddHeader (rushattackdsrRoutingHeader);
      /*
...
void
RushattackdsrRouting::SendReply (Ptr<Packet> packet,
                       Ipv4Address source,
                       Ipv4Address nextHop,
                       Ptr<Ipv4Route> route)
{
  ...
  /// \todo New RushattackdsrNetworkQueueEntry
  RushattackdsrNetworkQueueEntry newEntry (packet, source, nextHop, Simulator::Now (), route);
  if (rushattackdsrNetworkQueue->Enqueue (newEntry))
    {
      Scheduler (priority);
    }
...
void
RushattackdsrRouting::ScheduleInitialReply (Ptr<Packet> packet,
                                  Ipv4Address source,
                                  Ipv4Address nextHop,
                                  Ptr<Ipv4Route> route)
{
  NS_LOG_FUNCTION (this << packet << source << nextHop);
  Simulator::ScheduleNow (&RushattackdsrRouting::SendReply, this,
                          packet, source, nextHop, route);
}

void
RushattackdsrRouting::ScheduleCachedReply (Ptr<Packet> packet,
                                 Ipv4Address source,
                                 Ipv4Address destination,
                                 Ptr<Ipv4Route> route,
                                 double hops)
{
  ...
  Simulator::Schedule (Time (2 * m_nodeTraversalTime * (hops - 1 + m_uniformRandomVariable->GetValue (0,1))), &RushattackdsrRouting::SendReply, this, packet, source, destination, route);
}

void
RushattackdsrRouting::SendAck   (uint16_t ackId,
                       Ipv4Address destination,
                       Ipv4Address realSrc,
                       Ipv4Address realDst,
                       uint8_t protocol,
                       Ptr<Ipv4Route> route)
{
  ...
  RushattackdsrOptionAckHeader ack;
  /*
  ...
  rushattackdsrRoutingHeader.SetPayloadLength (uint16_t (length) + 2);
  rushattackdsrRoutingHeader.AddRushattackdsrOption (ack);
  ...
  NS_LOG_LOGIC ("Will be inserting into priority queue " << rushattackdsrNetworkQueue << " number: " << priority);

  //m_downTarget (packet, m_mainAddress, destination, GetProtocolNumber (), route);

  /// \todo New RushattackdsrNetworkQueueEntry
  RushattackdsrNetworkQueueEntry newEntry (packet, m_mainAddress, destination, Simulator::Now (), route);
  if (rushattackdsrNetworkQueue->Enqueue (newEntry))
    {
      Scheduler (priority);
    }
...
RushattackdsrRouting::Receive (Ptr<Packet> p,
                     Ipv4Header const &ip,
                     Ptr<Ipv4Interface> incomingInterface)
{
   ...
   */
  RushattackdsrRoutingHeader rushattackdsrRoutingHeader;
  packet->RemoveHeader (rushattackdsrRoutingHeader);          // Remove the DSR header in whole
  ...
  uint32_t offset = rushattackdsrRoutingHeader.GetRushattackdsrOptionsOffset ();        // Get the offset for option header, 8 bytes in this case
  ...
  rushattackdsrOption = GetOption (optionType);       // Get the relative rushattackdsr option and demux to the process function
  ...
  if (optionType == 1)        // This is the request option
    {
      ...
      rushattackdsrOption = GetOption (optionType);
      optionLength = rushattackdsrOption->Process (p, packet, m_mainAddress, source, ip, protocol, isPromisc, promiscSource);
      ...
      if (optionLength == 0)
      ...
      else
        {
          if (segmentsLeft == 0)
            {
              // / Get the next header
              uint8_t nextHeader = rushattackdsrRoutingHeader.GetNextHeader ();
  ...
  else
    {
      ...
      RushattackdsrOptionRerrUnsupportHeader rerrUnsupportHeader;
...
void
RushattackdsrRouting::SetDownTarget (DownTargetCallback callback)
{
  m_downTarget = callback;
}

void
RushattackdsrRouting::SetDownTarget6 (DownTargetCallback6 callback)
{
...
RushattackdsrRouting::GetDownTarget (void) const
{
  return m_downTarget;
}

IpL4Protocol::DownTargetCallback6
RushattackdsrRouting::GetDownTarget6 (void) const
{
  NS_FATAL_ERROR ("Unimplemented");
  return MakeNullCallback<void,Ptr<Packet>, Ipv6Address, Ipv6Address, uint8_t, Ptr<Ipv6Route> > ();
}

void RushattackdsrRouting::Insert (Ptr<rushattackdsr::RushattackdsrOptions> option)
{
  m_options.push_back (option);
}

Ptr<rushattackdsr::RushattackdsrOptions> RushattackdsrRouting::GetOption (int optionNumber)
{
  for (RushattackdsrOptionList_t::iterator i = m_options.begin (); i != m_options.end (); ++i)
    {

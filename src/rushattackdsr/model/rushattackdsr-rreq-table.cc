...
NS_LOG_COMPONENT_DEFINE ("RushattackdsrRreqTable");
  
namespace rushattackdsr {

NS_OBJECT_ENSURE_REGISTERED (RushattackdsrRreqTable);

TypeId RushattackdsrRreqTable::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::rushattackdsr::RushattackdsrRreqTable")
    .SetParent<Object> ()
    .SetGroupName ("Rushattackdsr")
    .AddConstructor<RushattackdsrRreqTable> ()
  ;
  return tid;
}

RushattackdsrRreqTable::RushattackdsrRreqTable ()
...
void
RushattackdsrRreqTable::RemoveLeastExpire ()
{
...
void
RushattackdsrRreqTable::FindAndUpdate (Ipv4Address dst)
{
...
void
RushattackdsrRreqTable::RemoveRreqEntry (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
  std::map<Ipv4Address, RreqTableEntry >::const_iterator i =
    m_rreqDstMap.find (dst);
...
uint32_t
RushattackdsrRreqTable::GetRreqCnt (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
  std::map<Ipv4Address, RreqTableEntry >::const_iterator i =
    m_rreqDstMap.find (dst);
 ...
 */
uint32_t
RushattackdsrRreqTable::CheckUniqueRreqId (Ipv4Address dst)
{
...
uint32_t
RushattackdsrRreqTable::GetRreqSize ()
{
...
void
RushattackdsrRreqTable::Invalidate ()
{
...
BlackList*
RushattackdsrRreqTable::FindUnidirectional (Ipv4Address neighbor)
{
...
bool
RushattackdsrRreqTable::MarkLinkAsUnidirectional (Ipv4Address neighbor, Time blacklistTimeout)
{
...
void
RushattackdsrRreqTable::PurgeNeighbor ()
{
  /*
...
bool
RushattackdsrRreqTable::FindSourceEntry (Ipv4Address src, Ipv4Address dst, uint16_t id)
{
  NS_LOG_FUNCTION (this << src << dst << id);
  RushattackdsrReceivedRreqEntry rreqEntry;
  rreqEntry.SetDestination (dst);
  rreqEntry.SetIdentification (id);
  std::list<RushattackdsrReceivedRreqEntry> receivedRreqEntryList;
  /*
   * this function will return false if the entry is not found, true if duplicate entry find
   */
  std::map<Ipv4Address, std::list<RushattackdsrReceivedRreqEntry> >::const_iterator i = m_sourceRreqMap.find (src);
  if (i == m_sourceRreqMap.end ())
  ...
  else
    {
      ...
      for (std::list<RushattackdsrReceivedRreqEntry>::const_iterator j = receivedRreqEntryList.begin (); j != receivedRreqEntryList.end (); ++j)
        {

...
class RushattackdsrOptions;
/**
 * \class RushattackdsrRouting
 * \brief Rushattackdsr Routing base
 */
class RushattackdsrRouting : public IpL4Protocol
{
public:
  /**
  ...
  /**
    * \brief Define the rushattackdsr protocol number.
    */
  static const uint8_t PROT_NUMBER;
  /**
   * \brief Constructor.
   */
  RushattackdsrRouting ();
  /**
   * \brief Destructor.
   */
  virtual ~RushattackdsrRouting ();
  /**
   * \brief Get the node.
   * \return the node
   */
   ...
   */
  void SetRouteCache (Ptr<rushattackdsr::RushattackdsrRouteCache> r);
  /**
   * \brief Get the route cache.
   * \return the route cache
   */
  Ptr<rushattackdsr::RushattackdsrRouteCache> GetRouteCache () const;
  /**
   * \brief Set the node.
   * \param r the request table to set
   */
  void SetRequestTable (Ptr<rushattackdsr::RushattackdsrRreqTable> r);
  /**
    * \brief Get the request table.
    * \return the request table
    */
  Ptr<rushattackdsr::RushattackdsrRreqTable> GetRequestTable () const;
  /**
   * \brief Set the node.
   * \param r the passive buffer to set
   */
  void SetPassiveBuffer (Ptr<rushattackdsr::RushattackdsrPassiveBuffer> r);
  /**
    * \brief Get the passive buffer
    * \return the passive buffer
    */
  Ptr<rushattackdsr::RushattackdsrPassiveBuffer> GetPassiveBuffer () const;

  /// functions used to direct to route cache
  //\{
  bool IsLinkCache ();
  void UseExtends (RushattackdsrRouteCacheEntry::IP_VECTOR rt);
  bool LookupRoute (Ipv4Address id, RushattackdsrRouteCacheEntry & rt);
  bool AddRoute_Link (RushattackdsrRouteCacheEntry::IP_VECTOR nodelist, Ipv4Address source);
  bool AddRoute (RushattackdsrRouteCacheEntry & rt);
  ...
  /**
    * \brief Get the rushattackdsr protocol number.
   ...
   */
  uint32_t GetPriority (RushattackdsrMessageType messageType);
  /**
   ...
   */
  void ForwardErrPacket (RushattackdsrOptionRerrUnreachHeader &rerr,
   ...
   */
  bool SendRealDown (RushattackdsrNetworkQueueEntry & newEntry);
  /**
   ...
   * \param protocol number
   */
  void SendPacketFromBuffer (RushattackdsrOptionSRHeader const &sourceRoute,
                             Ipv4Address nextHop,
                             uint8_t protocol);
  /**
  ...
  */
  void CancelPacketAllTimer (RushattackdsrMaintainBuffEntry & mb);
  /**
   ...
   */
  void CancelNetworkPacketTimer (RushattackdsrMaintainBuffEntry & mb);
  /**
   * \brief Cancel the passive packet retransmission timer for a specific maintenance entry
   * \param mb maintian byffer entry
   */
  void CancelPassivePacketTimer (RushattackdsrMaintainBuffEntry & mb);
  /**
   * \brief Cancel the link packet retransmission timer for a specific maintenance entry
   * \param mb maintian byffer entry
   */
  void CancelLinkPacketTimer (RushattackdsrMaintainBuffEntry & mb);
  /**
   ...
   */
  void ScheduleLinkPacketRetry   (RushattackdsrMaintainBuffEntry & mb,
                                  uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void SchedulePassivePacketRetry   (RushattackdsrMaintainBuffEntry & mb,
                                     uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void ScheduleNetworkPacketRetry   (RushattackdsrMaintainBuffEntry & mb,
                                     bool isFirst,
                                     uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void LinkScheduleTimerExpire  (RushattackdsrMaintainBuffEntry & mb,
                                 uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void NetworkScheduleTimerExpire  (RushattackdsrMaintainBuffEntry & mb,
                                    uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void PassiveScheduleTimerExpire  (RushattackdsrMaintainBuffEntry & mb,
                                    uint8_t protocol);
  /**
   ...
   * \param protocol the protocol number
   */
  void SendErrorRequest (RushattackdsrOptionRerrUnreachHeader &rerr, uint8_t protocol);
  /**
  ...
  /**
   * \brief Insert a new Rushattackdsr Option.
   * \param option the option to insert
   */
  void Insert (Ptr<rushattackdsr::RushattackdsrOptions> option);
  /**
   * \brief Get the option corresponding to optionNumber.
   * \param optionNumber the option number of the option to retrieve
   * \return a matching Rushattackdsr option
   */
  Ptr<rushattackdsr::RushattackdsrOptions> GetOption (int optionNumber);
  /**
 ...
 *          */
  ...
  TracedCallback <const RushattackdsrOptionSRHeader &> m_txPacketTrace; ///< packet trace callback
...
private:
   ...
   * \param protocol The protocol we receive, need to verify it is rushattackdsr protocol
   * \param from The from address we received the packet
   * \param to The address this packet is destined for
   * \param packetType The rushattackdsr packet type, 0 is for control packet, 1 for data packet
   ...
   */
  typedef std::list<Ptr<RushattackdsrOptions> > RushattackdsrOptionList_t;
  /**
   * \brief List of DSR Options supported.
   */
  RushattackdsrOptionList_t m_options;
  ...
  RushattackdsrSendBuffer m_sendBuffer;                           ///< The send buffer

  RushattackdsrErrorBuffer m_errorBuffer;                         ///< The error buffer to save the error messages
  ...
  RushattackdsrMaintainBuffer m_maintainBuffer;                   ///< The declaration of maintain buffer
  ...
  Ptr<rushattackdsr::RushattackdsrRouteCache> m_routeCache;                 ///< A "drop-front" queue used by the routing layer to cache routes found.

  Ptr<rushattackdsr::RushattackdsrRreqTable> m_rreqTable;                   ///< A "drop-front" queue used by the routing layer to cache route request sent.

  Ptr<rushattackdsr::RushattackdsrPassiveBuffer> m_passiveBuffer;           ///< A "drop-front" queue used by the routing layer to cache route request sent.
  ...
  std::map<uint32_t, Ptr<rushattackdsr::RushattackdsrNetworkQueue> > m_priorityQueue;   ///< priority queues

  RushattackdsrGraReply m_graReply;                               ///< The gratuitous route reply.

  RushattackdsrNetworkQueue m_networkQueue;                       ///< The network queue.

...
NS_LOG_COMPONENT_DEFINE ("RushattackdsrSendBuffer");

namespace rushattackdsr {

uint32_t
RushattackdsrSendBuffer::GetSize ()
{
  Purge ();
...
bool
RushattackdsrSendBuffer::Enqueue (RushattackdsrSendBuffEntry & entry)
{
  Purge ();
  for (std::vector<RushattackdsrSendBuffEntry>::const_iterator i = m_sendBuffer.begin (); i
       != m_sendBuffer.end (); ++i)
    {
...
void
RushattackdsrSendBuffer::DropPacketWithDst (Ipv4Address dst)
{
   ...
   */
  for (std::vector<RushattackdsrSendBuffEntry>::iterator i = m_sendBuffer.begin (); i
       != m_sendBuffer.end (); ++i)
    {
      if (IsEqual (*i, dst))
        {
  ...
  m_sendBuffer.erase (std::remove_if (m_sendBuffer.begin (), m_sendBuffer.end (),
                                      std::bind2nd (std::ptr_fun (RushattackdsrSendBuffer::IsEqual), dst)), m_sendBuffer.end ());
}

bool
RushattackdsrSendBuffer::Dequeue (Ipv4Address dst, RushattackdsrSendBuffEntry & entry)
{
  Purge ();
  /*
   * Dequeue the entry with destination address dst
   */
  for (std::vector<RushattackdsrSendBuffEntry>::iterator i = m_sendBuffer.begin (); i != m_sendBuffer.end (); ++i)
    {
...
bool
RushattackdsrSendBuffer::Find (Ipv4Address dst)
{
  /*
...
struct IsExpired
{
   ...
   */
  bool
  operator() (RushattackdsrSendBuffEntry const & e) const
  {
...
void
RushattackdsrSendBuffer::Drop (RushattackdsrSendBuffEntry en, std::string reason)
{

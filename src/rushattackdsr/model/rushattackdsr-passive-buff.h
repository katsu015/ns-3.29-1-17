 ...
 */
class RushattackdsrPassiveBuffEntry
{
public:
  /**
   * Construct a RushattackdsrPassiveBuffEntry with the given parameters
   *
   * \param pa packet
   ...
   */
  RushattackdsrPassiveBuffEntry (Ptr<const Packet> pa = 0, Ipv4Address d = Ipv4Address (), Ipv4Address s = Ipv4Address (),
   ...
   */
  bool operator== (RushattackdsrPassiveBuffEntry const & o) const
  {
...
/**
 * \ingroup rushattackdsr
 * \class RushattackdsrPassiveBuffer
 * \brief DSR passive buffer
 */
/************************************************************************************************************************/
class RushattackdsrPassiveBuffer  : public Object
{
public:
  /**
  ...
  RushattackdsrPassiveBuffer ();
  virtual ~RushattackdsrPassiveBuffer ();

  /// Push entry in queue, if there is no entry with the same packet and destination address in queue.
  /// \param entry Buffer Entry
  /// \return true on success adding the Entry.
  bool Enqueue (RushattackdsrPassiveBuffEntry & entry);
  /// Return first found (the earliest) entry for given destination
  /// \param [in] dst Entry destination
  /// \param [out] entry The Entry found (if any).
  /// \return true on success
  bool Dequeue (Ipv4Address dst, RushattackdsrPassiveBuffEntry & entry);
 ...
 /// \return true if an Entry was found and removed.
  bool AllEqual (RushattackdsrPassiveBuffEntry & entry);
  /// Number of entries
...
private:
  ...
  std::vector<RushattackdsrPassiveBuffEntry> m_passiveBuffer;
  /// Remove all expired entries
  void Purge ();
  /// Notify that packet is dropped from queue by timeout
  /// \param en BuffEntry Buffer entry
  /// \param reason Drop reason
  void Drop (RushattackdsrPassiveBuffEntry en, std::string reason);
  /// Notify that packet is dropped from queue by timeout
  /// \param en BuffEntry Buffer entry
  /// \param reason Drop reason
  void DropLink (RushattackdsrPassiveBuffEntry en, std::string reason);
  ...
  static bool LinkEqual (RushattackdsrPassiveBuffEntry en, const std::vector<Ipv4Address> link)
  {

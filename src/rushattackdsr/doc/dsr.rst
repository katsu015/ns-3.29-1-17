...
a packet buffer in rushattackdsr-rsendbuff.cc. The packet queue implements 
...
* The RushattackdsrFsHeader has added 3 fields: message type, source id, destination id, and these changes only for post-processing
...
* DSR works as a shim header between transport and network protocol, it needs its own forwarding mechanism, we are changing the packet transmission to hop-by-hop delivery, so we added two fields in rushattackdsr fixed header to notify packet delivery
...
To have a node run DSR, the easiest way would be to use the RushattackdsrHelper
and RushattackdsrMainHelpers in your simulation script. For instance:
...
The example scripts inside ``src/rushattackdsr/examples/`` demonstrate the use of DSR based nodes in different scenarios. 
The helper source can be found inside ``src/rushattackdsr/helper/rushattackdsr-main-helper.{h,cc}`` 
and ``src/rushattackdsr/helper/rushattackdsr-helper.{h,cc}``

Examples
********

The example can be found in ``src/rushattackdsr/examples/``:

* rushattackdsr.cc use DSR as routing protocol within a traditional MANETs environment[3].
...
* Unit tests have been written to verify the internals of DSR. This can be found in ``src/rushattackdsr/test/rushattackdsr-test-suite.cc``. These tests verify whether the methods inside DSR module which deal with packet buffer, headers work correctly.
...
The model is not fully compliant with :rfc:`4728`. As an example, Rushattackdsr fixed size header
...
[1] Original paper: http://www.monarch.cs.rice.edu/monarch-papers/rushattackdsr-chapter00.pdf

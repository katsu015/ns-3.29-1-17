...
do
  ...
  do
    tshark -r "${Scenario}p-$filenum-0.pcap" -Y "wlan.fc.type_subtype == 0x001c and wlan.ra==00:00:00:00:00:08" -T fields -E header=y -E separator=',' -e "frame.number" -e "frame.time_relative" -e "wlan.ta" -e "wlan.ra" > "${Scenario}p-seed$Seed-node$filenum.csv"
  done
  ...
  tshark -r "${Scenario}p-7-0.pcap" -Y "wlan.fc.type_subtype == 0x001b and wlan.ta==00:00:00:00:00:08" -T fields -E header=n -E separator=',' -e "frame.number" -e "frame.time_relative" -e "wlan.ta" -e "wlan.ra" > "${Scenario}p-seed$Seed-node7.csv"
  Seed=`echo "$Seed+1" | bc`

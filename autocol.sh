...
do
  ...
  if [ $Seed -eq $Finish_Seed ]; then
    ...
    : > allrtcheck${Scenario}p-run$Runset.csv
    : > all2${Scenario}p-run${Runset}.csv
    : > all2rtcheck${Scenario}p-run$Runset.csv
    for Seed in $(seq 1 $Finish_filenum);do
      ...
      wc -l < ${Scenario}p-seed$Seed-node12.csv >> "all2${Scenario}p-run${Runset}.csv"
      wc -l < "rtcheck${Scenario}p-seed$Seed-run$Runset-node12.csv" >> "all2rtcheck${Scenario}p-run$Runset.csv"
    done
  ...
  paste -d , all2${Scenario}p-run[${Seed}-$((Finish_Runset-1))].csv all2${Scenario}p-run${Finish_Runset}.csv > "all2${Scenario}p.csv"
  paste -d , all2rtcheck${Scenario}p-run[${Seed}-$((Finish_Runset-1))].csv all2rtcheck${Scenario}p-run${Finish_Runset}.csv > "all2rtcheck${Scenario}p.csv"
    exit 0
  fi
